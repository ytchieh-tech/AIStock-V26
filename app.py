...
APP_VERSION="V82.1 Research Engine Route Fixed"
APP_NAME="AI企業價值研究院"
...
/* V82.1 Research Engine Route Fixed responsive audit */
@media(max-width:768px){
...
# V82.1 Research Engine Route Fixed：擴充台股中文名稱對照；每位使用者使用自己的 session_state，不寫共用 watchlist.json
TW_STOCKS.update({
...
def maybe_reload(sec):
    # V82.1 Research Engine Route Fixed.2: 使用 Streamlit autorefresh，避免 browser reload 導致回首頁或股票重設
    if sec and sec > 0:
...
def effective_price(q, df):
    """V82.1 Research Engine Route Fixed: if Yahoo quote is N/A, use latest K-line close as backup so valuation models do not disappear."""
...
<div class="hero">
  <div style="display:flex;align-items:center;gap:10px;">
    ...
    <div>
      ...
      <div style="font-size:.78rem;color:#dbeafe;margin-top:2px;">
        V82.1 Research Engine Route Fixed｜企業評價 × 法人籌碼 × 融資融券燈號 × ESG永續 × 中文財報 × AI研究
      </div>
    </div>
  </div>
  <svg viewBox="0 0 1200 180" width="100%" height="140" style="margin-top:10px;border-radius:14px;border:1px solid rgba(148,163,184,.35);background:linear-gradient(135deg,#0f172a,#1e3a8a);">
    ...
    <text x="40" y="42" fill="#ffffff" font-size="28" font-weight="900">V82.1 Research Engine Route Fixed</text>
    ...
    <text x="40" y="158" fill="#dbeafe" font-size="13">V82 research engine banner</text>
  </svg>
</div>
...
with st.sidebar:
    ...
    # V82.1 Research Engine Route Fixed_SIDEBAR_SECTOR_FIX
...
def v76_ai_page(symbol,q,df,scores):
    st.markdown('## 🏛 V82.1 Research Engine Route Fixed.3')
...
def v762_banner():
    ...
    <div style="padding:28px;border-radius:22px;background:linear-gradient(135deg,#0f172a,#1d4ed8,#047857);color:white;margin:12px 0 22px 0;">
      ...
      <div style="font-size:20px;font-weight:800;margin-top:8px;">V82.1 Research Engine Route Fixed</div>
...
def v77_safe_df(df):
    """防止 Streamlit / PyArrow 因 mixed type、dict、list、None 造成 ArrowInvalid。"""
    try:
        if df is None:
            return pd.DataFrame()
        ...
        return pd.DataFrame({"訊息":[f"表格轉換失敗：{e}"]})

def v77_dataframe(df, **kwargs):
    return st.dataframe(v77_safe_df(df), **kwargs)

# 攔截所有 st.dataframe，避免 PyArrow 錯誤造成右側 log 爆量
try:
    if not hasattr(st, "_v77_original_dataframe"):
        st._v77_original_dataframe = st.dataframe
        def _v77_streamlit_dataframe(data=None, *args, **kwargs):
            try:
                if isinstance(data, pd.DataFrame):
                    data = v77_safe_df(data)
                return st._v77_original_dataframe(data, *args, **kwargs)
            except Exception as e:
                st.warning(f"表格顯示已防錯處理：{e}")
                try:
                    return st._v77_original_dataframe(v77_safe_df(data), *args, **kwargs)
                except Exception:
                    return None
        st.dataframe = _v77_streamlit_dataframe
except Exception:
    pass

# V77 DNA Master：在 V76.3 基礎上補齊更多常見股票與產業鏈
V77_DNA_EXTRA = [
    ("1513","中興電","上市","電子","電力重電","電力設備","變壓器/開關設備","電網/AI電力","中游"),
    ("1504","東元","上市","電子","電力重電","馬達/機電","馬達/機電系統","AI資料中心/電力","中游"),
    ("1519","華城","上市","電子","電力重電","變壓器","重電設備","電網/AI電力","中游"),
    ("1605","華新","上市","傳產","電線電纜","電纜/銅線","電線電纜","電網/基建","中游"),
    ("2357","華碩","上市","電子","電腦週邊","品牌電腦/伺服器","品牌/系統","AI PC/伺服器","中下游"),
    ("2324","仁寶","上市","電子","電腦週邊","NB/伺服器ODM","ODM","AI PC/伺服器","中游"),
    ("2353","宏碁","上市","電子","電腦週邊","品牌電腦","品牌/通路","AI PC","下游"),
    ("2360","致茂","上市","電子","測試設備","電源/半導體測試","測試設備","AI/電動車/半導體","上中游"),
    ("2368","金像電","上市","電子","PCB","伺服器PCB","高階PCB","AI Server","中游"),
    ("2376","技嘉","上市","電子","電腦週邊","主機板/伺服器","板卡/伺服器","AI PC/AI伺服器","中游"),
    ("2388","威盛","上市","電子","IC設計","CPU/AI邊緣","AIoT平台","AI Edge","上游"),
    ("2393","億光","上市","電子","LED","LED封裝","光電元件","車用/顯示","中游"),
    ("3008","大立光","上市","電子","光學","手機鏡頭","光學鏡頭","高階光學/AI視覺","中游"),
    ("3023","信邦","上市","電子","連接線束","線束/連接器","工業/車用線束","EV/工控/AI","中游"),
    ("3036","文曄","上市","電子","電子通路","半導體通路","代理/設計導入","半導體供應鏈","中下游"),
    ("3046","建碁","上市","電子","工業電腦","迷你電腦/IPC","邊緣運算設備","AIoT/工業電腦","中游"),
    ("3533","嘉澤","上市","電子","連接器","CPU Socket/連接器","高階連接器","AI Server","上游"),
    ("3540","曜越","上櫃","電子","散熱","散熱/機殼/電源","PC散熱與電源","電競/AI PC","中游"),
    ("3596","智易","上市","電子","網通","網通設備","寬頻/路由器/交換器","AI資料中心/家庭網路","中游"),
    ("3715","定穎投控","上市","電子","PCB","車用/高階PCB","PCB製造","車用/AI Server","中游"),
    ("4979","華星光","上櫃","電子","光通訊","光收發模組","光通訊模組","AI資料中心/光通訊","上游"),
    ("5388","中磊","上市","電子","網通","網通設備","寬頻/企業網通","家庭網路/電信設備","中游"),
    ("6176","瑞儀","上市","電子","光電","背光模組","顯示光學","面板/車用顯示","中游"),
    ("6274","台燿","上市","電子","PCB/CCL","銅箔基板","高速材料","AI Server/網通","上游"),
    ("6488","環球晶","上市","電子","半導體材料","矽晶圓","半導體矽晶圓","半導體上游","上游"),
    ("6669","緯穎","上市","電子","AI伺服器","雲端伺服器","伺服器整機","AI資料中心","中游"),
    ("6805","富世達","上市","電子","機構件","摺疊/伺服器機構件","精密機構","AI Server/消費電子","中游"),
    ("8210","勤誠","上市","電子","機構件","伺服器機殼","機構件","AI Server","中游"),
    ("8299","群聯","上市","電子","記憶體控制IC","NAND控制IC","控制晶片/模組","AI/邊緣儲存","上游"),
]

def v77_master_df():
    try:
        base = v763_master_df() if "v763_master_df" in globals() else pd.DataFrame()
    except Exception:
        base = pd.DataFrame()
    extra = pd.DataFrame(V77_DNA_EXTRA, columns=["code","name","market","level1","level2","level3","level4","level5","chain"])
...
def v77_row(x):
    code = str(x).upper().strip().replace(" ","").split(".")[0]
    df = v77_master_df()
    r = df[df["code"].astype(str)==code]
    return None if r.empty else r.iloc[0]
...
def clean_symbol(x): return v77_symbol(x)
def display_name(symbol): 
    s = v77_symbol(symbol)
    return f"{v77_name(s)} / {s}"
def stock_name_only(symbol): return v77_name(symbol)
def v755_stock_name(symbol): return v77_name(symbol)
def v756_name(symbol): return v77_name(symbol)
def v76_tw_name(symbol): return v77_name(symbol)
def v763_name(symbol): return v77_name(symbol)
def v76_normalize_symbol(symbol): return v77_symbol(symbol)
def v763_symbol(symbol): return v77_symbol(symbol)

def v77_profile(symbol):
    s = v77_symbol(symbol)
    r = v77_row(s)
    if r is None:
        return {
            "公司":v77_name(s),"代碼":s,"市場":"待確認",
            "Level 1":"待分類","Level 2":"待分類","Level 3":"待分類","Level 4":"待分類","Level 5":"待分類",
            "產業":"待分類","次產業":"待分類","產業鏈位置":"待確認","商業模式":"待確認",
            "產業成熟度":"待確認","產業景氣燈號":"⚪ 待確認","資料層":"未覆蓋"
        }
    return {
        "公司":r["name"],"代碼":s,"市場":r["market"],
        "Level 1":r["level1"],"Level 2":r["level2"],"Level 3":r["level3"],"Level 4":r["level4"],"Level 5":r["level5"],
        "產業":r["level2"],"次產業":r["level3"],"產業鏈位置":r["chain"],"商業模式":r["level3"],
        "產業成熟度":"成長期" if ("AI" in str(r["level5"]) or "電網" in str(r["level5"]) or "光通訊" in str(r["level5"])) else "成熟/循環",
        "產業景氣燈號":"🟢 熱絡" if ("AI" in str(r["level5"]) or "電網" in str(r["level5"]) or "光通訊" in str(r["level5"])) else "🟡 中立",
        "資料層":"官方代碼表 + V77 DNA Master"
    }

def v70_profile(symbol): return v77_profile(symbol)
def v75_profile(symbol): return v77_profile(symbol)
def v755_profile(symbol): return v77_profile(symbol)
def v756_profile(symbol): return v77_profile(symbol)
def v76_profile(symbol): return v77_profile(symbol)
def v761_profile(symbol): return v77_profile(symbol)
def v763_profile(symbol): return v77_profile(symbol)

def v77_company_dna_df(symbol):
    p = v77_profile(symbol)
    return pd.DataFrame([
        ["公司名稱", p["公司"]],
        ["股票代號", p["代碼"]],
        ["市場", p["市場"]],
...
def v756_company_dna_table(symbol): return v77_company_dna_df(symbol)
def v76_company_dna_df(symbol): return v77_company_dna_df(symbol)
def v762_company_dna_table(symbol): return v77_company_dna_df(symbol)

try:
    for _, r in v77_master_df().iterrows():
        sym = f"{r['code']}.TW" if r["market"] == "上市" else f"{r['code']}.TWO"
        CODE_NAME_MAP[sym] = r["name"]
        CODE_NAME_MAP[f"{r['code']}.TW"] = r["name"]
        CODE_NAME_MAP[f"{r['code']}.TWO"] = r["name"]
...
V77_GLOBAL_PEERS = {
    "晶圓代工": [("台積電","2330.TW","台灣","晶圓代工"),("聯電","2303.TW","台灣","成熟製程"),("世界先進","5347.TWO","台灣","特殊製程"),("GlobalFoundries","GFS","美國","成熟/特殊製程"),("Samsung Foundry","005930.KS","韓國","晶圓代工")],
    "IC設計": [("聯發科","2454.TW","台灣","IC設計"),("瑞昱","2379.TW","台灣","IC設計"),("NVIDIA","NVDA","美國","GPU/AI"),("AMD","AMD","美國","CPU/GPU"),("Broadcom","AVGO","美國","通訊/ASIC")],
    "AI伺服器": [("廣達","2382.TW","台灣","AI伺服器ODM"),("緯創","3231.TW","台灣","AI伺服器ODM"),("緯穎","6669.TW","台灣","雲端伺服器"),("Supermicro","SMCI","美國","AI伺服器"),("Dell","DELL","美國","企業伺服器")],
    "散熱": [("奇鋐","3017.TW","台灣","散熱模組"),("雙鴻","3324.TWO","台灣","散熱模組"),("健策","3653.TW","台灣","散熱/機構"),("Cooler Master","N/A","台灣","散熱品牌")],
    "PCB": [("台光電","2383.TW","台灣","CCL"),("欣興","3037.TW","台灣","ABF/PCB"),("南電","8046.TW","台灣","ABF/PCB"),("景碩","3189.TW","台灣","載板"),("健鼎","3044.TW","台灣","PCB")],
    "網通": [("智易","3596.TW","台灣","網通設備"),("中磊","5388.TW","台灣","網通設備"),("啟碁","6285.TW","台灣","無線通訊"),("Cisco","CSCO","美國","網通設備"),("Arista","ANET","美國","資料中心交換器")],
    "光通訊": [("華星光","4979.TWO","台灣","光通訊模組"),("聯亞","3081.TWO","台灣","光通訊磊晶"),("Coherent","COHR","美國","光通訊"),("Lumentum","LITE","美國","光通訊")],
    "電力重電": [("台達電","2308.TW","台灣","電源/電控"),("中興電","1513.TW","台灣","重電設備"),("華城","1519.TW","台灣","變壓器"),("Schneider","SU.PA","法國","電力設備"),("Eaton","ETN","美國","電力管理")],
    "電子通路": [("至上","8112.TW","台灣","半導體通路"),("豐藝","6189.TW","台灣","電子通路"),("大聯大","3702.TW","台灣","半導體通路"),("Arrow","ARW","美國","電子通路"),("Avnet","AVT","美國","電子通路")],
    "金融控股": [("富邦金","2881.TW","台灣","金控"),("國泰金","2882.TW","台灣","金控"),("玉山金","2884.TW","台灣","銀行金控"),("中信金","2891.TW","台灣","銀行金控"),("兆豐金","2886.TW","台灣","銀行金控")],
    "航運": [("長榮","2603.TW","台灣","貨櫃航運"),("陽明","2609.TW","台灣","貨櫃航運"),("萬海","2615.TW","台灣","貨櫃航運"),("Maersk","MAERSK-B.CO","丹麥","全球航運"),("Hapag-Lloyd","HLAG.DE","德國","全球航運")],
}

def v77_competitors(symbol):
    p = v77_profile(symbol)
    key = str(p["Level 2"])
    key3 = str(p["Level 3"])
    peers = None
    for k, v in V77_GLOBAL_PEERS.items():
        if k in key or k in key3:
            peers = v
            break
    rows = []
    if peers:
        ...
        df = v77_master_df()
        same = df[(df["level2"].astype(str) == key) & (df["code"].astype(str) != str(symbol).split(".")[0])].head(10)
        for _, r in same.iterrows():
            sym = f"{r['code']}.TW" if r["market"] == "上市" else f"{r['code']}.TWO"
            rows.append((r["name"], sym, "台灣", r["level3"]))
    if not rows:
        rows = [("同業資料待擴充","N/A","N/A","本公司產業DNA已建，但同業資料庫尚待補強")]
    return pd.DataFrame(rows, columns=["公司","代碼","國家","競爭/關聯角色"])

def v76_competitors(symbol): return v77_competitors(symbol)
def v76_competitor_mapping(symbol): return v77_competitors(symbol)
def v756_competitor_mapping(symbol): return v77_competitors(symbol)

def v77_esg_score(symbol):
    p = v77_profile(symbol)
    base = 68
    if p["Level 2"] in ["半導體","晶圓代工","IC設計","AI伺服器","電力重電"]:
        base += 4
    if "金融" in p["Level 1"]:
        base += 6
    if "AI" in p["Level 5"]:
        base += 3
    return min(95, base + abs(hash(str(symbol))) % 8)

def v77_esg_rank(symbol):
    p = v77_profile(symbol)
    df = v77_master_df()
    peers = df[df["level2"].astype(str) == str(p["Level 2"])]
    if peers.empty:
    ...
    if peers.empty:
        peers = df.head(30)
    rows = []
    for _, r in peers.head(50).iterrows():
        sym = f"{r['code']}.TW" if r["market"] == "上市" else f"{r['code']}.TWO"
        score = v77_esg_score(sym)
        rating = "AAA" if score>=90 else "AA" if score>=80 else "A" if score>=70 else "BBB" if score>=60 else "BB"
        rows.append([r["name"], sym, r["level2"], rating, f"{score:.1f}"])
    out = pd.DataFrame(rows, columns=["公司","代碼","產業","ESG評級","ESG分數"])
    out["__s"] = out["ESG分數"].astype(float)
    out = out.sort_values("__s", ascending=False).drop(columns=["__s"]).reset_index(drop=True)
    out.insert(0,"產業排名",range(1,len(out)+1))
    return out

def v76_esg_rank(symbol): return v77_esg_rank(symbol)
def v755_esg_rank(symbol, scores=None): return v77_esg_rank(symbol)

def v77_esg_valuation_detail(q=None, symbol=None):
    q = q or {}
    score = v77_esg_score(symbol or q.get("symbol",""))
    try:
        price = float(q.get("price", np.nan))
    except Exception:
        price = np.nan
    try:
        eps = float(q.get("eps", np.nan))
    except Exception:
        eps = np.nan
    try:
        pe0 = float(q.get("pe", np.nan))
    except Exception:
        pe0 = np.nan
    if not pd.notna(eps) or eps <= 0:
        eps = price / pe0 if pd.notna(price) and pd.notna(pe0) and pe0 > 0 else (price / 20 if pd.notna(price) else np.nan)
    base_pe = 18
    premium_rate = .20 if score >= 90 else .15 if score >= 80 else .10 if score >= 70 else .05 if score >= 60 else 0
    base_value = eps * base_pe if pd.notna(eps) else np.nan
    premium_amount = base_value * premium_rate if pd.notna(base_value) else np.nan
    fair_value = base_value + premium_amount if pd.notna(base_value) else np.nan
    df = pd.DataFrame([
        ["使用EPS", v77_fmt(eps), "優先使用 Yahoo EPS；缺值則由現價/PE 或 現價/20 代理。"],
        ["基礎PE", v77_fmt(base_pe), "目前使用保守基礎PE=18；後續可改為產業PE中位數。"],
        ["ESG分數", v77_fmt(score), "V77 ESG代理分數；正式版可串永續報告書/評級資料。"],
        ["ESG溢價率", f"{premium_rate*100:.1f}%", "60~69=5%；70~79=10%；80~89=15%；90以上=20%。"],
        ["ESG基礎估值", v77_fmt(base_value), f"EPS × 基礎PE = {v77_fmt(eps)} × {base_pe}"],
        ["ESG溢價股價金額", v77_fmt(premium_amount), f"ESG基礎估值 × ESG溢價率 = {v77_fmt(base_value)} × {premium_rate*100:.1f}%"],
        ["ESG合理價", v77_fmt(fair_value), f"ESG基礎估值 + ESG溢價金額 = {v77_fmt(base_value)} + {v77_fmt(premium_amount)}"],
        ["ESG牛市價", v77_fmt(fair_value*1.2 if pd.notna(fair_value) else np.nan), "ESG合理價 × 1.20"],
        ["ESG超級牛市價", v77_fmt(fair_value*1.5 if pd.notna(fair_value) else np.nan), "ESG合理價 × 1.50"],
    ], columns=["項目","數值","計算說明"])
    return df

def v77_calc_transparency(symbol, q=None, df=None, scores=None):
    q = q or {}
    rows = []
    # 企業評價情境
    try:
    ...
    try:
        ...
        eps = np.nan
    try:
        pe = float(q.get("pe", 18))
    except Exception:
        pe = 18
    if not pd.notna(eps) and pd.notna(price):
        eps = price / pe if pe > 0 else price/20
    if pd.notna(eps):
        conservative = eps * 15
        base = eps * 18
        bull = eps * 22
        rows += [
            ["保守價", v77_fmt(conservative), "EPS × 15", f"{v77_fmt(eps)} × 15", "納入"],
            ["基準價", v77_fmt(base), "EPS × 18", f"{v77_fmt(eps)} × 18", "納入"],
            ["樂觀價", v77_fmt(bull), "EPS × 22", f"{v77_fmt(eps)} × 22", "納入"],
        ]
    esg_df = v77_esg_valuation_detail(q, symbol)
    esg_val = esg_df[esg_df["項目"]=="ESG合理價"]["數值"].iloc[0] if not esg_df.empty else "N/A"
    rows.append(["ESG合理價", esg_val, "ESG基礎估值 + ESG溢價金額", "詳見ESG估值透明表", "納入"])
    vals = []
    for r in rows:
        try:
            ...
            pass
    consensus = float(np.nanmedian(vals)) if vals else np.nan
    rows.append(["AI共識價", v77_fmt(consensus), "納入模型價格中位數", "降低極端值影響", "最終"])
    return pd.DataFrame(rows, columns=["模型","價格","公式","使用數值","狀態"])

def v76_calc_transparency(symbol,q=None,df=None,scores=None): return v77_calc_transparency(symbol,q,df,scores)
def v763_master_panel():
    st.markdown("## 🇹🇼 V77 官方代碼中文名稱 + DNA Master")
    st.info("優先使用官方 TWSE/TPEx 代碼資料，再用 V77 DNA Master 補齊產業分級；不再使用 Yahoo 英文名稱當公司名。")
    st.dataframe(v77_master_df(), use_container_width=True, hide_index=True)
...
def v77_banner():
    ...
    <div style="padding:30px;border-radius:24px;background:linear-gradient(135deg,#0f172a,#1d4ed8,#047857);color:white;margin:10px 0 22px 0;">
      ...
      <div style="font-size:22px;font-weight:800;margin-top:8px;">V82.1 Research Engine Route Fixed</div>
      <div style="font-size:15px;margin-top:8px;opacity:.92;">公司DNA × 全球競爭資料庫 × ESG股價溢價 × AI共識價透明化</div>
    </div>
    """, unsafe_allow_html=True)

def v77_ai_page(symbol, q, df, scores):
    st.markdown("## 🏛 AI研究院 Pro / V77")
    tabs = st.tabs(["🧬公司DNA","🌱ESG排名","🌍競爭/同業","💰ESG估值透明","🔍AI共識價透明"])
    with tabs[0]:
        st.dataframe(v77_company_dna_df(symbol), use_container_width=True, hide_index=True)
    with tabs[1]:
        st.dataframe(v77_esg_rank(symbol), use_container_width=True, hide_index=True)
    with tabs[2]:
        st.dataframe(v77_competitors(symbol), use_container_width=True, hide_index=True)
    with tabs[3]:
        st.dataframe(v77_esg_valuation_detail(q, symbol), use_container_width=True, hide_index=True)
    with tabs[4]:
        st.dataframe(v77_calc_transparency(symbol, q, df, scores), use_container_width=True, hide_index=True)

def v76_ai_page(symbol, q, df, scores): return v77_ai_page(symbol, q, df, scores)
def v756_ai_page(symbol, q, df, scores): return v77_ai_page(symbol, q, df, scores)
...
def v80_safe_df(df):
    try:
        ...
        return pd.DataFrame({"訊息":[f"表格防錯處理：{e}"]})

try:
    if not hasattr(st, "_v80_original_dataframe"):
        st._v80_original_dataframe = st.dataframe
        def _v80_dataframe(data=None, *args, **kwargs):
            try:
                if isinstance(data, pd.DataFrame):
                    data = v80_safe_df(data)
                return st._v80_original_dataframe(data, *args, **kwargs)
            except Exception as e:
                st.warning(f"表格已自動防錯：{e}")
                return st._v80_original_dataframe(v80_safe_df(data), *args, **kwargs)
        st.dataframe = _v80_dataframe
except Exception:
    pass

def v80_quote(symbol):
    try:
        return v755_get_quote(symbol)
    except Exception:
        try:
...
def v80_profile(symbol):
    try:
        return v77_profile(symbol)
    except Exception:
        try:
            return v763_profile(symbol)
        except Exception:
            s = str(symbol).upper()
            return {"公司":s,"代碼":s,"Level 1":"待分類","Level 2":"待分類","Level 3":"待分類","Level 4":"待分類","Level 5":"待分類","產業鏈位置":"待確認","商業模式":"待確認","產業景氣燈號":"⚪ 待確認","產業成熟度":"待確認"}

def v80_company_quality(symbol, q=None):
    p = v80_profile(symbol)
    q = q or v80_quote(symbol)
    roe = v80_num(q.get("roe"), np.nan)
    pe = v80_num(q.get("pe"), np.nan)
    pb = v80_num(q.get("pb"), np.nan)
    moat = 55
    if "AI" in str(p.get("Level 5","")): moat += 10
    if str(p.get("Level 2","")) in ["晶圓代工","IC設計","AI伺服器","半導體","電力重電"]: moat += 8
    if "金融" in str(p.get("Level 1","")): moat += 5
    if pd.notna(roe): moat += min(15, max(-10, roe*50))
    moat = max(0, min(100, moat))
    quality = max(0, min(100, moat*0.55 + (70 if pd.notna(pe) else 50)*0.2 + (65 if pd.notna(pb) else 50)*0.25))
    return pd.DataFrame([
        ["企業品質分數", v80_fmt(quality), "護城河、財報可得性、估值資料完整度綜合"],
        ["護城河評分", v80_fmt(moat), "技術、規模、客戶黏著、產業地位代理"],
        ["商業模式", p.get("商業模式","待確認"), "Company DNA"],
        ["產業鏈位置", p.get("產業鏈位置","待確認"), "Company DNA"],
        ["產業成熟度", p.get("產業成熟度","待確認"), "產業資料庫"],
    ], columns=["項目","數值","說明"])

def v80_industry_research(symbol):
    p = v80_profile(symbol)
    l2 = str(p.get("Level 2","待分類"))
    l5 = str(p.get("Level 5",""))
    health = 60
    if any(k in l2+l5 for k in ["AI","伺服器","電力","半導體","光通訊","散熱"]): health += 20
    if any(k in l2 for k in ["面板","航運","鋼鐵","塑化"]): health -= 5
    health = max(0, min(100, health))
    return pd.DataFrame([
        ["Level 1", p.get("Level 1","待分類"), "市場大類"],
        ["Level 2", p.get("Level 2","待分類"), "主要產業"],
        ["Level 3", p.get("Level 3","待分類"), "次產業"],
        ["產業健康度", v80_fmt(health), "景氣、成長主題、產業循環代理"],
        ["景氣燈號", "🟢 擴張" if health>=80 else "🟡 中立" if health>=55 else "🔴 低迷", "產業健康度轉換"],
        ["主要驅動", p.get("Level 5","待分類"), "投資主題與需求來源"],
        ["主要風險", "資本支出循環、庫存循環、利率、終端需求", "產業共通風險"],
    ], columns=["項目","內容","說明"])

def v80_competitor_center(symbol):
    try:
        df = v77_competitors(symbol)
        if isinstance(df, pd.DataFrame) and not df.empty:
            return df
    except Exception:
        pass
    return pd.DataFrame([["同業資料待擴充","N/A","N/A","為避免錯誤，不用無關產業代替"]], columns=["公司","代碼","國家","競爭/關聯角色"])

def v80_peer_compare(symbol):
    comps = v80_competitor_center(symbol)
    rows = []
    for _, r in comps.iterrows():
        sym = r.get("代碼","")
        if sym == "N/A":
            rows.append([r.get("公司","N/A"), sym, r.get("競爭/關聯角色",""), "N/A","N/A","N/A","N/A","待擴充"])
            continue
        q = v80_quote(sym)
        rows.append([r.get("公司",""), sym, r.get("競爭/關聯角色",""), v80_fmt(q.get("price")), v80_fmt(q.get("eps")), v80_fmt(q.get("pe")), v80_fmt(q.get("pb")), "Yahoo/內建代理"])
    return pd.DataFrame(rows, columns=["公司","代碼","角色","現價","EPS","PE","PB","資料來源"])

def v80_esg_value(symbol, q=None):
    q = q or v80_quote(symbol)
    try:
        return v77_esg_valuation_detail(q, symbol)
    except Exception:
        price = v80_num(q.get("price"))
        eps = v80_num(q.get("eps"))
        pe = v80_num(q.get("pe"))
        ...
        prem = 0.10
        base = eps * base_pe if pd.notna(eps) else np.nan
        plus = base * prem if pd.notna(base) else np.nan
        fair = base + plus if pd.notna(base) else np.nan
        return pd.DataFrame([
            ["使用EPS", v80_fmt(eps), "Yahoo EPS或價格/PE反推"],
            ["基礎PE", v80_fmt(base_pe), "基礎估值倍數"],
            ["ESG分數", v80_fmt(score), "代理ESG分數"],
            ["ESG溢價率", f"{prem*100:.1f}%", "產業相對ESG溢價"],
            ["ESG基礎估值", v80_fmt(base), "EPS × 基礎PE"],
            ["ESG溢價股價金額", v80_fmt(plus), "基礎估值 × ESG溢價率"],
            ["ESG合理價", v80_fmt(fair), "基礎估值 + ESG溢價"],
        ], columns=["項目","數值","計算說明"])

def v80_institutional_center(symbol, q=None, df=None, scores=None):
    q = q or v80_quote(symbol)
    scores = scores or {}
    tech = v80_num(scores.get("tech", 50), 50)
    inst = v80_num(scores.get("inst", 55), 55)
    main = v80_num(scores.get("main", inst), inst)
    foreign = max(0, min(100, inst + (tech-50)*0.20))
    trust = max(0, min(100, inst + (main-50)*0.15))
    dealer = max(0, min(100, (foreign+trust+tech)/3))
    consensus = foreign*0.45 + trust*0.30 + dealer*0.25
    qfactor = 1 + max(-0.18, min(0.22, (consensus-60)/180))
    price = v80_num(q.get("price"))
    eps = v80_num(q.get("eps"))
    pe = v80_num(q.get("pe"), 18)
    target = eps*pe*qfactor if pd.notna(eps) else (price*qfactor if pd.notna(price) else np.nan)
    return pd.DataFrame([
        ["外資評分", v80_fmt(foreign), "法人分數+技術動能代理"],
        ["投信評分", v80_fmt(trust), "法人分數+主力籌碼代理"],
        ["自營商評分", v80_fmt(dealer), "法人與技術平均代理"],
        ["法人共識分數", v80_fmt(consensus), "外資45% + 投信30% + 自營25%"],
        ["法人估值係數", v80_fmt(qfactor), "1 + (法人共識-60)/180，上下限修正"],
        ["法人共識價", v80_fmt(target), "EPS×PE×法人係數；EPS缺值改用現價×係數"],
    ], columns=["項目","數值","計算說明"])

def v80_valuation_weights(symbol):
    p = v80_profile(symbol)
    l1, l2 = str(p.get("Level 1","")), str(p.get("Level 2",""))
    if "金融" in l1:
        rows = [["PB",40],["ROE/EBO",30],["ESG",15],["法人",15]]
    elif any(k in l2 for k in ["AI伺服器","IC設計","半導體","晶圓代工","電力重電"]):
        rows = [["DCF",30],["PE",20],["EVA/EBO",20],["PEG/成長",15],["ESG",10],["法人",5]]
    elif any(k in l2 for k in ["航運","鋼鐵","塑化","面板"]):
        rows = [["PB/NAV",30],["PE",25],["景氣循環",20],["法人",15],["ESG",10]]
    else:
        rows = [["PE",25],["PB",20],["DCF",20],["EBO",15],["ESG",10],["法人",10]]
    return pd.DataFrame(rows, columns=["模型","權重%"])

def v80_fair_value(symbol, q=None, scores=None):
    q = q or v80_quote(symbol)
    price = v80_num(q.get("price"))
    eps = v80_num(q.get("eps"))
    pe = v80_num(q.get("pe"))
    ...
    values, rows = [], []
    if pd.notna(eps):
        for name, mult in [("PE保守價",15),("PE基準價",18),("PE樂觀價",22)]:
            val = eps*mult
            rows.append([name, v80_fmt(val), f"EPS({v80_fmt(eps)}) × {mult}", "納入"])
            values.append(val)
    try:
        esg = v80_esg_value(symbol, q)
        v = esg[esg["項目"].astype(str).str.contains("ESG合理價")]["數值"].iloc[-1]
        ev = v80_num(v)
        if pd.notna(ev):
            rows.append(["ESG合理價", v80_fmt(ev), "ESG基礎估值 + ESG溢價金額", "納入"])
            values.append(ev)
    except Exception:
        pass
    try:
        inst = v80_institutional_center(symbol, q, None, scores)
        v = inst[inst["項目"]=="法人共識價"]["數值"].iloc[0]
        iv = v80_num(v)
        if pd.notna(iv):
            rows.append(["法人共識價", v80_fmt(iv), "EPS×PE×法人係數", "納入"])
            values.append(iv)
    except Exception:
        pass
    consensus = float(np.nanmedian(values)) if values else np.nan
    rows.append(["AI企業合理價", v80_fmt(consensus), "所有納入模型之中位數，降低極端值影響", "最終"])
    return pd.DataFrame(rows, columns=["模型","合理價","計算方式","狀態"])

def v80_decision_center(symbol, q=None, scores=None):
    q = q or v80_quote(symbol)
    price = v80_num(q.get("price"))
    fvdf = v80_fair_value(symbol, q, scores)
    fair = v80_num(fvdf[fvdf["模型"]=="AI企業合理價"]["合理價"].iloc[0]) if not fvdf.empty else np.nan
    upside = (fair/price-1)*100 if pd.notna(fair) and pd.notna(price) and price>0 else np.nan
    if pd.isna(upside): decision = "資料不足"
    elif upside >= 30: decision = "強力買進"
    elif upside >= 15: decision = "買進"
    elif upside >= -10: decision = "中立"
    elif upside >= -25: decision = "減碼"
    else: decision = "賣出"
    return pd.DataFrame([
        ["目前股價", v80_fmt(price), "Yahoo/即時價格"],
        ["AI企業合理價", v80_fmt(fair), "企業評價+ESG+法人之中位數"],
        ["潛在空間", f"{upside:.1f}%" if pd.notna(upside) else "N/A", "合理價 / 現價 - 1"],
        ["投資結論", decision, "依潛在空間分級"],
        ["補充說明", "研究與教學用途，非投資建議", "需搭配風險與個人資金控管"],
    ], columns=["項目","結果","說明"])
...
def v80_enterprise_value_page(symbol, q, df, scores):
    v80_banner()
    st.markdown(f"### 研究標的：{display_name(symbol) if 'display_name' in globals() else symbol}")
    tabs = st.tabs(["①公司DNA", "②產業研究", "③全球競爭", "④同業比較", "⑤企業評價", "⑥ESG估值", "⑦法人籌碼", "⑧AI共識價", "⑨投資決策"])
    with tabs[0]:
        try: st.dataframe(v77_company_dna_df(symbol), use_container_width=True, hide_index=True)
        except Exception: st.dataframe(pd.DataFrame([v80_profile(symbol)]), use_container_width=True, hide_index=True)
        st.markdown("#### 企業品質 / 護城河")
        st.dataframe(v80_company_quality(symbol, q), use_container_width=True, hide_index=True)
    with tabs[1]: st.dataframe(v80_industry_research(symbol), use_container_width=True, hide_index=True)
    with tabs[2]: st.dataframe(v80_competitor_center(symbol), use_container_width=True, hide_index=True)
    with tabs[3]: st.dataframe(v80_peer_compare(symbol), use_container_width=True, hide_index=True)
    with tabs[4]:
        st.markdown("#### 產業自動權重")
        st.dataframe(v80_valuation_weights(symbol), use_container_width=True, hide_index=True)
        st.markdown("#### AI企業評價")
        st.dataframe(v80_fair_value(symbol, q, scores), use_container_width=True, hide_index=True)
    with tabs[5]: st.dataframe(v80_esg_value(symbol, q), use_container_width=True, hide_index=True)
    with tabs[6]: st.dataframe(v80_institutional_center(symbol, q, df, scores), use_container_width=True, hide_index=True)
    with tabs[7]: st.dataframe(v80_fair_value(symbol, q, scores), use_container_width=True, hide_index=True)
    with tabs[8]: st.dataframe(v80_decision_center(symbol, q, scores), use_container_width=True, hide_index=True)

def v77_ai_page(symbol, q, df, scores): return v80_enterprise_value_page(symbol, q, df, scores)
def v76_ai_page(symbol, q, df, scores): return v80_enterprise_value_page(symbol, q, df, scores)
def v756_ai_page(symbol, q, df, scores): return v80_enterprise_value_page(symbol, q, df, scores)
# ================= V80 ENTERPRISE VALUATION INSTITUTE LAYER END =================

# ================= V81 INSTITUTIONAL EDITION TRUE INTEGRATION LAYER =================
APP_BRAND = "AI企業價值研究院"
APP_VERSION="V82.1 Research Engine Route Fixed"

def v81_num(x, default=np.nan):
    try:
        if x is None or str(x).strip() in ["", "N/A", "None", "nan"]:
            return default
...
try:
    if not hasattr(st, "_v81_original_dataframe"):
        st._v81_original_dataframe = st.dataframe
        def _v81_dataframe(data=None, *args, **kwargs):
            try:
                if isinstance(data, pd.DataFrame):
                    data = v81_safe_df(data)
                return st._v81_original_dataframe(data, *args, **kwargs)
            except Exception as e:
                st.warning(f"表格已自動防錯：{e}")
                return st._v81_original_dataframe(v81_safe_df(data), *args, **kwargs)
        st.dataframe = _v81_dataframe
except Exception:
    pass

try:
    if not hasattr(st, "_v81_original_radio"):
        st._v81_original_radio = st.radio
        def _v81_radio(label, options, *args, **kwargs):
            try:
                if str(label).strip() == "主選單":
                    return st._v81_original_radio(label, ["🏠首頁","📊監控","📈K線","📄中文財報","🏛企業價值研究院","⚙設定"], *args, **kwargs)
            except Exception:
                pass
            return st._v81_original_radio(label, options, *args, **kwargs)
        st.radio = _v81_radio
except Exception:
    pass

def v81_quote(symbol):
    for fn in ["v755_get_quote", "v77_get_quote", "get_quote"]:
        try:
            if fn in globals():
                q = globals()[fn](symbol)
...
def v81_profile(symbol):
    for fn in ["v77_profile", "v763_profile", "v76_profile", "v756_profile"]:
        try:
            if fn in globals():
                p = globals()[fn](symbol)
            ...
            pass
    s = str(symbol).upper()
    return {"公司":s,"代碼":s,"市場":"待確認","Level 1":"待分類","Level 2":"待分類","Level 3":"待分類","Level 4":"待分類","Level 5":"待分類","產業鏈位置":"待確認","商業模式":"待確認","產業景氣燈號":"⚪ 待確認","產業成熟度":"待確認"}

def v81_stock_display(symbol):
    try:
        return display_name(symbol)
    except Exception:
        p = v81_profile(symbol)
        return f"{p.get('公司', symbol)} / {p.get('代碼', symbol)}"

def v81_model_template(symbol):
    p = v81_profile(symbol)
    text = "".join([str(p.get(k,"")) for k in ["Level 1","Level 2","Level 3","Level 4","Level 5","商業模式"]])
    if "金融" in text:
        return "金融股模型", "金融股價值主要由淨值、ROE、資本品質、股利與風險控管驅動。"
    if any(k in text for k in ["設備", "自動化", "AOI", "測試"]):
        return "成長型設備股模型", "設備股受產業景氣、訂單週期與成長率影響，估值重視PE/PEG/EBO與產業前景。"
    if any(k in text for k in ["晶圓代工", "半導體", "IC設計", "AI"]):
    ...
    if any(k in text for k in ["營建", "建設", "不動產"]):
        return "資產/NAV模型", "營建資產類企業估值重視NAV、土地資產、現金流與去化速度。"
    return "一般企業價值模型", "資料不足以歸類到特殊模板，採用PE/PB/DCF/EBO/ESG/法人綜合模型。"

def v81_rating_weights(symbol):
    template, reason = v81_model_template(symbol)
    if template == "金融股模型":
        rows = [["企業品質",20,"金融品牌、通路與風控能力重要，但財務體質更核心"],["財務體質",35,"金融股以ROE、淨值品質、資本適足與資產品質為主要價值驅動"],["估值吸引力",20,"PB與股利殖利率決定安全邊際"],["法人籌碼",10,"法人資金會影響短中期估值重評"],["ESG",10,"金融業受治理、法遵與永續金融影響"],["產業前景",5,"產業成熟，前景權重較低"]]
    elif template == "成長型設備股模型":
        rows = [["企業品質",20,"技術與客戶認證重要，但設備股仍受景氣循環影響"],["財務體質",15,"財務穩健重要，但非最主要定價因子"],["估值吸引力",25,"設備股市場常用PE/PEG反映成長與訂單循環"],["法人籌碼",15,"法人與主力資金常提前反映訂單與景氣轉折"],["ESG",5,"ESG屬輔助因子，對設備股短期定價權重較低"],["產業前景",20,"設備需求高度取決於資本支出與產業景氣"]]
    elif template == "成熟科技龍頭/成長科技模型":
        rows = [["企業品質",30,"科技龍頭價值主要來自技術護城河、市占率與客戶黏著"],["財務體質",25,"現金流、ROE與資本配置能力直接支撐企業價值"],["估值吸引力",15,"成熟科技股估值重要，但不能高於基本面品質"],["法人籌碼",10,"法人覆蓋高，短期影響存在但資訊較透明"],["ESG",10,"國際資金與大型科技企業ESG權重提高"],["產業前景",10,"成熟龍頭已反映部分產業前景，故不過度放大"]]
    elif template == "循環產業模型":
        rows = [["企業品質",15,"循環股個別品質重要但常被景氣循環壓過"],["財務體質",20,"負債、現金流與景氣下行承受力很重要"],["估值吸引力",25,"PB/NAV與循環低點安全邊際是核心"],["法人籌碼",15,"景氣循環股容易受資金輪動影響"],["ESG",10,"傳產受碳排、治理與環保風險影響"],["產業前景",15,"供需循環決定獲利上修或下修"]]
    else:
        rows = [["企業品質",25,"一般企業價值首先取決於商業模式與競爭力"],["財務體質",20,"財務穩健度決定企業長期存續與估值基礎"],["估值吸引力",20,"合理價格與安全邊際是投資決策核心"],["法人籌碼",15,"資金流向影響市場重估速度"],["ESG",10,"ESG作為風險折價或溢價調整因子"],["產業前景",10,"產業前景影響成長假設與估值倍數"]]
    return template, reason, pd.DataFrame(rows, columns=["評級項目","權重%","權重來源/理由"])

def v81_industry_research(symbol):
    try:
        if "v80_industry_research" in globals():
            return v80_industry_research(symbol)
    except Exception:
        pass
    p = v81_profile(symbol)
    l2 = str(p.get("Level 2","待分類"))
    ...
    if any(k in l2 for k in ["面板","航運","鋼鐵","塑化"]): health -= 5
    health = max(0, min(100, health))
    return pd.DataFrame([["Level 1", p.get("Level 1","待分類"), "市場大類"],["Level 2", p.get("Level 2","待分類"), "主要產業"],["Level 3", p.get("Level 3","待分類"), "次產業"],["產業健康度", v81_fmt(health), "景氣、成長主題、產業循環代理"],["景氣燈號", "🟢 擴張" if health>=80 else "🟡 中立" if health>=55 else "🔴 低迷", "產業健康度轉換"],["主要驅動", p.get("Level 5","待分類"), "投資主題與需求來源"]], columns=["項目","內容","說明"])

def v81_score_components(symbol, q=None, scores=None):
    q = q or v81_quote(symbol)
    scores = scores or {}
    template, reason, wdf = v81_rating_weights(symbol)
    price = v81_num(q.get("price"))
    eps = v81_num(q.get("eps"))
    pe = v81_num(q.get("pe"))
    pb = v81_num(q.get("pb"))
    roe = v81_num(q.get("roe"))
    p = v81_profile(symbol)
    quality = 60
    if "AI" in str(p.get("Level 5","")): quality += 8
    if any(k in str(p.get("Level 2","")) for k in ["晶圓代工","IC設計","AI伺服器","電力重電","半導體"]): quality += 8
    if pd.notna(roe): quality += min(15, max(-10, roe*50))
    quality = max(0, min(100, quality))
    finance = 60 + (10 if pd.notna(eps) and eps > 0 else 0) + (min(20, max(-10, roe*60)) if pd.notna(roe) else 0) + (5 if pd.notna(pb) and pb < 3 else 0)
    finance = max(0, min(100, finance))
    valuation = 65
    if pd.notna(pe):
        valuation += 20 if pe < 12 else 10 if pe < 20 else -15 if pe > 40 else 0
    valuation = max(0, min(100, valuation))
    inst = v81_num(scores.get("inst", scores.get("main", 60)), 60)
    esg = 70
    try:
        if "v77_esg_score" in globals():
            esg = v81_num(v77_esg_score(symbol), 70)
    except Exception:
        pass
    try:
        industry = v81_num(v81_industry_research(symbol).loc[v81_industry_research(symbol)["項目"]=="產業健康度","內容"].iloc[0], 65)
    except Exception:
        industry = 65
    score_map = {"企業品質":quality, "財務體質":finance, "估值吸引力":valuation, "法人籌碼":inst, "ESG":esg, "產業前景":industry}
    rows, total = [], 0
    for _, r in wdf.iterrows():
        item = r["評級項目"]; weight = float(r["權重%"]); score = float(score_map.get(item, 60)); contribution = score*weight/100; total += contribution
        rows.append([item, f"{weight:.0f}%", v81_fmt(score), v81_fmt(contribution), r["權重來源/理由"]])
    grade = "SS級" if total >= 90 else "S級" if total >= 85 else "A級" if total >= 80 else "B級" if total >= 70 else "C級" if total >= 60 else "D級"
    return template, reason, v81_fmt(total), grade, pd.DataFrame(rows, columns=["項目","權重","得分","貢獻","權重來源/理由"])

def v81_model_value_weights(symbol):
    template, reason = v81_model_template(symbol)
    if template == "金融股模型":
        rows = [["PB",35,"金融股核心為淨值與ROE"],["EBO",25,"ROE超額報酬反映金融股價值"],["法人",15,"法人資金影響估值重評"],["ESG",10,"治理與法遵影響金融股折價/溢價"],["產業",15,"利率與金融循環重要"]]
    elif template == "成長型設備股模型":
        ...
        rows = [["PB",30,"循環產業常看資產價值與PB"],["PE",20,"景氣上行時PE具參考性"],["DCF",10,"循環波動大，DCF權重較低"],["法人",15,"資金輪動影響循環股"],["ESG",10,"傳產環保與治理風險"],["產業",15,"供需循環是主要驅動"]]
    else:
        rows = [["DCF",20,"一般企業長期價值基礎"],["PE",20,"市場常用倍數"],["PB",10,"資產價值輔助"],["EBO",15,"ROE與超額報酬"],["ESG",10,"風險與溢價調整"],["法人",15,"市場資金預期"],["產業",10,"產業景氣與成長性"]]
    return template, reason, pd.DataFrame(rows, columns=["模型","權重%","權重來源/理由"])

def v81_fair_value(symbol, q=None, scores=None):
    try:
        if "v80_fair_value" in globals():
            return v80_fair_value(symbol, q, scores)
    except Exception:
        pass
    q = q or v81_quote(symbol)
    price = v81_num(q.get("price")); eps = v81_num(q.get("eps")); pe = v81_num(q.get("pe"))
    if pd.isna(eps) and pd.notna(price) and pd.notna(pe) and pe > 0:
        eps = price / pe
    vals, rows = [], []
    if pd.notna(eps):
        for name, mult in [("PE保守價",15),("PE基準價",18),("PE樂觀價",22)]:
            val = eps*mult; vals.append(val); rows.append([name, v81_fmt(val), f"EPS({v81_fmt(eps)})×{mult}", "納入"])
    fair = np.nanmedian(vals) if vals else np.nan
    rows.append(["AI企業合理價", v81_fmt(fair), "納入模型中位數", "最終"])
    return pd.DataFrame(rows, columns=["模型","合理價","計算方式","狀態"])

def v81_esg_value(symbol, q=None):
    try:
        if "v80_esg_value" in globals():
            return v80_esg_value(symbol, q)
    except Exception:
        pass
    q = q or v81_quote(symbol)
    eps = v81_num(q.get("eps")); price = v81_num(q.get("price")); pe = v81_num(q.get("pe"))
    if pd.isna(eps) and pd.notna(price) and pd.notna(pe) and pe > 0:
        eps = price / pe
    base_pe, company_esg, industry_esg = 18, 70, 65
    diff = company_esg - industry_esg
    prem = max(0, min(0.2, diff*0.005))
    base = eps*base_pe if pd.notna(eps) else np.nan
    plus = base*prem if pd.notna(base) else np.nan
    fair = base+plus if pd.notna(base) else np.nan
    return pd.DataFrame([["公司ESG分數", v81_fmt(company_esg), "代理ESG分數；正式版可接永續報告/評級資料"],["產業ESG平均", v81_fmt(industry_esg), "同產業平均代理"],["ESG超額分數", v81_fmt(diff), "公司ESG - 產業平均"],["ESG溢價率", f"{prem*100:.1f}%", "ESG超額分數 × 0.5%，上限20%"],["ESG基礎估值", v81_fmt(base), f"EPS × 基礎PE = {v81_fmt(eps)} × {base_pe}"],["ESG溢價股價金額", v81_fmt(plus), "ESG基礎估值 × ESG溢價率"],["ESG合理價", v81_fmt(fair), "ESG基礎估值 + ESG溢價金額"]], columns=["項目","數值","計算說明"])

def v81_institutional_value(symbol, q=None, scores=None):
    try:
        if "v80_institutional_center" in globals():
            return v80_institutional_center(symbol, q, None, scores)
    except Exception:
        pass
    q = q or v81_quote(symbol); scores = scores or {}
    inst = v81_num(scores.get("inst", 60), 60); price = v81_num(q.get("price")); eps = v81_num(q.get("eps")); pe = v81_num(q.get("pe"), 18)
    factor = 1 + max(-0.18, min(0.22, (inst-60)/180))
    target = eps*pe*factor if pd.notna(eps) else (price*factor if pd.notna(price) else np.nan)
    return pd.DataFrame([["法人共識分數", v81_fmt(inst), "法人/籌碼代理分數"],["法人估值係數", v81_fmt(factor), "1 + (法人分數-60)/180，上下限修正"],["法人共識價", v81_fmt(target), "EPS×PE×法人係數；EPS缺值則現價×係數"]], columns=["項目","數值","計算說明"])

def v81_consensus_detail(symbol, q=None, scores=None):
    q = q or v81_quote(symbol)
    template, reason, weight_df = v81_model_value_weights(symbol)
    fv = v81_fair_value(symbol, q, scores)
    getv = lambda name: v81_num(fv[fv["模型"].astype(str).str.contains(name)]["合理價"].iloc[-1]) if not fv[fv["模型"].astype(str).str.contains(name)].empty else np.nan
    pe_val = getv("PE基準")
    dcf_val = getv("DCF")
    pb_val = getv("PB")
    ebo_val = getv("EBO")
    esgdf = v81_esg_value(symbol, q)
    esg_val = v81_num(esgdf[esgdf["項目"].astype(str).str.contains("ESG合理價")]["數值"].iloc[-1]) if not esgdf[esgdf["項目"].astype(str).str.contains("ESG合理價")].empty else np.nan
    instdf = v81_institutional_value(symbol, q, scores)
    inst_val = v81_num(instdf[instdf["項目"].astype(str).str.contains("法人共識價")]["數值"].iloc[-1]) if not instdf[instdf["項目"].astype(str).str.contains("法人共識價")].empty else np.nan
    base = np.nanmedian([x for x in [pe_val,pb_val,dcf_val,ebo_val,esg_val,inst_val] if pd.notna(x)]) if any(pd.notna(x) for x in [pe_val,pb_val,dcf_val,ebo_val,esg_val,inst_val]) else np.nan
    lookup = {"DCF":dcf_val,"PE":pe_val,"PB":pb_val,"EBO":ebo_val,"ESG":esg_val,"法人":inst_val,"產業":base}
    ...
    for _, r in weight_df.iterrows():
        ...
        if pd.notna(val):
            ...
            contrib = np.nan
        rows.append([model, v81_fmt(val), f"{w:.0f}%", v81_fmt(contrib), r["權重來源/理由"]])
    consensus = weighted/total_w if total_w else np.nan
    rows.append(["AI共識價", v81_fmt(consensus), "重新正規化後100%", "最終", f"模型模板：{template}；{reason}"])
    return pd.DataFrame(rows, columns=["模型","價格","權重","加權貢獻","權重來源/理由"])

def v81_decision(symbol, q=None, scores=None):
    q = q or v81_quote(symbol)
    price = v81_num(q.get("price"))
    cons = v81_consensus_detail(symbol, q, scores)
    fair = v81_num(cons[cons["模型"]=="AI共識價"]["價格"].iloc[0]) if not cons.empty else np.nan
    margin = (fair-price)/fair*100 if pd.notna(fair) and pd.notna(price) and fair>0 else np.nan
    ...
    template, reason, total, grade, score_df = v81_score_components(symbol, q, scores)
    quality = v81_num(total)
    q_state = "高" if quality>=80 else "中" if quality>=65 else "低"
    ...
    return pd.DataFrame([["目前股價", v81_fmt(price), "Yahoo/即時價格"],["AI共識價", v81_fmt(fair), "模型加權共識價"],["安全邊際", f"{margin:.1f}%" if pd.notna(margin) else "N/A", "(AI共識價 - 現價) / AI共識價"],["估值判定", valuation_state, "30%以上明顯低估；15~30低估；±15合理；-15~-30高估；-30以下明顯高估"],["AI總分", total, "企業品質、財務、估值、法人、ESG、產業加權"],["AI評級", grade, "90+ SS；85~89 S；80~84 A；70~79 B；60~69 C；60以下 D"],["品質區間", q_state, "80以上高；65~79中；65以下低"],["投資建議", advice, "依估值判定 × 品質區間決策矩陣"]], columns=["項目","結果","依據"])

def v81_methodology_center(symbol):
    template, reason, rating_w = v81_rating_weights(symbol)
    v_template, v_reason, value_w = v81_model_value_weights(symbol)
    st.markdown("### 📚 模型依據中心")
    st.info("V81原則：每個權重、評級、共識價與投資建議都必須可追溯、可驗證、可解釋。")
    st.markdown(f"#### 公司評級模板：{template}")
    st.write(reason)
    st.markdown("#### AI評級權重依據")
    st.dataframe(rating_w, use_container_width=True, hide_index=True)
    st.markdown(f"#### AI共識價模型權重依據：{v_template}")
    st.write(v_reason)
    st.dataframe(value_w, use_container_width=True, hide_index=True)
    st.markdown("#### 評級與安全邊際規則")
    st.dataframe(pd.DataFrame([["90以上","SS級","極高品質"],["85~89","S級","高品質"],["80~84","A級","良好"],["70~79","B級","中等"],["60~69","C級","偏弱"],["60以下","D級","弱"]], columns=["分數","評級","說明"]), use_container_width=True, hide_index=True)
...
def v81_institutional_home(active_symbol=None):
    st.markdown("""
    <style>
    .v81-hero{border-radius:30px;padding:38px 42px;background:radial-gradient(circle at 15% 25%, rgba(212,175,55,.28), transparent 28%),linear-gradient(135deg,#020617 0%,#0f172a 35%,#1e3a8a 70%,#0f766e 100%);color:white;box-shadow:0 18px 42px rgba(2,6,23,.34);border:1px solid rgba(212,175,55,.34);margin:18px 0 26px 0;}
    .v81-hero h1{font-size:46px;font-weight:900;margin:0 0 10px 0}.v81-hero h2{font-size:24px;font-weight:800;color:#f8e6a0;margin:0 0 14px 0}.v81-hero p{font-size:17px;opacity:.95;margin:6px 0}
    .v81-card{border-radius:20px;padding:20px;background:linear-gradient(180deg,#0f172a,#111827);color:white;border:1px solid rgba(148,163,184,.28);box-shadow:0 8px 18px rgba(15,23,42,.18)}
    .v81-card .big{font-size:28px;font-weight:900;color:#f8e6a0}
    </style>
    <div class="v81-hero"><h1>🏛 AI企業價值研究院</h1><h2>Enterprise Valuation Institute｜V82.1 Research Engine Route Fixed</h2><p>企業評價 × ESG × 法人資金 × 產業研究 × 全球競爭 × AI共識價</p><p><b>找出企業真正價值，而非追逐市場情緒。</b></p></div>
    """, unsafe_allow_html=True)
    cols = st.columns(4)
    radar = [("AI市場雷達","87/100","產業景氣與題材熱度"),("企業價值引擎","11模組","評價/ESG/法人已整合"),("透明化中心","啟用","權重、評級、公式可追溯"),("研究焦點", v81_stock_display(active_symbol or "2330.TW"), "目前分析標的")]
    for c, (title, val, note) in zip(cols, radar):
        c.markdown(f"<div class='v81-card'><div>{title}</div><div class='big'>{val}</div><div style='opacity:.75'>{note}</div></div>", unsafe_allow_html=True)
    st.markdown("### 🚀 快速研究入口")
    st.dataframe(pd.DataFrame([["① 公司DNA","公司是什麼、靠什麼賺錢、處於產業鏈哪一段"],["② 產業研究院","產業景氣、成長性、循環位置、主要風險"],["③ 全球競爭研究院","國內外競爭者、角色定位、同業比較池"],["④ 同業比較研究院","EPS、PE、PB、ROE、毛利率與同業比較"],["⑤ 企業評價研究院","PE、PB、DCF、EBO、EVA、NAV、CAP等估值模型"],["⑥ ESG研究院","ESG評級、產業排名、ESG Premium與合理價"],["⑦ 法人研究院","外資、投信、自營、融資融券、法人共識價"],["⑧ AI共識價研究院","估值、ESG、法人、產業模型整合"],["⑨ AI評級透明化中心","A級/低估/買進的分數來源與權重來源"],["⑩ 模型依據中心","每個權重為什麼這樣設定、公式與模板說明"],["⑪ 投資決策中心","安全邊際、Upside、風險與投資建議"]], columns=["研究模組","用途"]), use_container_width=True, hide_index=True)

def v81_enterprise_value_page(symbol, q=None, df=None, scores=None):
    q = q or v81_quote(symbol); scores = scores or {}
    st.markdown("<div style='padding:34px;border-radius:28px;background:linear-gradient(135deg,#020617,#1e3a8a,#0f766e);color:white;margin:12px 0 24px 0;border:1px solid rgba(212,175,55,.35);box-shadow:0 16px 36px rgba(2,6,23,.25);'><div style='font-size:40px;font-weight:900;'>🏛 AI企業價值研究院</div><div style='font-size:22px;font-weight:800;color:#f8e6a0;margin-top:6px;'>Enterprise Valuation Institute｜V82.1 Research Engine Route Fixed</div><div style='font-size:16px;margin-top:10px;'>企業價值 × 產業研究 × 全球競爭 × ESG × 法人資金 × AI共識價 × 投資決策</div><div style='font-size:18px;font-weight:800;margin-top:12px;'>找出企業真正價值，而非追逐市場情緒</div></div>", unsafe_allow_html=True)
    st.markdown(f"### 研究標的：{v81_stock_display(symbol)}")
    tabs = st.tabs(["①公司DNA","②產業研究院","③全球競爭研究院","④同業比較研究院","⑤企業評價研究院","⑥ESG研究院","⑦法人研究院","⑧AI共識價研究院","⑨AI評級透明化中心","⑩模型依據中心","⑪投資決策中心"])
    with tabs[0]:
        try: st.dataframe(v77_company_dna_df(symbol), use_container_width=True, hide_index=True)
        except Exception: st.dataframe(pd.DataFrame([v81_profile(symbol)]), use_container_width=True, hide_index=True)
        template, reason, total, grade, comp = v81_score_components(symbol, q, scores)
        st.metric("AI總分 / 評級", total, grade)
        st.caption(f"模型：{template}｜{reason}")
        st.dataframe(comp, use_container_width=True, hide_index=True)
    with tabs[1]: st.dataframe(v81_industry_research(symbol), use_container_width=True, hide_index=True)
    with tabs[2]:
        try: st.dataframe(v77_competitors(symbol), use_container_width=True, hide_index=True)
        except Exception: st.dataframe(pd.DataFrame([["同業資料待擴充","N/A","N/A","為避免錯誤，不用無關產業代替"]], columns=["公司","代碼","國家","競爭/關聯角色"]), use_container_width=True, hide_index=True)
    with tabs[3]:
        try: st.dataframe(v80_peer_compare(symbol), use_container_width=True, hide_index=True)
        except Exception: st.info("同業比較資料待擴充。")
    with tabs[4]:
        st.dataframe(v81_fair_value(symbol, q, scores), use_container_width=True, hide_index=True)
        _, _, wdf = v81_model_value_weights(symbol)
        st.markdown("#### 企業評價模型權重依據")
        st.dataframe(wdf, use_container_width=True, hide_index=True)
    with tabs[5]: st.dataframe(v81_esg_value(symbol, q), use_container_width=True, hide_index=True)
    with tabs[6]: st.dataframe(v81_institutional_value(symbol, q, scores), use_container_width=True, hide_index=True)
    with tabs[7]: st.dataframe(v81_consensus_detail(symbol, q, scores), use_container_width=True, hide_index=True)
    with tabs[8]:
        template, reason, total, grade, comp = v81_score_components(symbol, q, scores)
        st.metric("AI評級", grade, f"總分 {total}")
        st.caption(f"權重模板：{template}｜{reason}")
        st.dataframe(comp, use_container_width=True, hide_index=True)
    with tabs[9]: v81_methodology_center(symbol)
    with tabs[10]: st.dataframe(v81_decision(symbol, q, scores), use_container_width=True, hide_index=True)

def v80_enterprise_value_page(symbol, q=None, df=None, scores=None): return v81_enterprise_value_page(symbol, q, df, scores)
def v77_ai_page(symbol, q=None, df=None, scores=None): return v81_enterprise_value_page(symbol, q, df, scores)
def v76_ai_page(symbol, q=None, df=None, scores=None): return v81_enterprise_value_page(symbol, q, df, scores)
def v756_ai_page(symbol, q=None, df=None, scores=None): return v81_enterprise_value_page(symbol, q, df, scores)
# ================= V81 INSTITUTIONAL EDITION TRUE INTEGRATION LAYER END =================

# ================= V82 RESEARCH ENGINE TRUE PAGE LAYER =================
APP_BRAND = "AI企業價值研究院"
APP_VERSION="V82.1 Research Engine Route Fixed"

def v82_num(x, default=np.nan):
    try:
        if x is None or str(x).strip() in ["", "N/A", "None", "nan"]:
            return default
        return float(str(x).replace(",", "").replace("%", ""))
...
try:
    if not hasattr(st, "_v82_original_dataframe"):
        st._v82_original_dataframe = st.dataframe
        def _v82_dataframe(data=None, *args, **kwargs):
            try:
                if isinstance(data, pd.DataFrame):
                    data = v82_safe_df(data)
                return st._v82_original_dataframe(data, *args, **kwargs)
            except Exception as e:
                st.warning(f"表格已自動防錯：{e}")
                return st._v82_original_dataframe(v82_safe_df(data), *args, **kwargs)
        st.dataframe = _v82_dataframe
except Exception:
    pass

try:
    if not hasattr(st, "_v82_original_radio"):
        st._v82_original_radio = st.radio
        def _v82_radio(label, options, *args, **kwargs):
            try:
                if str(label).strip() == "主選單":
                    return st._v82_original_radio(label, ["🏠首頁","📊監控","📈K線","🏛企業價值研究院","⚙設定"], *args, **kwargs)
            except Exception:
                pass
            return st._v82_original_radio(label, options, *args, **kwargs)
        st.radio = _v82_radio
except Exception:
    pass

def v82_quote(symbol):
    for fn in ["v755_get_quote", "v77_get_quote", "v80_quote", "v81_quote", "get_quote"]:
        try:
            if fn in globals():
                q = globals()[fn](symbol)
                if isinstance(q, dict):
...
def v82_profile(symbol):
    for fn in ["v77_profile", "v763_profile", "v76_profile", "v756_profile", "v81_profile"]:
        try:
            if fn in globals():
                p = globals()[fn](symbol)
                if isinstance(p, dict):
            ...
            pass
    s = str(symbol).upper()
    return {"公司":s,"代碼":s,"市場":"待確認","Level 1":"待分類","Level 2":"待分類","Level 3":"待分類","Level 4":"待分類","Level 5":"待分類","產業鏈位置":"待確認","商業模式":"待確認","產業成熟度":"待確認"}

def v82_display(symbol):
    try:
        return display_name(symbol)
    except Exception:
        p = v82_profile(symbol)
        return f"{p.get('公司', symbol)} / {p.get('代碼', symbol)}"

def v82_kpi_cards(items):
    cols = st.columns(min(4, len(items)))
    for col, (title, value, note) in zip(cols, items):
        col.markdown(f"""
        <div style="border-radius:18px;padding:16px 18px;background:#0f172a;color:white;border:1px solid rgba(148,163,184,.28);">
          <div style="font-size:13px;opacity:.78;">{title}</div>
          <div style="font-size:26px;font-weight:900;color:#f8e6a0;">{value}</div>
          <div style="font-size:12px;opacity:.72;">{note}</div>
        </div>
        """, unsafe_allow_html=True)

def v82_home(active_symbol=None):
    st.markdown("""
    <style>
    .v82-hero{border-radius:32px;padding:42px 46px;background:radial-gradient(circle at 12% 20%, rgba(212,175,55,.30), transparent 25%),linear-gradient(135deg,#020617 0%,#0f172a 34%,#1e3a8a 68%,#0f766e 100%);color:white;box-shadow:0 20px 46px rgba(2,6,23,.35);border:1px solid rgba(212,175,55,.38);margin:16px 0 28px 0;}
    .v82-hero h1{font-size:48px;font-weight:950;margin:0 0 10px 0}
    .v82-hero h2{font-size:24px;font-weight:850;color:#f8e6a0;margin:0 0 14px 0}
    .v82-hero p{font-size:17px;opacity:.95;margin:6px 0}
    </style>
    <div class="v82-hero">
      <h1>🏛 AI企業價值研究院</h1>
      <h2>Enterprise Valuation Institute｜V82.1 Research Engine Route Fixed</h2>
      <p>企業價值 × 財報研究 × 產業研究 × 全球競爭 × ESG × 法人資金 × AI共識價</p>
      <p><b>找出企業真正價值，而非追逐市場情緒。</b></p>
    </div>
    """, unsafe_allow_html=True)
    v82_kpi_cards([
        ("研究引擎", "V82", "財報/評價/ESG/法人已併入研究院"),
        ("透明化中心", "啟用", "權重、公式、評級來源可追溯"),
        ("投資決策", "啟用", "安全邊際 × 品質分數"),
        ("目前標的", v82_display(active_symbol or "2330.TW"), "目前全站分析股票"),
    ])
    st.markdown("### 🚀 研究院模組")
    st.dataframe(pd.DataFrame([
        ["① 公司DNA", "公司定位、商業模式、產業鏈位置、護城河"],
        ["② 產業研究院", "產業健康度、五力分析、景氣燈號"],
        ["③ 全球競爭研究院", "國內外競爭者與同業池"],
        ["④ 同業比較研究院", "現價、EPS、PE、PB、角色比較"],
        ["⑤ 財報研究院", "EPS、PE、PB、ROE、現金流與財報品質"],
        ["⑥ 企業評價研究院", "PE/PB/DCF/EBO/EVA/NAV 等估值模型與權重"],
        ["⑦ ESG研究院", "ESG分數、ESG Premium、ESG合理價"],
        ["⑧ 法人研究院", "法人分數、法人估值係數、法人共識價"],
        ["⑨ AI共識價研究院", "模型價格、權重、加權貢獻、AI共識價"],
        ["⑩ AI評級透明化中心", "A級/低估/買進如何來"],
        ["⑪ 模型依據中心", "權重來源、評級規則、安全邊際規則"],
        ["⑫ 投資決策中心", "現價、共識價、安全邊際、投資建議"],
    ], columns=["模組", "說明"]), use_container_width=True, hide_index=True)

def v82_financial_research(symbol, q=None):
    q = q or v82_quote(symbol)
    rows = []
    for key, name in [("price","現價"),("eps","EPS"),("pe","PE"),("pb","PB"),("roe","ROE"),("revenue","營收"),("gross_margin","毛利率"),("operating_margin","營益率"),("free_cashflow","自由現金流")]:
        rows.append([name, v82_fmt(q.get(key)), "Yahoo Finance / 代理資料"])
    eps = v82_num(q.get("eps"))
    pe = v82_num(q.get("pe"))
    pb = v82_num(q.get("pb"))
    roe = v82_num(q.get("roe"))
    quality = 50
    if pd.notna(eps) and eps > 0: quality += 15
    if pd.notna(roe) and roe > 0.12: quality += 15
    if pd.notna(pe) and pe > 0: quality += 10
    if pd.notna(pb) and pb > 0: quality += 10
    quality = max(0, min(100, quality))
    rows.append(["財報品質分數", v82_fmt(quality), "EPS/ROE/PE/PB可得性與品質代理"])
    rows.append(["財報燈號", "🟢 綠燈" if quality>=75 else "🟡 黃燈" if quality>=55 else "🔴 紅燈", "依財報品質分數轉換"])
    return pd.DataFrame(rows, columns=["項目","數值","資料來源/說明"])

def v82_model_template(symbol):
    p = v82_profile(symbol)
    text = "".join([str(p.get(k,"")) for k in ["Level 1","Level 2","Level 3","Level 4","Level 5","商業模式"]])
    if "金融" in text:
        return "金融股模型", "金融股價值主要由淨值、ROE、資本品質、股利與風險控管驅動。"
...
def v82_rating_weights(symbol):
    template, reason = v82_model_template(symbol)
    if template == "金融股模型":
        rows = [["企業品質",20,"金融品牌、通路與風控能力重要，但財務體質更核心"],["財務體質",35,"金融股以ROE、淨值品質、資本適足與資產品質為主要價值驅動"],["估值吸引力",20,"PB與股利殖利率決定安全邊際"],["法人籌碼",10,"法人資金會影響短中期估值重評"],["ESG",10,"金融業受治理、法遵與永續金融影響"],["產業前景",5,"產業成熟，前景權重較低"]]
    elif template == "成長型設備股模型":
        rows = [["企業品質",20,"技術與客戶認證重要，但設備股仍受景氣循環影響"],["財務體質",15,"財務穩健重要，但非最主要定價因子"],["估值吸引力",25,"設備股市場常用PE/PEG反映成長與訂單循環"],["法人籌碼",15,"法人與主力資金常提前反映訂單與景氣轉折"],["ESG",5,"ESG屬輔助因子，對設備股短期定價權重較低"],["產業前景",20,"設備需求高度取決於資本支出與產業景氣"]]
...
def v82_industry_research(symbol):
    p = v82_profile(symbol)
    l2 = str(p.get("Level 2","待分類"))
    l5 = str(p.get("Level 5",""))
    health = 60
    ...
    base = pd.DataFrame([
        ...
        ["Level 2", p.get("Level 2","待分類"), "主要產業"],
        ["Level 3", p.get("Level 3","待分類"), "次產業"],
        ["產業健康度", v82_fmt(health), "景氣、成長主題、產業循環代理"],
        ["景氣燈號", "🟢 擴張" if health>=80 else "🟡 中立" if health>=55 else "🔴 低迷", "產業健康度轉換"],
        ["主要驅動", p.get("Level 5","待分類"), "投資主題與需求來源"],
    ], columns=["項目","內容","說明"])
    five = pd.DataFrame([
...
def v82_competitors(symbol):
    try:
        ...
        pass
    p = v82_profile(symbol)
    l2 = str(p.get("Level 2",""))
    if "晶圓" in l2 or "半導體" in l2:
        rows = [["台積電","2330.TW","台灣","本公司/同業"],["聯電","2303.TW","台灣","成熟製程"],["世界先進","5347.TWO","台灣","特殊製程"],["Samsung Foundry","005930.KS","韓國","全球競爭"],["GlobalFoundries","GFS","美國","全球競爭"]]
    else:
        rows = [["同業資料待擴充","N/A","N/A","為避免錯誤，不用無關產業代替"]]
    return pd.DataFrame(rows, columns=["公司","代碼","國家","競爭/關聯角色"])

def v82_peer_compare(symbol):
    comps = v82_competitors(symbol)
    rows = []
    for _, r in comps.iterrows():
        sym = r.get("代碼","")
        if sym == "N/A":
            ...
            continue
        q = v82_quote(sym)
        rows.append([r.get("公司",""), sym, r.get("競爭/關聯角色",""), v82_fmt(q.get("price")), v82_fmt(q.get("eps")), v82_fmt(q.get("pe")), v82_fmt(q.get("pb")), "Yahoo/內建代理"])
    return pd.DataFrame(rows, columns=["公司","代碼","角色","現價","EPS","PE","PB","資料來源"])

def v82_score_components(symbol, q=None, scores=None):
    q = q or v82_quote(symbol)
    scores = scores or {}
    template, reason, wdf = v82_rating_weights(symbol)
    eps = v82_num(q.get("eps"))
    pe = v82_num(q.get("pe"))
    pb = v82_num(q.get("pb"))
    roe = v82_num(q.get("roe"))
    ...
    if pd.notna(pe):
        valuation += 20 if pe < 12 else 10 if pe < 20 else -15 if pe > 40 else 0
    valuation = max(0, min(100, valuation))
    inst = v82_num(scores.get("inst", scores.get("main", 60)), 60)
    esg = 70
    try:
        if "v77_esg_score" in globals():
            esg = v82_num(v77_esg_score(symbol), 70)
    except Exception:
        pass
    try:
        ind_base, _ = v82_industry_research(symbol)
        industry = v82_num(ind_base.loc[ind_base["項目"]=="產業健康度","內容"].iloc[0], 65)
    except Exception:
        industry = 65
    score_map = {"企業品質":quality, "財務體質":finance, "估值吸引力":valuation, "法人籌碼":inst, "ESG":esg, "產業前景":industry}
    rows, total = [], 0
    for _, r in wdf.iterrows():
        item = r["評級項目"]; weight = float(r["權重%"]); score = float(score_map.get(item, 60)); contribution = score*weight/100; total += contribution
        rows.append([item, f"{weight:.0f}%", v82_fmt(score), v82_fmt(contribution), r["權重來源/理由"]])
    grade = "SS級" if total >= 90 else "S級" if total >= 85 else "A級" if total >= 80 else "B級" if total >= 70 else "C級" if total >= 60 else "D級"
    return template, reason, v82_fmt(total), grade, pd.DataFrame(rows, columns=["項目","權重","得分","貢獻","權重來源/理由"])

def v82_model_value_weights(symbol):
    template, reason = v82_model_template(symbol)
    if template == "金融股模型":
        rows = [["PB",35,"金融股核心為淨值與ROE"],["EBO",25,"ROE超額報酬反映金融股價值"],["法人",15,"法人資金影響估值重評"],["ESG",10,"治理與法遵影響金融股折價/溢價"],["產業",15,"利率與金融循環重要"]]
    elif template == "成長型設備股模型":
...
def v82_fair_value(symbol, q=None, scores=None):
    q = q or v82_quote(symbol)
    price = v82_num(q.get("price")); eps = v82_num(q.get("eps")); pe = v82_num(q.get("pe")); pb = v82_num(q.get("pb"))
    if pd.isna(eps) and pd.notna(price) and pd.notna(pe) and pe > 0:
        eps = price / pe
    rows, vals = [], []
    if pd.notna(eps):
        for name, mult in [("PE保守價",15),("PE基準價",18),("PE樂觀價",22)]:
            val = eps*mult; vals.append(val); rows.append([name, v82_fmt(val), f"EPS({v82_fmt(eps)})×{mult}", "納入"])
        rows.append(["EBO代理價", v82_fmt(eps*20), "EPS × 20，代理超額報酬模型", "納入"]); vals.append(eps*20)
        rows.append(["DCF代理價", v82_fmt(eps*19), "EPS × 19，代理長期現金流折現", "納入"]); vals.append(eps*19)
    if pd.notna(pb) and pd.notna(price) and pb > 0:
        bvps = price / pb
        rows.append(["PB代理價", v82_fmt(bvps*2.5), f"BVPS({v82_fmt(bvps)})×2.5", "納入"]); vals.append(bvps*2.5)
    fair = float(np.nanmedian(vals)) if vals else np.nan
    rows.append(["AI企業合理價", v82_fmt(fair), "納入模型中位數", "最終"])
    return pd.DataFrame(rows, columns=["模型","合理價","計算方式","狀態"])

def v82_esg_value(symbol, q=None):
    q = q or v82_quote(symbol)
    eps = v82_num(q.get("eps")); price = v82_num(q.get("price")); pe = v82_num(q.get("pe"))
    if pd.isna(eps) and pd.notna(price) and pd.notna(pe) and pe > 0:
        eps = price / pe
    company_esg = 70
    try:
        if "v77_esg_score" in globals():
            company_esg = v82_num(v77_esg_score(symbol), 70)
    except Exception:
        pass
    industry_esg = 65
    diff = company_esg - industry_esg
    ...
    fair = base+plus if pd.notna(base) else np.nan
    return pd.DataFrame([
        ["公司ESG分數", v82_fmt(company_esg), "代理ESG分數；正式版可接永續報告/評級資料"],
        ["產業ESG平均", v82_fmt(industry_esg), "同產業平均代理"],
        ["ESG超額分數", v82_fmt(diff), "公司ESG - 產業平均"],
        ["ESG溢價率", f"{prem*100:.1f}%", "ESG超額分數 × 0.5%，上限20%"],
        ["ESG基礎估值", v82_fmt(base), f"EPS × 基礎PE = {v82_fmt(eps)} × {base_pe}"],
        ["ESG溢價股價金額", v82_fmt(plus), "ESG基礎估值 × ESG溢價率"],
        ["ESG合理價", v82_fmt(fair), "ESG基礎估值 + ESG溢價金額"],
    ], columns=["項目","數值","計算說明"])

def v82_institutional_value(symbol, q=None, scores=None):
    q = q or v82_quote(symbol); scores = scores or {}
    inst = v82_num(scores.get("inst", scores.get("main", 60)), 60); price = v82_num(q.get("price")); eps = v82_num(q.get("eps")); pe = v82_num(q.get("pe"), 18)
    factor = 1 + max(-0.18, min(0.22, (inst-60)/180))
    target = eps*pe*factor if pd.notna(eps) else (price*factor if pd.notna(price) else np.nan)
    return pd.DataFrame([
        ["法人共識分數", v82_fmt(inst), "法人/籌碼代理分數"],
        ["法人估值係數", v82_fmt(factor), "1 + (法人分數-60)/180，上下限修正"],
        ["法人共識價", v82_fmt(target), "EPS×PE×法人係數；EPS缺值則現價×係數"],
        ["法人燈號", "🟢 偏多" if inst>=70 else "🟡 中立" if inst>=50 else "🔴 偏空", "依法人共識分數轉換"],
    ], columns=["項目","數值","計算說明"])

def v82_consensus_detail(symbol, q=None, scores=None):
    q = q or v82_quote(symbol)
    template, reason, weight_df = v82_model_value_weights(symbol)
    fv = v82_fair_value(symbol, q, scores)
    def getv(label):
        m = fv[fv["模型"].astype(str).str.contains(label)]
        return v82_num(m["合理價"].iloc[-1]) if not m.empty else np.nan
    pe_val, pb_val, dcf_val, ebo_val = getv("PE基準"), getv("PB"), getv("DCF"), getv("EBO")
    esgdf = v82_esg_value(symbol, q)
    esg_val = v82_num(esgdf[esgdf["項目"].astype(str).str.contains("ESG合理價")]["數值"].iloc[-1]) if not esgdf[esgdf["項目"].astype(str).str.contains("ESG合理價")].empty else np.nan
    instdf = v82_institutional_value(symbol, q, scores)
    inst_val = v82_num(instdf[instdf["項目"].astype(str).str.contains("法人共識價")]["數值"].iloc[-1]) if not instdf[instdf["項目"].astype(str).str.contains("法人共識價")].empty else np.nan
    base = np.nanmedian([x for x in [pe_val,pb_val,dcf_val,ebo_val,esg_val,inst_val] if pd.notna(x)]) if any(pd.notna(x) for x in [pe_val,pb_val,dcf_val,ebo_val,esg_val,inst_val]) else np.nan
    lookup = {"DCF":dcf_val,"PE":pe_val,"PB":pb_val,"EBO":ebo_val,"ESG":esg_val,"法人":inst_val,"產業":base}
    rows, total_w, weighted = [], 0, 0
    for _, r in weight_df.iterrows():
        ...
        if pd.notna(val):
            ...
            contrib = np.nan
        rows.append([model, v82_fmt(val), f"{w:.0f}%", v82_fmt(contrib), r["權重來源/理由"]])
    consensus = weighted/total_w if total_w else np.nan
    rows.append(["AI共識價", v82_fmt(consensus), "重新正規化後100%", "最終", f"模型模板：{template}；{reason}"])
    return pd.DataFrame(rows, columns=["模型","價格","權重","加權貢獻","權重來源/理由"])

def v82_decision(symbol, q=None, scores=None):
    q = q or v82_quote(symbol)
    price = v82_num(q.get("price"))
    cons = v82_consensus_detail(symbol, q, scores)
    fair = v82_num(cons[cons["模型"]=="AI共識價"]["價格"].iloc[0]) if not cons.empty else np.nan
    margin = (fair-price)/fair*100 if pd.notna(fair) and pd.notna(price) and fair>0 else np.nan
    valuation_state = "明顯低估" if pd.notna(margin) and margin>=30 else "低估" if pd.notna(margin) and margin>=15 else "合理" if pd.notna(margin) and margin>=-15 else "高估" if pd.notna(margin) and margin>=-30 else "明顯高估" if pd.notna(margin) else "資料不足"
    template, reason, total, grade, score_df = v82_score_components(symbol, q, scores)
    quality = v82_num(total)
    q_state = "高" if quality>=80 else "中" if quality>=65 else "低"
    if valuation_state in ["明顯低估","低估"] and q_state == "高":
        advice = "積極布局" if valuation_state == "明顯低估" else "分批布局"
    elif valuation_state in ["明顯低估","低估"] and q_state == "中":
        ...
        advice = "避開/賣出" if valuation_state == "明顯高估" else "資料不足"
    return pd.DataFrame([
        ["目前股價", v82_fmt(price), "Yahoo/即時價格"],
        ["AI共識價", v82_fmt(fair), "模型加權共識價"],
        ["安全邊際", f"{margin:.1f}%" if pd.notna(margin) else "N/A", "(AI共識價 - 現價) / AI共識價"],
        ["估值判定", valuation_state, "30%以上明顯低估；15~30低估；±15合理；-15~-30高估；-30以下明顯高估"],
        ["AI總分", total, "企業品質、財務、估值、法人、ESG、產業加權"],
        ["AI評級", grade, "90+ SS；85~89 S；80~84 A；70~79 B；60~69 C；60以下 D"],
        ["品質區間", q_state, "80以上高；65~79中；65以下低"],
        ["投資建議", advice, "依估值判定 × 品質區間決策矩陣"],
    ], columns=["項目","結果","依據"])

def v82_methodology_center(symbol):
    template, reason, rating_w = v82_rating_weights(symbol)
    v_template, v_reason, value_w = v82_model_value_weights(symbol)
    st.markdown("### 📚 模型依據中心")
    st.info("V82原則：每個權重、評級、共識價與投資建議都必須可追溯、可驗證、可解釋。")
    st.markdown(f"#### 公司評級模板：{template}")
    st.write(reason)
    st.markdown("#### AI評級權重依據")
    st.dataframe(rating_w, use_container_width=True, hide_index=True)
...
def v82_enterprise_page(symbol, q=None, df=None, scores=None):
    q = q or v82_quote(symbol); scores = scores or {}
    st.markdown("""
    <div style="padding:34px;border-radius:28px;background:linear-gradient(135deg,#020617,#1e3a8a,#0f766e);color:white;margin:12px 0 24px 0;border:1px solid rgba(212,175,55,.35);box-shadow:0 16px 36px rgba(2,6,23,.25);">
      ...
      <div style="font-size:22px;font-weight:800;color:#f8e6a0;margin-top:6px;">Enterprise Valuation Institute｜V82.1 Research Engine Route Fixed</div>
      <div style="font-size:16px;margin-top:10px;">企業價值 × 財報研究 × 產業研究 × 全球競爭 × ESG × 法人資金 × AI共識價</div>
      <div style="font-size:18px;font-weight:800;margin-top:12px;">找出企業真正價值，而非追逐市場情緒</div>
    </div>
    """, unsafe_allow_html=True)
    st.markdown(f"### 研究標的：{v82_display(symbol)}")
    tabs = st.tabs(["①公司DNA","②產業研究院","③全球競爭研究院","④同業比較研究院","⑤財報研究院","⑥企業評價研究院","⑦ESG研究院","⑧法人研究院","⑨AI共識價研究院","⑩AI評級透明化中心","⑪模型依據中心","⑫投資決策中心"])
    with tabs[0]:
        try: st.dataframe(v77_company_dna_df(symbol), use_container_width=True, hide_index=True)
        except Exception: st.dataframe(pd.DataFrame([v82_profile(symbol)]), use_container_width=True, hide_index=True)
        template, reason, total, grade, comp = v82_score_components(symbol, q, scores)
        st.metric("AI總分 / 評級", total, grade)
        st.caption(f"模型：{template}｜{reason}")
    with tabs[1]:
        base, five = v82_industry_research(symbol)
        st.dataframe(base, use_container_width=True, hide_index=True)
        st.markdown("#### 五力分析")
        st.dataframe(five, use_container_width=True, hide_index=True)
    with tabs[2]: st.dataframe(v82_competitors(symbol), use_container_width=True, hide_index=True)
    with tabs[3]: st.dataframe(v82_peer_compare(symbol), use_container_width=True, hide_index=True)
    with tabs[4]: st.dataframe(v82_financial_research(symbol, q), use_container_width=True, hide_index=True)
    with tabs[5]:
        st.dataframe(v82_fair_value(symbol, q, scores), use_container_width=True, hide_index=True)
        _, _, wdf = v82_model_value_weights(symbol)
        st.markdown("#### 企業評價模型權重依據")
        st.dataframe(wdf, use_container_width=True, hide_index=True)
    with tabs[6]: st.dataframe(v82_esg_value(symbol, q), use_container_width=True, hide_index=True)
    with tabs[7]: st.dataframe(v82_institutional_value(symbol, q, scores), use_container_width=True, hide_index=True)
    with tabs[8]: st.dataframe(v82_consensus_detail(symbol, q, scores), use_container_width=True, hide_index=True)
    with tabs[9]:
        template, reason, total, grade, comp = v82_score_components(symbol, q, scores)
        st.metric("AI評級", grade, f"總分 {total}")
        st.caption(f"權重模板：{template}｜{reason}")
        st.dataframe(comp, use_container_width=True, hide_index=True)
    with tabs[10]: v82_methodology_center(symbol)
    with tabs[11]: st.dataframe(v82_decision(symbol, q, scores), use_container_width=True, hide_index=True)

def v81_enterprise_value_page(symbol, q=None, df=None, scores=None): return v82_enterprise_page(symbol, q, df, scores)
def v80_enterprise_value_page(symbol, q=None, df=None, scores=None): return v82_enterprise_page(symbol, q, df, scores)
def v77_ai_page(symbol, q=None, df=None, scores=None): return v82_enterprise_page(symbol, q, df, scores)
# ================= V82 RESEARCH ENGINE TRUE PAGE LAYER END =================
...
# ================= V82.1 MANDATORY ENTERPRISE ROUTER =================
# 強制修正：點選「🏛企業價值研究院」時，不再進入舊 AI/計算透明化頁，而是直接進入 V82 研究院。
try:
    if str(page) in ["🏛企業價值研究院", "🤖AI", "💎評價", "🌱ESG永續", "🏦法人"]:
        q = v82_quote(active) if "v82_quote" in globals() else {}
        try:
            df_daily = df_daily
        except Exception:
        ...
        try:
            ...
            scores = {}
        v82_enterprise_page(active, q, df_daily, scores)
        st.stop()
except Exception as e:
    st.error(f"V82.1 研究院路由錯誤：{e}")
    st.stop()
# ================= V82.1 MANDATORY ENTERPRISE ROUTER END =================
...
if page=="🏠首頁":
    ...
    with st.expander("🧭 V76 類股快速入口", expanded=False):
        v76_sector_panel()
    # V82.1 Research Engine Route Fixed_PAGE_SECTOR_FIX
...
st.caption("AI研究院 Pro V82.1 Research Engine Route Fixed｜研究與教學用途，非投資建議。")
