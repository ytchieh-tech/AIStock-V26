...
APP_VERSION = "V245.0 Classic Home Restore + More Coverage"
...
APP_VERSION = "V245.0 Classic Home Restore + More Coverage"
DB_VERSION = "TW-STOCK-20260628-V245"
...
def settings_page():
    ...
    st.write('資料庫版本：', DB_VERSION)
    st.write('資料庫股票數：', len(STOCK_DB))
    st.write('本版修正：首頁恢復 V238/V239 經典版型，保留排行榜、底部產業選擇器與個股估值查詢。')
    try: v240_system_dashboard(v230_rows_df())
    except Exception: pass
    st.info('公開版優先提供現價、估值區間與產業位置；主要客戶若未正式揭露，會以公開供應鏈/產業合理推估並持續校正。')

# ===== V244.0 DASHBOARD HOME + SELECTOR RESTORE END =====
