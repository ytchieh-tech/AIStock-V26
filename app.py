...
APP_VERSION = "V244.0 Dashboard Home + Selector Restore"
...
APP_VERSION = "V244.0 Dashboard Home + Selector Restore"
DB_VERSION = "TW-STOCK-20260628-V244"
...
def home():
    ...
    if 'v227_active_symbol' not in st.session_state:
        st.session_state['v227_active_symbol']='2330.TW'
    v244_header()
    st.markdown("<div class='v244-search'>", unsafe_allow_html=True)
    q=st.text_input('搜尋公司名稱 / 代號 / 產業 / 主題標籤', placeholder='例如：2330、台積電、低軌衛星、CoWoS、華通', key='v244_search')
    st.markdown("</div>", unsafe_allow_html=True)
    if str(q or '').strip():
        st.session_state['v227_active_symbol']=v230_symbol(q)
    v244_nav_cards()
    ...
    selected_code=v243_home_selector(df)
    v243_query_panel(selected_code)
    st.markdown("<div class='v244-section'>資料庫完成度</div>", unsafe_allow_html=True)
    v240_system_dashboard(df)
    st.markdown("<div class='v244-section'>核心快速查詢</div>", unsafe_allow_html=True)
    quick=[('台積電','2330'),('華通','2313'),('昇達科','3491'),('廣達','2382'),('奇鋐','3017'),('聯鈞','3450')]
    cols=st.columns(len(quick))
    for col,(name,code_) in zip(cols,quick):
        with col:
            if st.button(name,key=f'v244_quick_{code_}',use_container_width=True):
                st.session_state['v227_active_symbol']=v230_symbol(code_); st.rerun()
    v244_leaderboards(df, rank)
    with st.expander('公開版說明與排行計算標準', expanded=False):
        st.markdown('**V244 修正重點：** 首頁改回投資分析平台儀表板風格，保留搜尋、主產業→子產業→個股估值選單、熱門主題、排行榜與資料庫完成度。')
        st.caption('估值區間為公開版初估，深度 DCF/EVA/EBO/CAP 將放入研究院/Pro 版。')
...
def settings_page():
    ...
    st.write('資料庫版本：', DB_VERSION)
    st.write('資料庫股票數：', len(STOCK_DB))
    st.write('本版修正：首頁改為儀表板版型，並保留主產業 → 子產業 → 個股選擇估值查詢。')
