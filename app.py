...
APP_VERSION="V89.3.4 Calibration Trial Edition"
...
/* V89.3.4 Calibration Trial Edition responsive audit */
@media(max-width:768px){
...
# V89.3.4 Calibration Trial Edition：擴充台股中文名稱對照；每位使用者使用自己的 session_state，不寫共用 watchlist.json
TW_STOCKS.update({
...
def maybe_reload(sec):
    # V89.3.4 Calibration Trial Edition.2: 使用 Streamlit autorefresh，避免 browser reload 導致回首頁或股票重設
    if sec and sec > 0:
...
def effective_price(q, df):
    """V89.3.4 Calibration Trial Edition: if Yahoo quote is N/A, use latest K-line close as backup so valuation models do not disappear."""
...
def financial_center(symbol,q,df):
    ...
    with tabs[0]:
        st.dataframe(v8934_format_financial_df(summary), use_container_width=True, hide_index=True)
    ...
    with tabs[1]:
        ...
        if income_zh.empty:
            ...
            st.dataframe(v8934_format_financial_df(income_zh), use_container_width=True, hide_index=True)
    ...
    with tabs[2]:
        ...
        if balance_zh.empty:
            ...
            st.dataframe(v8934_format_financial_df(balance_zh), use_container_width=True, hide_index=True)
    ...
    with tabs[3]:
        ...
        if cashflow_zh.empty:
            ...
            st.dataframe(v8934_format_financial_df(cashflow_zh), use_container_width=True, hide_index=True)
...
<div class="hero">
  <div style="display:flex;align-items:center;gap:10px;">
    ...
    <div>
      ...
      <div style="font-size:.78rem;color:#dbeafe;margin-top:2px;">
        V89.3.4 Calibration Trial Edition｜企業評價 × 法人籌碼 × 融資融券燈號 × ESG永續 × 中文財報 × AI研究
      </div>
    </div>
  </div>
  <svg viewBox="0 0 1200 180" width="100%" height="140" style="margin-top:10px;border-radius:14px;border:1px solid rgba(148,163,184,.35);background:linear-gradient(135deg,#0f172a,#1e3a8a);">
    ...
    <text x="40" y="42" fill="#ffffff" font-size="28" font-weight="900">V89.3.4 Calibration Trial Edition</text>
...
with st.sidebar:
    ...
    # V89.3.4 Calibration Trial Edition_SIDEBAR_SECTOR_FIX
...
def v76_ai_page(symbol,q,df,scores):
    st.markdown('## 🏛 V89.3.4 Calibration Trial Edition.3')
...
def v762_banner():
    ...
    <div style="padding:28px;border-radius:22px;background:linear-gradient(135deg,#0f172a,#1d4ed8,#047857);color:white;margin:12px 0 22px 0;">
      ...
      <div style="font-size:20px;font-weight:800;margin-top:8px;">V89.3.4 Calibration Trial Edition</div>
...
def v87_research_institute(active, q, df_daily, scores):
    ...
    <div style="padding:34px;border-radius:28px;background:linear-gradient(135deg,#020617,#1e3a8a,#0f766e);color:white;margin:12px 0 24px 0;border:1px solid rgba(212,175,55,.35);box-shadow:0 16px 36px rgba(2,6,23,.25);">
      ...
      <div style="font-size:22px;font-weight:800;color:#f8e6a0;margin-top:6px;">Enterprise Valuation Institute｜V89.3.4 Calibration Trial Edition</div>
...
def v8934_format_financial_df(df):
...
try:
    ...
    def chinese_financial_analysis(symbol, q, ft):
        ...
        return v8934_format_financial_df(summary), ratios, score
except Exception:
    pass
...
try:
    ...
    def zh_financial_df(df):
        return v8934_format_financial_df(_v8932_old_zh_financial_df(df))
except Exception:
    pass
...
try:
    ...
    def zh_financial_df(df):
        return v8934_format_financial_df(_v8933_old_zh_financial_df(df))
except Exception:
    pass
...
v8934_format_financial_df = v8934_format_financial_df
v8934_format_financial_df = v8934_format_financial_df
...
try:
    ...
    def zh_financial_df(df):
        return v8934_format_financial_df(_v8934_old_zh_financial_df(df))
except Exception:
    pass
...
def v8934_calibrated_range(symbol, val, price=None):
    ...
    multipliers = {
        ...
        "6770": (0.80, 0.92, 1.10, 64),  # 力積電：景氣循環，信心度較低
    }
...
if page=="🏠首頁":
    ...
    with st.expander("🧭 V76 類股快速入口", expanded=False):
        v76_sector_panel()
    # V89.3.4 Calibration Trial Edition_PAGE_SECTOR_FIX
...
st.caption("AI研究院 Pro V89.3.4 Calibration Trial Edition｜研究與教學用途，非投資建議。")
