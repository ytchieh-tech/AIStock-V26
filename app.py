...
APP_VERSION = "V253.0 Formal Valuation Guard + Ranking Fix"
DB_VERSION = "TW-STOCK-20260628-V251"
...
# ===== V244.0 DASHBOARD HOME + SELECTOR RESTORE START =====
APP_VERSION = "V253.0 Formal Valuation Guard + Ranking Fix"
DB_VERSION = "TW-STOCK-20260628-V251"
...
def v244_css():
    ...
    st.markdown("""
    <style>
    .v244-hero{display:flex;align-items:center;justify-content:space-between;gap:18px;background:#ffffff;border:1px solid #e6edf7;border-radius:22px;padding:18px 24px;margin:6px 0 20px;box-shadow:0 12px 34px rgba(15,23,42,.06)}
    .v244-brand{display:flex;gap:12px;align-items:center}.v244-logo{font-size:34px}.v244-title{font-size:28px;font-weight:900;color:#0f172a;line-height:1.1}.v244-sub{font-size:13px;color:#64748b;margin-top:4px}.v244-ver{text-align:right;color:#2563eb;font-weight:800;font-size:13px}
    .v244-search input{border-radius:16px!important;min-height:48px!important;font-size:16px!important}
    .v244-nav{display:grid;grid-template-columns:repeat(6,1fr);gap:14px;margin:8px 0 20px}.v244-nav-card{background:#fff;border:1px solid #e6edf7;border-radius:16px;padding:14px;text-align:center;box-shadow:0 8px 22px rgba(15,23,42,.04);font-weight:800;color:#172554}.v244-nav-card .ico{font-size:25px;display:block;margin-bottom:5px}
    .v244-kpi-grid{display:grid;grid-template-columns:repeat(6,1fr);gap:14px;margin:12px 0 20px}.v244-kpi{background:#fff;border:1px solid #e6edf7;border-radius:18px;padding:16px 18px;min-height:118px;box-shadow:0 10px 24px rgba(15,23,42,.05)}.v244-kpi .ico{font-size:30px;float:left;margin-right:12px}.v244-kpi .lab{font-size:13px;color:#475569;font-weight:800}.v244-kpi .val{font-size:28px;color:#0f172a;font-weight:900;margin-top:8px}.v244-kpi .note{font-size:12px;color:#64748b;background:transparent;border:0;padding:0;margin:3px 0 0 0}
    .v244-pill-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.v244-pill{border:1px solid #dbeafe;background:#f8fbff;border-radius:13px;padding:12px 14px;font-weight:850;text-align:center;color:#1e3a8a}.v244-pill:nth-child(2n){background:#faf5ff;color:#6b21a8;border-color:#e9d5ff}.v244-pill:nth-child(3n){background:#fff7ed;color:#c2410c;border-color:#fed7aa}.v244-pill:nth-child(4n){background:#ecfdf5;color:#166534;border-color:#bbf7d0}
    .v244-section{font-size:26px;font-weight:950;color:#0f172a;margin:22px 0 12px}.v244-card{background:#fff;border:1px solid #e6edf7;border-radius:18px;padding:16px;margin:8px 0 16px;box-shadow:0 10px 24px rgba(15,23,42,.04)}
    section[data-testid="stSidebar"] *{color:#0f172a!important} section[data-testid="stSidebar"]{background:#f3f6fb!important}
    @media(max-width:1000px){.v244-kpi-grid{grid-template-columns:repeat(2,1fr)}.v244-nav{grid-template-columns:repeat(3,1fr)}.v244-hero{display:block}.v244-ver{text-align:left;margin-top:8px}.v244-pill-grid{grid-template-columns:repeat(2,1fr)}}
    @media(max-width:640px){.v244-kpi-grid,.v244-nav,.v244-pill-grid{grid-template-columns:1fr}.v244-title{font-size:24px}}
    </style>
    """, unsafe_allow_html=True)
...
def v244_header():
    ...
    st.markdown(f"""
    <div class='v244-hero'>
      <div class='v244-brand'><div class='v244-logo'>📈</div><div><div class='v244-title'>智策股市 AI 決策平台</div><div class='v244-sub'>企業價值研究平台｜產業鏈 × 全球競爭力 × 財務預測 × 合理價推估</div></div></div>
      <div class='v244-ver'>{APP_VERSION}<br>{now_show}</div>
    </div>
    """, unsafe_allow_html=True)

def v244_nav_cards():
    items=[('🏭','產業分析'),('🌐','全球競爭力'),('🏛️','企業評價'),('🔮','財務預測'),('♻️','ESG'),('⚖️','法人評等')]
    html="<div class='v244-nav'>" + ''.join([f"<div class='v244-nav-card'><span class='ico'>{i}</span>{t}</div>" for i,t in items]) + "</div>"
    st.markdown(html, unsafe_allow_html=True)

def v244_kpis(df, rank):
    total=len(df)
    hot_ind=df['產業'].nunique() if '產業' in df.columns else 0
    hot_theme=len(set('、'.join(df.get('主題標籤',pd.Series(dtype=str)).fillna('').astype(str)).split('、')))-1 if '主題標籤' in df.columns else 0
    valid_price=int(rank['現價'].notna().sum()) if not rank.empty and '現價' in rank.columns else 0
    ai9=int((pd.to_numeric(df.get('AI受惠度',pd.Series(dtype=float)), errors='coerce').fillna(0)>=9).sum()) if 'AI受惠度' in df.columns else 0
    global5=int(df.get('全球競爭力',pd.Series(dtype=str)).astype(str).str.contains('★★★★★', regex=False).sum()) if '全球競爭力' in df.columns else 0
    cards=[('🔥','熱門產業',hot_ind,'涵蓋主要產業'),('🏆','個股資料庫',total,'持續補齊中'),('🏷️','主題標籤',hot_theme,'可多重歸屬'),('✅','有效現價',valid_price,'Yahoo Finance'),('🤖','AI高受惠',ai9,'AI受惠度 ≥ 9'),('🌏','全球強勢',global5,'★★★★★')]
    html="<div class='v244-kpi-grid'>"+''.join([f"<div class='v244-kpi'><span class='ico'>{ico}</span><div class='lab'>{lab}</div><div class='val'>{val}</div><div class='note'>{note}</div></div>" for ico,lab,val,note in cards])+"</div>"
    st.markdown(html, unsafe_allow_html=True)

def v244_theme_pills(df):
    st.markdown("<div class='v244-section'>熱門主題標籤</div>", unsafe_allow_html=True)
    themes=[]
    if '主題標籤' in df.columns:
        for s in df['主題標籤'].fillna('').astype(str):
            for p in s.replace(',', '、').split('、'):
                p=p.strip()
                if p and p not in themes: themes.append(p)
    themes=(themes[:12] or ['AI伺服器','CoWoS','先進封裝','HPC','光通訊','CPO','低軌衛星','機器人'])
    st.markdown("<div class='v244-pill-grid'>"+''.join([f"<div class='v244-pill'>{t}</div>" for t in themes])+"</div>", unsafe_allow_html=True)

def v244_leaderboards(df, rank):
    left,right=st.columns([1.05,1])
    with left:
        st.markdown("<div class='v244-section'>熱門個股 TOP 10</div>", unsafe_allow_html=True)
        show=df.copy()
        if '全球競爭力' in show.columns: show['_stars']=show['全球競爭力'].astype(str).str.count('★')
        else: show['_stars']=0
        if 'AI受惠度' in show.columns: show['_ai']=pd.to_numeric(show['AI受惠度'],errors='coerce').fillna(0)
        else: show['_ai']=0
        show=show.sort_values(['_ai','_stars'],ascending=False).head(10)
        cols=[c for c in ['公司','代碼','產業','子產業','AI受惠度','全球競爭力'] if c in show.columns]
        st.dataframe(show[cols],use_container_width=True,hide_index=True)
    with right:
        v244_theme_pills(df)
        st.markdown("<div class='v244-section'>低估排行 TOP 5</div>", unsafe_allow_html=True)
        if not rank.empty and {'現價','綜合合理價','預期報酬%'}.issubset(rank.columns):
            low=rank.dropna(subset=['現價','綜合合理價','預期報酬%'])
            low=low[low['現價']>0].sort_values('預期報酬%',ascending=False).head(5)
            cols=[c for c in ['公司','代碼','現價','綜合合理價','預期報酬%'] if c in low.columns]
            st.dataframe(v231_fmt_rank(low[cols]),use_container_width=True,hide_index=True)
        st.markdown("<div class='v244-section'>AI 受惠度排行 TOP 5</div>", unsafe_allow_html=True)
        ai=df.copy(); ai['_ai']=pd.to_numeric(ai.get('AI受惠度',0),errors='coerce').fillna(0)
        ai=ai.sort_values('_ai',ascending=False).head(5)
        cols=[c for c in ['公司','代碼','AI受惠度','主題標籤'] if c in ai.columns]
        st.dataframe(ai[cols],use_container_width=True,hide_index=True)

def home():
    """V246：保留 V238/V239 以前的首頁節奏，並修正查詢結果標題重複。
    版面順序：上方標題/搜尋 → KPI → 核心快速查詢 → 排行 Tabs → 產業/子產業/個股選單 → 個股估值區間。
    """
    v230_css()
    try:
        v244_css()
    ...
    q = st.text_input('搜尋公司名稱 / 代號 / 產業 / 主題標籤', placeholder='例如：2330、台積電、2313、華通、低軌衛星、CoWoS', key='v245_search')
    if str(q or '').strip():
        st.session_state['v227_active_symbol'] = v230_symbol(q)
    ...
    hot_ind = df['產業'].nunique() if '產業' in df.columns else 0
    if '主題標籤' in df.columns:
        hot_theme = len({p.strip() for s0 in df['主題標籤'].fillna('').astype(str) for p in s0.replace(',', '、').split('、') if p.strip()})
    else:
        hot_theme = 0
    ai9 = int((pd.to_numeric(df.get('AI受惠度', pd.Series(dtype=float)), errors='coerce').fillna(0) >= 9).sum()) if 'AI受惠度' in df.columns else 0
    valid_price = int(rank['現價'].notna().sum()) if not rank.empty and '現價' in rank.columns else 0
    ...
    with st.expander('📊 資料庫完成度', expanded=False):
        try:
            v240_system_dashboard(df)
        except Exception:
            st.caption(f'目前個股資料庫：{total} 檔')
    ...
    for col, (name, code_) in zip(cols, quick):
        with col:
            if st.button(name, key=f'v245_quick_{code_}', use_container_width=True):
                st.session_state['v227_active_symbol'] = v230_symbol(code_)
                st.rerun()
    ...
    with st.expander('排行計算標準', expanded=False):
        ...
        **低估排行 TOP 10**：`預期報酬 = (綜合合理價 - 現價) ÷ 現價`。  
        現價優先抓 Yahoo Finance 最近價格；抓不到現價者不列入低估排行，避免錯誤價格造成排行失真。  
        目前公開版估值區間以現價、產業乘數與資料庫初估建立；深度 DCF / EVA / EBO / CAP 將放入研究院 / Pro 版。
        """)

    tab1, tab2, tab3, tab4 = st.tabs(['熱門個股', '低估排行', 'AI受惠排行', '全球競爭力排行'])
    with tab1:
        show = df.copy()
        show['_stars'] = show['全球競爭力'].astype(str).str.count('★') if '全球競爭力' in show.columns else 0
        show['_ai'] = pd.to_numeric(show['AI受惠度'], errors='coerce').fillna(0) if 'AI受惠度' in show.columns else 0
        show = show.sort_values(['_ai','_stars'], ascending=False).head(10)
        cols = [c for c in ['公司','代碼','產業','子產業','AI受惠度','全球競爭力','產業地位'] if c in show.columns]
        st.dataframe(show[cols], use_container_width=True, hide_index=True)
    with tab2:
        if not rank.empty and {'現價','綜合合理價','預期報酬%'}.issubset(rank.columns):
            low = rank.dropna(subset=['現價','綜合合理價','預期報酬%'])
            low = low[low['現價'] > 0].sort_values('預期報酬%', ascending=False).head(10)
            cols = [c for c in ['公司','代碼','產業','子產業','現價','綜合合理價','預期報酬%','AI受惠度'] if c in low.columns]
            st.dataframe(v231_fmt_rank(low[cols]), use_container_width=True, hide_index=True)
            st.caption('若現價為 N/A，代表 Yahoo Finance 暫時抓不到，系統會自動排除低估排行。')
        else:
            st.info('目前尚無足夠現價資料可建立低估排行。')
    with tab3:
        ai = df.copy()
        ai['_stars'] = ai['全球競爭力'].astype(str).str.count('★') if '全球競爭力' in ai.columns else 0
        ai['_ai'] = pd.to_numeric(ai['AI受惠度'], errors='coerce').fillna(0) if 'AI受惠度' in ai.columns else 0
        ai = ai.sort_values(['_ai','_stars'], ascending=False).head(10)
        cols = [c for c in ['公司','代碼','產業','子產業','AI受惠度','全球競爭力','主題標籤'] if c in ai.columns]
        st.dataframe(ai[cols], use_container_width=True, hide_index=True)
    with tab4:
        gl = df.copy()
        gl['_stars'] = gl['全球競爭力'].astype(str).str.count('★') if '全球競爭力' in gl.columns else 0
        gl['_ai'] = pd.to_numeric(gl['AI受惠度'], errors='coerce').fillna(0) if 'AI受惠度' in gl.columns else 0
        gl = gl.sort_values(['_stars','_ai'], ascending=False).head(10)
        cols = [c for c in ['公司','代碼','產業','子產業','全球競爭力','全球排名','產業地位'] if c in gl.columns]
        st.dataframe(gl[cols], use_container_width=True, hide_index=True)

    with st.expander('產業 → 子產業 → 個股（選到即查詢估值）', expanded=False):
        c1, c2, c3 = st.columns(3)
        with c1:
            industries = sorted(df['產業'].dropna().astype(str).unique()) if '產業' in df.columns else []
            ind = st.selectbox('主產業', industries, key='v245_home_ind') if industries else None
        dff = df[df['產業'].astype(str) == str(ind)] if ind and '產業' in df.columns else df.copy()
        with c2:
            subs = sorted(dff['子產業'].dropna().astype(str).unique()) if '子產業' in dff.columns else []
            sub = st.selectbox('子產業', subs, key='v245_home_sub') if subs else None
        dff = dff[dff['子產業'].astype(str) == str(sub)] if sub and '子產業' in dff.columns else dff
        labels = {f"{r['公司']} / {r['代碼']}": r['代碼'] for _, r in dff.iterrows() if '公司' in r and '代碼' in r}
        with c3:
            picked = st.selectbox('個股', list(labels.keys()), key='v245_home_stock') if labels else None
        if picked:
            code = labels[picked]
            if st.session_state.get('v245_last_pick') != picked:
                st.session_state['v245_last_pick'] = picked
                st.session_state['v227_active_symbol'] = code
                st.rerun()

    st.markdown('---')
    # V246：v243_query_panel 內部已包含標題，這裡不要再印一次，避免首頁出現兩個『查詢結果 / 個股估值區間』。
    try:
        v243_query_panel(st.session_state.get('v227_active_symbol', '2330.TW'))
    except Exception:
        v230_price_block(st.session_state.get('v227_active_symbol', '2330.TW'))
...
def settings_page():
    ...
    st.write('資料庫版本：', DB_VERSION)
    st.write('資料庫股票數：', len(STOCK_DB))
    st.write('本版修正：首頁保留 V238/V239 經典版型與新版資料庫，並修正查詢結果標題重複顯示。')
    try: v240_system_dashboard(v230_rows_df())
    except Exception: pass
    st.info('公開版優先提供現價、估值區間與產業位置；主要客戶若未正式揭露，會以公開供應鏈/產業合理推估並持續校正。')

# ===== V246.0 CLASSIC HOME QUERY TITLE FIX END =====


# ===== V247.0 MORE COVERAGE + VALUATION ROW RESTORE START =====
APP_VERSION = "V253.0 Formal Valuation Guard + Ranking Fix"
DB_VERSION = "TW-STOCK-20260628-V251"

# V247：續補一般投資人常搜尋的其他類股，重點是讓首頁可查詢股價區間與基本產業位置。
# 未正式揭露的主要客戶，以「產業鏈/通路/終端客群」描述，後續再逐步校正成公司名單。
V247_EXTRA_STOCKS = {
    "2892.TW": _v238_stock("第一金","金融","公股銀行金控","公股金控","★★★☆☆","公股銀行型金控，企金與個金通路穩定","兆豐金、合庫金、華南金、彰銀","中：公股資源、分行通路與企金客戶","利差、信用風險、景氣循環",1.02,"金融、銀行、穩健配息","下游金融服務","公股銀行金控","企業授信客戶、個人金融客戶、政府/公股相關客戶"),
    "2889.TW": _v238_stock("國票金","金融","票券/證券/銀行投資","票券型金控","★★★☆☆","票券、證券與金融投資服務金控","元大金、永豐金、群益證、金融同業","中：票券業務與金融通路","利率變化、投資收益波動、金融市場景氣",1.01,"金融、票券、證券","下游金融服務","票券型金控","企業短期融資客戶、投資人、證券與金融服務客戶"),
    "2328.TW": _v238_stock("廣宇","電子零組件","連接線/電子製造","電子連接與製造供應商","★★★☆☆","連接線、電子零組件與系統組裝相關供應商","鴻海集團、貿聯-KY、良維、連接器同業","中：集團資源、電子製造與客戶基礎","消費電子循環、毛利率、客戶集中",1.03,"電子零組件、連接線、鴻海集團","中游零組件/組裝","電子連接與製造供應商","品牌電子、通訊、電腦與工業電子客戶"),
    "3005.TW": _v238_stock("神基","強固電腦/車用","強固型電腦/航太車用機構","強固型電腦與機構件供應商","★★★★☆","強固型電腦、車用機構件與航太相關製造供應商","研華、Panasonic Toughbook、車用機構同業","中高：強固電腦技術、特殊應用認證與客戶基礎","專案波動、匯率、車用景氣",1.05,"強固電腦、車用、航太、工業電腦","中下游系統/機構件","強固型電腦主要供應商","軍警、工業、物流、公共安全、車用與航太客戶"),
    "4938.TW": _v238_stock("和碩","電子代工/ODM","消費電子ODM/組裝","全球電子代工主要廠","★★★★☆","手機、PC、消費電子與伺服器相關代工供應商","鴻海、廣達、緯創、仁寶","中高：製造規模、客戶關係與全球產能","毛利率、客戶集中、消費電子循環",1.03,"電子代工、AI PC、伺服器、蘋果供應鏈","中下游ODM/EMS","全球電子代工主要廠","Apple等品牌客戶、PC/消費電子/伺服器客戶","AI PC與伺服器間接受惠"),
    "1907.TW": _v238_stock("永豐餘","造紙/包材","工紙/紙器/消費品","台灣造紙與包材主要廠","★★★☆☆","工業用紙、紙器、消費紙品與包材供應商","正隆、華紙、國際造紙廠","中：紙器通路、包材客戶與品牌資源","紙價、原料成本、景氣循環",1.00,"造紙、包材、民生消費","上中游紙品/包材","造紙與包材主要廠","電商、食品、製造業包材客戶與消費紙品通路"),
    "1802.TW": _v238_stock("台玻","玻璃/建材","平板玻璃/玻纖","玻璃與玻纖材料供應商","★★★☆☆","平板玻璃、玻璃纖維與建材相關材料供應商","和成、冠軍、國際玻璃廠","中：玻璃產能、產品線與材料基礎","房市景氣、能源成本、價格競爭",1.00,"建材、玻璃、材料","上中游建材材料","玻璃材料主要供應商","建築、汽車、電子材料與工業客戶"),
    "1434.TW": _v238_stock("福懋","紡織/材料","工業用布/輪胎簾布","工業用紡織材料供應商","★★★☆☆","輪胎簾布、工業用布、成衣布料與材料供應商","南紡、遠東新、工業紡織同業","中：工業用布技術與集團資源","輪胎/紡織需求、原料成本、匯率",1.01,"紡織、工業材料、輪胎供應鏈","上中游紡織材料","工業用紡織材料供應商","輪胎廠、成衣品牌供應鏈與工業材料客戶"),
    "2206.TW": _v238_stock("三陽工業","汽車/機車","機車/汽車代理製造","台灣機車與汽車相關廠","★★★☆☆","機車製造、汽車代理與相關通路服務供應商","光陽、Gogoro、中華車、和泰車","中：品牌通路、機車製造與售後服務","車市需求、政策、電動化競爭",1.03,"機車、汽車、通路、電動化","下游品牌/製造通路","台灣機車主要品牌","機車消費者、汽車代理客戶、通路與售後服務客戶"),
    "4105.TW": _v238_stock("東洋","生技醫療","學名藥/特殊製劑","台灣特殊學名藥主要廠","★★★☆☆","特殊學名藥、針劑與醫藥通路供應商","中化、生達、國際學名藥廠","中：藥證、製劑技術與醫院通路","藥價政策、藥證時程、產品競爭",1.04,"生技、學名藥、醫療通路","醫藥製造/通路","特殊學名藥供應商","醫院、診所、藥局、醫藥通路與海外授權客戶"),
    "4743.TW": _v238_stock("合一","生技醫療","新藥/傷口照護","新藥與醫材題材公司","★★★☆☆","新藥、傷口照護與醫療產品商業化公司","中天、生技新藥同業、國際醫材藥廠","中：研發管線、藥證與商業化題材","臨床/法規、銷售成長、股價波動",1.08,"生技、新藥、醫材","醫療產品/新藥","新藥商業化公司","醫院、醫師、患者、醫材與藥品通路客戶"),
    "1795.TWO": _v238_stock("美時","生技醫療","學名藥/特殊製劑","特殊學名藥國際供應商","★★★★☆","特殊學名藥、口服製劑與國際市場布局藥廠","東洋、保瑞、國際學名藥廠","中高：藥證、國際通路、產品組合","藥價、競爭、海外市場法規",1.06,"生技、學名藥、國際藥證","醫藥製造/國際通路","特殊學名藥供應商","國際藥廠、通路商、醫院與藥局客戶"),
    "4736.TW": _v238_stock("泰博","醫材/檢測","血糖檢測/醫療耗材","醫療檢測耗材供應商","★★★☆☆","血糖檢測、醫療耗材與居家檢測產品供應商","五鼎、國際醫材檢測廠","中：檢測產品、通路與製造能力","檢測需求波動、價格競爭、法規",1.04,"醫材、檢測、居家醫療","醫療耗材/檢測","醫療檢測耗材供應商","醫療通路、藥局、醫院、品牌檢測客戶"),
    "6550.TW": _v238_stock("北極星藥業-KY","生技醫療","新藥研發/腫瘤","新藥研發公司","★★☆☆☆","腫瘤與新藥研發相關公司","藥華藥、中裕、國際新藥公司","低中：研發管線與題材性","臨床結果、法規、資金需求、商業化不確定",1.05,"生技、新藥、腫瘤治療","新藥研發","新藥研發公司","授權合作方、醫院/患者市場、潛在藥廠夥伴"),
    "5904.TWO": _v238_stock("寶雅","零售/通路","美妝生活用品通路","台灣美妝生活用品通路龍頭","★★★★☆","美妝、生活用品與零售通路品牌商","小北、康是美、屈臣氏、momo通路","中高：門市密度、選品、品牌與會員經營","消費景氣、展店速度、電商競爭",1.05,"零售、通路、美妝、民生消費","下游零售通路","美妝生活用品通路龍頭","一般消費者、品牌供應商、會員與生活用品客戶"),
    "8454.TW": _v238_stock("富邦媒","電商/通路","電商平台/電視購物","台灣大型電商平台","★★★★☆","momo電商、電視購物與倉儲物流通路平台","PChome、蝦皮、統一超、零售電商同業","中高：平台流量、物流、會員與供應商網路","電商競爭、行銷成本、消費景氣",1.05,"電商、通路、物流、民生消費","下游電商平台","台灣大型電商平台","一般消費者、品牌供應商、平台商家與物流合作夥伴"),
    "2731.TWO": _v238_stock("雄獅","觀光/旅遊","旅行社/旅遊服務","台灣旅行社主要品牌","★★★☆☆","旅行社、團體旅遊、商務旅遊與旅遊服務平台","鳳凰、五福、易飛網、Klook等旅遊平台","中：品牌、通路與旅遊產品設計","景氣、匯率、油價、地緣與疫情風險",1.04,"觀光、旅遊、消費復甦","下游旅遊服務","旅行社主要品牌","旅遊消費者、企業商旅、團體旅遊客戶"),
    "2729.TW": _v238_stock("瓦城","餐飲/通路","連鎖餐飲/多品牌","連鎖餐飲集團","★★★☆☆","多品牌餐飲、展店與餐飲通路經營公司","王品、六角、漢來美食、餐飲同業","中：品牌經營、展店管理與供應鏈","食材成本、人力成本、展店速度、消費景氣",1.04,"餐飲、民生消費、展店","下游餐飲通路","連鎖餐飲集團","一般消費者、商場通路、外送平台客戶"),
    "2739.TW": _v238_stock("寒舍","觀光/飯店","飯店/餐飲","飯店與餐飲服務業者","★★★☆☆","飯店住宿、宴會、餐飲與觀光服務供應商","晶華、國賓、雲品、飯店同業","中：品牌、地點與餐飲宴會服務","觀光景氣、人力成本、住房率波動",1.03,"觀光、飯店、餐飲","下游服務業","飯店餐飲服務業者","旅客、商務客、宴會客戶、餐飲消費者"),
    "9917.TW": _v238_stock("中保科","保全/智慧城市","保全服務/系統整合","台灣保全服務主要業者","★★★★☆","保全、智慧城市、系統整合與物聯網服務供應商","新光保、物業與系統整合同業","中高：客戶基礎、服務網路與系統平台","人力成本、競爭、景氣",1.04,"保全、智慧城市、IoT、服務業","下游服務/系統整合","保全服務主要業者","家庭、企業、社區、政府與智慧城市客戶"),
    "9940.TW": _v238_stock("信義","房仲/服務","不動產仲介/代銷","台灣房仲主要品牌","★★★☆☆","房仲、不動產服務與代銷相關服務業者","永慶、住商、房仲同業","中：品牌、門市與不動產交易服務","房市交易量、政策、利率",1.02,"房仲、服務業、房市","下游不動產服務","房仲主要品牌","買賣屋客戶、屋主、建商與不動產投資客"),
    "9925.TW": _v238_stock("新保","保全/智慧城市","保全服務/物業科技","台灣保全服務業者","★★★☆☆","保全、物業科技、智慧家庭與安全服務供應商","中保科、物業與保全同業","中：服務網路、客戶基礎與系統整合","人力成本、競爭、服務品質",1.03,"保全、智慧家庭、服務業","下游服務/系統整合","保全服務業者","家庭、企業、社區、物業與政府客戶"),
}

try:
    STOCK_DB.update(V247_EXTRA_STOCKS)
    ALIASES.clear()
    for _sym, _v in STOCK_DB.items():
        ALIASES[_sym.upper()] = _sym
        ALIASES[_sym.split('.')[0]] = _sym
        ALIASES[_v.get('name','')] = _sym
        ALIASES[_v.get('name','').upper()] = _sym
    OTC_CODES = {_s.split('.')[0] for _s in STOCK_DB if _s.endswith('.TWO')}
except Exception:
    pass

# V247：恢復估值區間說明列。使用者會先看價格，所以在價格下方補一行清楚說明估值方法、資料可信度與區間定義。
def v247_query_panel(symbol):
    ...
    # 補回原本少掉的「估值區間說明」一行，避免使用者只看到數字但不知道邏輯。
    try:
        mult_txt = f"{float(info.get('fair_mult', 1.0)):.2f}x"
    except Exception:
        mult_txt = '產業倍率'
    valuation_row = [{
        '估值方法': info.get('valuation_method', '公開版初估：即時現價 × 產業/競爭力倍率'),
        '使用倍率': mult_txt,
        '區間定義': '安全邊際價 / 合理價 / 樂觀價',
        '資料可信度': info.get('confidence', 'B：公開資料與產業鏈合理推估'),
        '資料說明': '深度DCF/EVA/EBO/CAP會放入研究院/Pro版'
    }]
    ...
    with st.expander('展開完整公司資料：主要客戶 / AI客戶 / 競爭者 / 風險', expanded=False):
        ...
        st.write('護城河：', info.get('moat', '待補'))
        st.write('主要風險：', info.get('risk', '待補'))
        st.caption(f"資料更新時間：{d.get('updated','N/A')}｜現價來源：{d.get('source','Yahoo Finance')}｜公開版估值區間為初估，深度模型會逐步補入研究院/Pro版。")

# 覆蓋 V246 的查詢面板，其他首頁功能不動。
v243_query_panel = v247_query_panel
...
def settings_page():
    ...
    st.write('資料庫版本：', DB_VERSION)
    st.write('資料庫股票數：', len(STOCK_DB))
    st.write('本版修正：續增其他類股個股資料，並補回首頁估值區間的「估值方法 / 區間定義 / 資料可信度」說明列。')
    try: v240_system_dashboard(v230_rows_df())
    except Exception: pass
    st.info('公開版優先提供現價、估值區間與產業位置；主要客戶若未正式揭露，會以公開供應鏈/產業合理推估並持續校正。')

# ===== V247.0 MORE COVERAGE + VALUATION ROW RESTORE END =====

# ===== V248.0 MORE STOCK PRICE COVERAGE + DATA EXPANSION START =====
APP_VERSION = "V253.0 Formal Valuation Guard + Ranking Fix"
DB_VERSION = "TW-STOCK-20260628-V251"

# V249：800~1000檔計畫第一階段，續補更多常用台股，先讓搜尋與估值區間覆蓋率提升；
# 主要客戶若未正式揭露，先以終端客群/通路/供應鏈位置描述，後續再逐步校正。
V248_EXTRA_STOCKS = {
    "1460.TW": _v238_stock("宏遠","紡織/成衣","布料/成衣代工","紡織布料與成衣供應商","★★★☆☆","布料、成衣與機能紡織供應商","遠東新、南紡、聚陽、儒鴻","中：布料開發、成衣製造與品牌供應鏈客戶","匯率、原料成本、品牌庫存循環",1.01,"紡織、成衣、機能布料","上中游紡織/成衣","紡織布料與成衣供應商","國際成衣品牌、貿易商、運動休閒服飾客戶"),
    "1476.TW": _v238_stock("儒鴻","紡織/成衣","機能服飾/成衣代工","全球機能成衣主要供應商","★★★★☆","機能成衣、彈性布料與運動服飾供應鏈重要廠商","聚陽、遠東新、Makalot、國際成衣代工同業","高：機能布料、設計打樣、交期與品牌客戶黏著","品牌庫存循環、匯率、產能與工資成本",1.06,"紡織、運動服飾、機能布料","中下游成衣供應鏈","全球機能成衣重要供應商","國際運動品牌、戶外服飾品牌、快時尚與零售品牌客戶"),
    "1477.TW": _v238_stock("聚陽","紡織/成衣","成衣代工/快速反應供應鏈","成衣代工主要廠","★★★★☆","成衣代工、快速反應供應鏈與國際品牌服飾供應商","儒鴻、遠東新、成衣代工同業","中高：客戶組合、供應鏈管理與快速交期","消費景氣、品牌庫存、匯率與人力成本",1.05,"紡織、成衣、品牌供應鏈","中下游成衣供應鏈","成衣代工主要廠","國際服飾品牌、零售通路、運動休閒與快時尚客戶"),
    "1504.TW": _v238_stock("東元","電機/重電","馬達/電控/能源設備","台灣馬達與電機主要廠","★★★★☆","馬達、電控、節能設備與重電系統供應商","ABB、Siemens、士電、大同、華城","中高：馬達技術、品牌、通路與工業客戶基礎","原物料、景氣循環、海外競爭",1.05,"重電、馬達、節能、資料中心電力","中游電機設備","馬達與電機主要廠","工業客戶、資料中心機電、空調節能、交通與基礎建設客戶"),
    "1507.TW": _v238_stock("永大","電梯/機電","電梯/機電設備","電梯與機電設備供應商","★★★☆☆","電梯、升降設備與維保服務供應商","崇友、Otis、Mitsubishi、Kone","中：安裝基礎、維保服務與建築通路","房市景氣、原料成本、維保競爭",1.02,"電梯、建築、機電服務","下游機電設備/維保","電梯設備供應商","建商、商辦、住宅社區、公共工程與維保客戶"),
    "1514.TW": _v238_stock("亞力","電力/重電","配電盤/電力設備","台灣電力設備供應商","★★★★☆","配電盤、變電設備、電網與軌道電力相關供應商","華城、中興電、士電、大同","中高：台電與公共工程案源、電力設備認證","政策節奏、交期、原物料、專案毛利",1.08,"重電、電網、台電強韌電網、軌道建設","中游電力設備","台灣電力設備供應商","台電、公共工程、科技廠、軌道交通與工業用電客戶"),
    "1605.TW": _v238_stock("華新","電線電纜/材料","電線電纜/不鏽鋼/能源材料","電線電纜與材料主要廠","★★★☆☆","電線電纜、不鏽鋼、能源與材料相關集團","大亞、宏泰、中鋼、不鏽鋼材料同業","中：電纜通路、材料產能與集團資源","銅鎳價格、景氣循環、匯率",1.03,"電線電纜、電網、材料、不鏽鋼","上中游材料/電纜","電線電纜與材料主要廠","電力工程、建築工程、工業材料、能源基建客戶"),
    "1609.TW": _v238_stock("大亞","電線電纜/能源","電線電纜/太陽能/儲能","電纜與綠能供應商","★★★☆☆","電線電纜、太陽能、儲能與能源工程相關供應商","華新、宏泰、電纜同業、綠能業者","中：電纜基礎、能源轉型布局與工程客戶","銅價、綠能政策、工程進度",1.04,"電線電纜、綠能、儲能、電網","中游電纜/能源工程","電纜與綠能供應商","台電、工程商、建商、太陽能與儲能客戶"),
    "1717.TW": _v238_stock("長興","化工/材料","合成樹脂/電子材料","化工材料供應商","★★★☆☆","合成樹脂、電子化學材料與特用化學品供應商","南亞、台塑化、國際化工材料廠","中：化工材料配方、客戶基礎與產品線","原料價格、景氣循環、環保法規",1.03,"化工、電子材料、樹脂","上游材料","化工材料供應商","PCB、塗料、電子材料、工業材料與化工客戶"),
    "1722.TW": _v238_stock("台肥","化肥/資產","肥料/生技/土地資產","台灣肥料與資產題材公司","★★★☆☆","肥料、農業資材、生技與土地資產相關公司","東鹼、農業資材同業、資產股同業","中：品牌、農業通路與土地資產","肥料價格、政策、土地開發進度",1.02,"肥料、資產、生技、農業","上游農業資材/資產","肥料與資產題材公司","農業客戶、通路商、土地開發與租賃客戶"),
    "1736.TW": _v238_stock("喬山","運動器材","健身器材/商用設備","全球健身器材品牌與製造商","★★★☆☆","商用與家用健身器材品牌、製造與通路供應商","Dyaco、Peloton、Life Fitness、Technogym","中：品牌、通路、產品設計與商用客戶","消費景氣、庫存、匯率、運費",1.04,"運動器材、健康、品牌通路","下游品牌/製造","健身器材主要供應商","健身房、飯店、企業、家庭消費者與運動器材通路"),
    "1789.TW": _v238_stock("神隆","生技醫療","原料藥/API","原料藥供應商","★★★☆☆","高活性原料藥、學名藥原料與CDMO相關供應商","旭富、國際API/CDMO廠","中：GMP產線、原料藥製程與國際客戶","藥證、客戶訂單、價格競爭、法規",1.03,"生技、原料藥、CDMO","上游醫藥製造","原料藥供應商","國際藥廠、學名藥廠、CDMO客戶與研發合作夥伴"),
    "1817.TW": _v238_stock("凱撒衛","建材/衛浴","衛浴設備/建材通路","衛浴設備品牌供應商","★★★☆☆","衛浴設備、建材通路與住宅修繕市場供應商","和成、TOTO、建材衛浴同業","中：品牌、通路與產品線","房市景氣、原料成本、通路競爭",1.02,"建材、衛浴、房市修繕","下游建材通路","衛浴設備品牌供應商","建商、設計裝修、通路商與住宅修繕客戶"),
    "2014.TW": _v238_stock("中鴻","鋼鐵","熱軋/冷軋鋼品","中鋼集團鋼品供應商","★★★☆☆","熱軋、冷軋與鋼品加工供應商","中鋼、燁輝、東和鋼鐵、鋼鐵同業","中：中鋼集團資源、鋼品產線與通路","鋼價循環、原料成本、需求波動",1.01,"鋼鐵、材料、景氣循環","上中游鋼品","鋼品供應商","製造業、建築、機械、汽車零組件與鋼材通路客戶"),
    "2023.TW": _v238_stock("燁輝","鋼鐵","鍍鋅鋼捲/烤漆鋼捲","鍍面鋼品供應商","★★★☆☆","鍍鋅、烤漆鋼捲與建材用鋼品供應商","中鋼、中鴻、盛餘、國際鋼品廠","中：鍍面鋼品產線與外銷通路","鋼價、匯率、建築需求、出口競爭",1.01,"鋼鐵、建材、鍍面鋼品","中游鋼品加工","鍍面鋼品供應商","建材、家電、機械、鋼材通路與外銷客戶"),
    "2031.TW": _v238_stock("新光鋼","鋼鐵/加工","鋼材裁剪/通路","鋼材加工與通路商","★★★☆☆","鋼材裁剪、加工、物流與通路供應商","中鋼構、大成鋼、鋼材通路同業","中：鋼材加工、倉儲與通路能力","鋼價循環、庫存評價、需求波動",1.02,"鋼鐵、鋼材加工、通路","中下游鋼材通路","鋼材加工與通路商","製造業、建築、機械設備與鋼材需求客戶"),
    "2103.TW": _v238_stock("台橡","橡膠/材料","合成橡膠/SBR/BR","合成橡膠材料供應商","★★★☆☆","合成橡膠、熱塑性橡膠與工業材料供應商","國際合成橡膠廠、南帝、橡膠同業","中：材料技術、產能與輪胎/鞋材客戶","原料價格、景氣循環、同業競爭",1.02,"橡膠、材料、輪胎鞋材","上游化工材料","合成橡膠材料供應商","輪胎廠、鞋材、工業橡膠與塑膠改質客戶"),
    "2105.TW": _v238_stock("正新","輪胎/橡膠","輪胎品牌/製造","華人輪胎品牌主要廠","★★★☆☆","汽機車、自行車與工業輪胎品牌製造商","建大、Bridgestone、Michelin、Goodyear","中高：品牌、通路、製造規模與產品線","橡膠原料、車市景氣、匯率、競爭",1.03,"輪胎、汽車、機車、自行車","下游輪胎品牌/製造","輪胎品牌主要廠","汽車售後市場、機車/自行車通路、車廠與輪胎經銷商"),
    "2106.TW": _v238_stock("建大","輪胎/橡膠","輪胎品牌/製造","輪胎品牌與製造商","★★★☆☆","汽機車、自行車與工業輪胎製造供應商","正新、國際輪胎廠、輪胎同業","中：品牌、通路與產品線","原料成本、外銷需求、匯率、競爭",1.02,"輪胎、汽機車、自行車","下游輪胎品牌/製造","輪胎製造商","汽機車售後市場、自行車品牌、經銷通路與車廠客戶"),
    "2201.TW": _v238_stock("裕隆","汽車/資產","汽車製造/品牌代理/資產","汽車集團與資產題材公司","★★★☆☆","汽車製造、品牌代理、轉投資與土地資產相關公司","和泰車、中華車、三陽、汽車同業","中：通路、品牌代理、轉投資與資產","車市需求、轉投資波動、電動車競爭",1.03,"汽車、資產、品牌代理","下游汽車製造/通路","汽車集團","汽車消費者、品牌代理通路、售後服務與轉投資客戶"),
    "2211.TW": _v238_stock("長榮鋼","鋼構/工程","鋼構/工程/電梯","鋼構與工程供應商","★★★☆☆","鋼構工程、機電工程與電梯相關供應商","中鋼構、鋼構同業、工程廠商","中：工程案源、鋼構製造與集團資源","工程進度、鋼價、公共工程與建築景氣",1.04,"鋼構、工程、建設、電梯","中下游工程/鋼構","鋼構與工程供應商","建商、公共工程、工業廠房、商辦與基礎建設客戶"),
    "2231.TW": _v238_stock("為升","汽車電子","胎壓偵測/車用電子","車用電子零組件供應商","★★★☆☆","胎壓偵測器、車用電子與售後市場零組件供應商","同致、車王電、國際車用電子廠","中：車用電子產品、AM通路與認證","車市景氣、匯率、競爭與產品週期",1.04,"車用電子、TPMS、汽車零組件","中游車用電子","車用電子零組件供應商","汽車售後市場、車廠、零組件通路與維修市場客戶"),
    "2301.TW": _v238_stock("光寶科","電子代工/電源","電源/光電/車電","電源與光電電子供應商","★★★★☆","電源供應器、光電、車用電子與雲端電源相關供應商","台達電、群電、康舒、電源同業","中高：電源技術、製造規模、客戶基礎","消費電子循環、匯率、毛利率",1.05,"電源、雲端、車用電子、光電","中游電子零組件/電源","電源與光電電子供應商","PC、雲端伺服器、品牌電子、車用電子與工業客戶","AI伺服器電源間接受惠"),
    "2329.TW": _v238_stock("華泰","半導體封測","記憶體封測/模組","記憶體封測供應商","★★★☆☆","記憶體封裝測試、模組與半導體後段服務供應商","日月光、力成、南茂、封測同業","中：記憶體封測產能與客戶基礎","記憶體循環、價格、稼動率",1.04,"半導體封測、記憶體、AI記憶體","中下游封測","記憶體封測供應商","記憶體IC廠、模組廠、IDM與封測客戶","HBM/記憶體景氣間接受惠"),
    "2347.TW": _v238_stock("聯強","電子通路","資訊產品通路/系統整合","亞太資訊通路主要商","★★★☆☆","IT產品、通訊產品、企業解決方案與通路服務供應商","神腦、燦坤、資訊通路同業","中高：通路網路、品牌代理與企業客戶","通路毛利、庫存、景氣循環",1.03,"電子通路、AI PC、企業IT","下游通路/系統整合","資訊通路主要商","品牌原廠、企業IT客戶、零售通路與經銷商","AI PC換機間接受惠"),
    "2352.TW": _v238_stock("佳世達","電子代工/醫療","顯示器/醫療/工業電腦","電子製造與醫療事業集團","★★★☆☆","顯示器、醫療、工業電腦與企業解決方案供應商","友達集團、緯創、研華、醫療設備同業","中：集團資源、產品組合與通路","顯示器景氣、轉投資波動、競爭",1.03,"顯示器、醫療、工業電腦、企業IT","中下游電子製造/醫療","電子製造與醫療事業集團","品牌客戶、醫療院所、企業IT與工業客戶"),
    "2360.TW": _v238_stock("致茂","測試設備","電子量測/自動化測試","測試量測設備主要供應商","★★★★☆","電子量測、自動化測試、電動車與半導體測試設備供應商","Keysight、Teradyne、測試設備同業","中高：量測技術、客戶認證與產品線","資本支出循環、專案出貨、匯率",1.08,"測試設備、半導體、EV、AI伺服器","中游設備/量測","測試量測設備主要供應商","半導體、電動車、電池、電源、ICT與自動化測試客戶","AI伺服器電源/半導體測試間接受惠"),
    "2362.TW": _v238_stock("藍天","電腦/百貨資產","筆電/商場/資產","筆電與商場資產公司","★★★☆☆","筆電、通路商場與資產相關集團","華碩、宏碁、百貨通路同業","中：品牌歷史、通路與資產基礎","PC景氣、商場營運、資產評價",1.02,"PC、商場、資產","下游品牌/通路資產","筆電與商場資產公司","PC通路客戶、商場租戶、消費者與資產開發客戶"),
    "2385.TW": _v238_stock("群光","電子零組件","鍵盤/影像/電源","電腦周邊零組件供應商","★★★★☆","鍵盤、影像模組、電源與消費電子零組件供應商","致伸、光寶科、零組件同業","中高：產品設計、品牌客戶與製造規模","PC需求、毛利率、匯率",1.04,"鍵盤、影像、AI PC、電源","中游電腦零組件","電腦周邊零組件供應商","筆電品牌、PC ODM、消費電子與企業產品客戶","AI PC換機間接受惠"),
    "2395.TW": _v238_stock("研華","工業電腦","工業電腦/邊緣AI","全球工業電腦領導廠","★★★★★","工業電腦、邊緣運算、嵌入式平台與自動化解決方案供應商","凌華、Kontron、Siemens IPC、DFI","高：品牌、通路、產品線、全球客戶與軟硬整合","工業景氣循環、庫存、匯率",1.10,"工業電腦、邊緣AI、自動化、IoT","中下游工業運算平台","全球工業電腦領導廠","工業自動化、醫療、交通、零售、能源與邊緣AI客戶","邊緣AI與智慧製造受惠"),
    "2404.TW": _v238_stock("漢唐","無塵室/工程","半導體廠務/無塵室工程","半導體廠務工程供應商","★★★★☆","無塵室、廠務工程、機電與半導體建廠服務供應商","亞翔、聖暉、洋基工程、工程同業","中高：半導體廠務經驗、客戶認證與專案管理","資本支出循環、專案認列、客戶集中",1.08,"半導體工程、無塵室、AI建廠","下游廠務工程","半導體廠務工程供應商","晶圓廠、封測廠、電子廠與科技廠建廠客戶","AI/HPC建廠與先進製程投資受惠"),
    "2474.TW": _v238_stock("可成","機構件/資產","金屬機殼/投資資產","金屬機殼與投資資產公司","★★★☆☆","金屬機構件、消費電子機殼與投資/資產配置公司","鴻準、鎧勝歷史同業、機構件同業","中：金屬加工技術、財務資源與資產部位","消費電子需求、轉型進度、投資波動",1.02,"機構件、消費電子、資產","中游機構件/資產","金屬機構件與資產公司","品牌電子、筆電/手機供應鏈與投資資產相關收益"),
    "2486.TW": _v238_stock("一詮","LED/光電","LED導線架/光電零件","LED零組件供應商","★★★☆☆","LED導線架、光電零組件與電子材料供應商","億光、光磊、LED零組件同業","中：LED零組件製造與客戶基礎","LED景氣、價格競爭、產品轉型",1.02,"LED、光電、電子零組件","中游光電零組件","LED零組件供應商","LED封裝廠、照明、顯示與光電模組客戶"),
    "2498.TW": _v238_stock("宏達電","XR/品牌","VR/AR/手機品牌","XR與智慧裝置品牌公司","★★★☆☆","VR/AR、智慧裝置、軟硬體平台與品牌業者","Meta、Sony、Pico、XR裝置品牌","中：品牌、XR技術與專利資源","XR市場成長、產品銷售、虧損與競爭",1.05,"XR、VR、AR、元宇宙、智慧裝置","下游品牌/平台","XR智慧裝置品牌公司","消費者、企業XR應用、教育訓練與內容生態客戶","AI/XR應用間接受惠"),
    "2542.TW": _v238_stock("興富發","營建/建設","住宅建設/商辦","台灣大型建商","★★★☆☆","住宅、商辦與土地開發建設公司","國建、華固、遠雄、營建同業","中：土地庫存、品牌與推案經驗","房市政策、利率、銷售與交屋認列",1.02,"營建、房市、資產","下游建設開發","大型建商","購屋客戶、商辦租售客戶、土地與建築合作夥伴"),
    "2606.TW": _v238_stock("裕民","散裝航運","散裝船/航運","散裝航運主要公司","★★★☆☆","散裝航運、船隊營運與大宗物資運輸服務供應商","慧洋-KY、台航、新興、國際散裝航運商","中：船隊規模、客戶關係與航運經驗","BDI運價、油價、匯率、景氣循環",1.04,"航運、散裝、大宗商品","下游航運服務","散裝航運主要公司","礦砂、煤炭、穀物與大宗物資運輸客戶"),
    "2609.TW": _v238_stock("陽明","貨櫃航運","貨櫃航運/海運","全球貨櫃航運公司","★★★☆☆","貨櫃航運、海運服務與國際物流供應商","長榮、萬海、Maersk、MSC、Hapag-Lloyd","中：航線布局、船隊與聯盟合作","運價循環、油價、供需、地緣風險",1.05,"航運、貨櫃、全球貿易","下游海運服務","貨櫃航運公司","進出口商、物流商、跨國企業與全球貨運客戶"),
    "2610.TW": _v238_stock("華航","航空/觀光","航空客運/貨運","台灣航空主要公司","★★★☆☆","客運、貨運、航線營運與航空服務供應商","長榮航、星宇航空、國際航空公司","中：航權、機隊、品牌與貨運能力","油價、匯率、旅遊需求、景氣與地緣",1.03,"航空、觀光、貨運","下游航空服務","航空主要公司","旅客、貨運代理、跨境物流、商務與觀光客戶"),
    "2618.TW": _v238_stock("長榮航","航空/觀光","航空客運/貨運","台灣航空主要公司","★★★★☆","客運、貨運、航線營運與航空服務供應商","華航、星宇航空、國際航空公司","中高：品牌、航線、機隊與服務品質","油價、匯率、旅遊需求、景氣與地緣",1.04,"航空、觀光、貨運","下游航空服務","航空主要公司","旅客、貨運代理、跨境物流、商務與觀光客戶"),
    "2636.TW": _v238_stock("台驊投控","物流/貨代","海空運承攬/物流","國際貨運承攬與物流商","★★★☆☆","海運、空運承攬、物流整合與供應鏈服務供應商","中菲行、捷迅、貨代同業","中：客戶關係、航運資源與物流網路","運價波動、全球貿易、景氣循環",1.03,"物流、貨代、供應鏈","下游物流服務","國際貨運承攬商","進出口商、電商、製造業與跨境供應鏈客戶"),
    "2707.TW": _v238_stock("晶華","飯店/觀光","高端飯店/餐飲","飯店餐飲品牌集團","★★★☆☆","高端飯店、餐飲、旅館管理與觀光服務供應商","寒舍、國賓、雲品、飯店同業","中高：品牌、地點、餐飲與管理能力","觀光景氣、住房率、人力與食材成本",1.04,"觀光、飯店、餐飲","下游服務業","飯店餐飲品牌集團","國內外旅客、商務客、宴會與餐飲消費者"),
    "2912.TW": _v238_stock("統一超","零售/通路","便利商店/零售服務","台灣便利商店龍頭","★★★★★","便利商店、零售服務、物流與生活服務平台","全家、萊爾富、OK、零售通路同業","高：門市密度、物流、品牌、會員與生活服務生態","展店飽和、人力成本、消費景氣",1.06,"零售、便利商店、民生消費、物流","下游零售通路","便利商店龍頭","一般消費者、品牌供應商、加盟主、電商/物流與生活服務客戶"),
    "3010.TW": _v238_stock("華立","電子通路/材料","半導體材料通路/工程塑膠","電子材料通路商","★★★☆☆","半導體材料、工程塑膠與電子材料通路供應商","崇越、大聯大、材料通路同業","中：材料代理、客戶基礎與供應商關係","半導體景氣、匯率、通路毛利",1.04,"半導體材料、電子通路、工程塑膠","中游材料通路","電子材料通路商","晶圓廠、封測廠、電子製造、塑膠與工業材料客戶","半導體材料需求受惠"),
    "3023.TW": _v238_stock("信邦","連接器/線束","連接線/工業醫療線束","高階線束與連接解決方案供應商","★★★★☆","工業、醫療、綠能、車用與通訊線束/連接解決方案供應商","貿聯-KY、良維、胡連、連接器同業","中高：客製化、認證、分散客戶與產品組合","景氣循環、匯率、客戶訂單",1.06,"連接器、醫療、綠能、車用、工業","中游連接解決方案","高階線束供應商","醫療設備、工業設備、綠能、車用、通訊與電子設備客戶"),
    "3045.TW": _v238_stock("台灣大","電信/通訊","電信服務/寬頻/數位服務","台灣電信主要業者","★★★★☆","行動通訊、寬頻、企業通訊、電商與數位服務供應商","中華電、遠傳、亞太整併同業","中高：頻譜、用戶基礎、品牌與企業服務","價格競爭、資本支出、監管",1.03,"電信、5G、數位服務、雲端通訊","下游電信服務","電信主要業者","個人用戶、企業客戶、寬頻、雲端與數位服務客戶"),
    "3059.TW": _v238_stock("華晶科","光學/影像","數位影像/車用影像","影像模組與數位相機供應商","★★★☆☆","數位影像、車用影像與光學相關產品供應商","群光、亞光、影像模組同業","中：影像產品設計、製造與客戶基礎","消費電子波動、車用導入、競爭",1.03,"影像、車用電子、光學","中下游影像模組","影像產品供應商","車用、消費影像、工業影像與品牌客戶"),
    "3702.TW": _v238_stock("大聯大","電子通路","半導體通路/供應鏈服務","亞太半導體通路龍頭","★★★★☆","IC元件代理、供應鏈服務與電子通路平台","文曄、益登、增你強、Avnet、Arrow","中高：代理線、客戶網路、供應鏈金融與規模","通路毛利、庫存、半導體循環",1.04,"半導體通路、IC代理、AI供應鏈","中游電子通路","半導體通路龍頭","IC原廠、電子製造商、ODM/OEM與系統廠客戶","AI伺服器與邊緣AI元件通路受惠"),
    "3706.TW": _v238_stock("神達","工業電腦/伺服器","伺服器/車用/工業電腦","伺服器與工業系統供應商","★★★☆☆","伺服器、車用電子、工業電腦與系統整合供應商","研華、廣達、緯創、工業電腦同業","中：系統整合、客戶基礎與集團資源","毛利率、專案波動、伺服器需求",1.05,"伺服器、工業電腦、車用電子、AI","中下游系統/ODM","伺服器與工業系統供應商","雲端客戶、企業伺服器、車用與工業系統客戶","AI伺服器間接受惠"),
    "4904.TW": _v238_stock("遠傳","電信/通訊","電信服務/5G/數位服務","台灣電信主要業者","★★★★☆","行動通訊、寬頻、企業通訊、雲端與數位服務供應商","中華電、台灣大、電信同業","中高：頻譜、用戶基礎、企業服務與整併效益","價格競爭、資本支出、監管",1.03,"電信、5G、雲端、企業通訊","下游電信服務","電信主要業者","個人用戶、企業客戶、雲端/資安/通訊服務客戶"),
    "4961.TW": _v238_stock("天鈺","IC設計","電源管理IC/DDI","電源管理與顯示IC供應商","★★★☆☆","電源管理IC、顯示驅動與消費電子IC設計公司","聯詠、矽力、致新、PMIC/DDI同業","中：PMIC與顯示IC設計、客戶導入","面板循環、消費電子需求、價格競爭",1.04,"IC設計、PMIC、DDI、面板","上游IC設計","電源管理與顯示IC供應商","面板廠、品牌電子、消費電子與工業應用客戶"),
    "6271.TW": _v238_stock("同欣電","半導體封裝","陶瓷基板/影像感測封裝","利基封裝與陶瓷基板供應商","★★★☆☆","陶瓷基板、影像感測、車用與高頻元件封裝供應商","日月光、精材、利基封裝同業","中高：利基封裝、陶瓷基板與客戶認證","車用/消費需求、產品組合、稼動率",1.05,"封裝、車用、影像感測、陶瓷基板","中下游封裝材料/服務","利基封裝供應商","車用、影像感測、RF/高頻與電子元件客戶"),
    "6409.TW": _v238_stock("旭隼","電源/UPS","不斷電系統/電源設備","全球UPS電源設備供應商","★★★★☆","UPS、不斷電系統、電源保護與能源設備供應商","Schneider/APC、Eaton、台達電、電源同業","中高：產品可靠度、通路、品牌與電源技術","匯率、通路庫存、競爭、原料成本",1.06,"UPS、資料中心、電源設備、能源管理","中下游電源設備","UPS電源設備供應商","資料中心、企業IT、工業、通路商與電源設備客戶","AI資料中心電源需求間接受惠"),
    "6415.TW": _v238_stock("矽力*-KY","IC設計","電源管理IC","電源管理IC設計公司","★★★★☆","高效電源管理IC、工業/車用/消費電子PMIC供應商","TI、MPS、致新、天鈺、PMIC同業","中高：PMIC設計能力、產品線與客戶基礎","中國競爭、庫存循環、產品價格",1.07,"IC設計、PMIC、電源管理、車用","上游IC設計","電源管理IC供應商","工業、車用、消費電子、通訊與電源系統客戶","AI伺服器電源鏈間接受惠"),
    "8044.TW": _v238_stock("網家","電商/通路","電商平台/物流","台灣電商平台業者","★★★☆☆","PChome電商、網路購物、倉儲物流與平台服務供應商","富邦媒、蝦皮、電商同業","中：品牌、平台流量與物流基礎","電商競爭、行銷成本、虧損改善進度",1.02,"電商、通路、物流、民生消費","下游電商平台","電商平台業者","一般消費者、品牌供應商、平台商家與物流客戶"),
    "8112.TW": _v238_stock("至上","電子通路","半導體通路/記憶體通路","半導體與記憶體通路商","★★★☆☆","IC、記憶體、電子零組件代理與通路服務供應商","大聯大、文曄、益登、通路同業","中：代理線、客戶基礎與記憶體通路經驗","記憶體價格、庫存、通路毛利",1.04,"半導體通路、記憶體、AI供應鏈","中游電子通路","半導體通路商","IC原廠、電子製造商、模組廠與系統客戶","AI記憶體景氣間接受惠"),
    "8464.TW": _v238_stock("億豐","居家建材","窗簾/遮陽產品","全球窗簾遮陽產品供應商","★★★★☆","窗簾、遮陽產品、居家裝修與建材通路供應商","Hunter Douglas、居家建材同業","中高：產品設計、製造效率、通路與客戶基礎","美國房市、匯率、消費景氣",1.05,"居家建材、窗簾、修繕、外銷","下游居家建材","窗簾遮陽產品供應商","北美通路、居家建材零售、裝修與消費者市場"),
}

try:
    STOCK_DB.update(V248_EXTRA_STOCKS)
    ALIASES.clear()
    for _sym, _v in STOCK_DB.items():
        ALIASES[_sym.upper()] = _sym
        ALIASES[_sym.split('.')[0]] = _sym
...
# ===== V248.0 MORE STOCK PRICE COVERAGE + DATA EXPANSION END =====


# ===== V249.0 800 STOCK PHASE 1 + MORE COVERAGE START =====
APP_VERSION = "V253.0 Formal Valuation Guard + Ranking Fix"
DB_VERSION = "TW-STOCK-20260628-V251"

# V249：800~1000檔計畫第一階段。先補常用傳產、食品、塑化、紡織、重電、鋼鐵、車用等股票，讓搜尋與估值區間覆蓋率快速提升。
V249_EXTRA_STOCKS = {
    '1101.TW': _v238_stock('台泥', '水泥/建材', '水泥/儲能', '水泥/建材供應商', '★★★★☆', '水泥、預拌混凝土、建材：水泥/儲能', '台泥、亞泥、環泥、國際水泥廠', '中：產能、通路與區域市場', '原燃料、電價、房市與公共工程循環', 1.03, '建材、水泥、工程', '上游建材', '水泥建材供應商', '建商、公共工程、預拌廠、基礎建設客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1102.TW': _v238_stock('亞泥', '水泥/建材', '水泥', '水泥/建材供應商', '★★★★☆', '水泥、預拌混凝土、建材：水泥', '台泥、亞泥、環泥、國際水泥廠', '中：產能、通路與區域市場', '原燃料、電價、房市與公共工程循環', 1.03, '建材、水泥、工程', '上游建材', '水泥建材供應商', '建商、公共工程、預拌廠、基礎建設客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1103.TW': _v238_stock('嘉泥', '水泥/建材', '水泥/資產', '水泥/建材供應商', '★★★☆☆', '水泥、預拌混凝土、建材：水泥/資產', '台泥、亞泥、環泥、國際水泥廠', '中：產能、通路與區域市場', '原燃料、電價、房市與公共工程循環', 1.01, '建材、水泥、工程', '上游建材', '水泥建材供應商', '建商、公共工程、預拌廠、基礎建設客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1104.TW': _v238_stock('環泥', '水泥/建材', '水泥/建材', '水泥/建材供應商', '★★★☆☆', '水泥、預拌混凝土、建材：水泥/建材', '台泥、亞泥、環泥、國際水泥廠', '中：產能、通路與區域市場', '原燃料、電價、房市與公共工程循環', 1.01, '建材、水泥、工程', '上游建材', '水泥建材供應商', '建商、公共工程、預拌廠、基礎建設客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1108.TW': _v238_stock('幸福', '水泥/建材', '水泥', '水泥/建材供應商', '★★★☆☆', '水泥、預拌混凝土、建材：水泥', '台泥、亞泥、環泥、國際水泥廠', '中：產能、通路與區域市場', '原燃料、電價、房市與公共工程循環', 1.01, '建材、水泥、工程', '上游建材', '水泥建材供應商', '建商、公共工程、預拌廠、基礎建設客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1109.TW': _v238_stock('信大', '水泥/建材', '水泥', '水泥/建材供應商', '★★★☆☆', '水泥、預拌混凝土、建材：水泥', '台泥、亞泥、環泥、國際水泥廠', '中：產能、通路與區域市場', '原燃料、電價、房市與公共工程循環', 1.01, '建材、水泥、工程', '上游建材', '水泥建材供應商', '建商、公共工程、預拌廠、基礎建設客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1110.TW': _v238_stock('東泥', '水泥/建材', '水泥', '水泥/建材供應商', '★★★☆☆', '水泥、預拌混凝土、建材：水泥', '台泥、亞泥、環泥、國際水泥廠', '中：產能、通路與區域市場', '原燃料、電價、房市與公共工程循環', 1.01, '建材、水泥、工程', '上游建材', '水泥建材供應商', '建商、公共工程、預拌廠、基礎建設客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1201.TW': _v238_stock('味全', '食品/民生', '乳品/食品', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：乳品/食品', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1210.TW': _v238_stock('大成', '食品/民生', '肉品/飼料', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：肉品/飼料', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1213.TW': _v238_stock('大飲', '食品/民生', '飲料', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：飲料', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1215.TW': _v238_stock('卜蜂', '食品/民生', '肉品/飼料', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：肉品/飼料', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1216.TW': _v238_stock('統一', '食品/民生', '食品/通路', '食品民生消費供應商', '★★★★☆', '食品加工、飲料、通路：食品/通路', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.04, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1217.TW': _v238_stock('愛之味', '食品/民生', '食品飲料', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：食品飲料', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1218.TW': _v238_stock('泰山', '食品/民生', '食品/油品', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：食品/油品', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1219.TW': _v238_stock('福壽', '食品/民生', '飼料/食品', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：飼料/食品', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1220.TW': _v238_stock('台榮', '食品/民生', '飼料/食品', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：飼料/食品', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1225.TW': _v238_stock('福懋油', '食品/民生', '油脂', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：油脂', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1227.TW': _v238_stock('佳格', '食品/民生', '穀物/保健食品', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：穀物/保健食品', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1229.TW': _v238_stock('聯華', '食品/民生', '麵粉/食品', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：麵粉/食品', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1231.TW': _v238_stock('聯華食', '食品/民生', '鮮食/零食', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：鮮食/零食', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1232.TW': _v238_stock('大統益', '食品/民生', '油脂', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：油脂', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1233.TW': _v238_stock('天仁', '食品/民生', '茶飲/餐飲', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：茶飲/餐飲', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1234.TW': _v238_stock('黑松', '食品/民生', '飲料', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：飲料', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1235.TW': _v238_stock('興泰', '食品/民生', '食品/資產', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：食品/資產', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1236.TW': _v238_stock('宏亞', '食品/民生', '食品/巧克力', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：食品/巧克力', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1256.TW': _v238_stock('鮮活果汁-KY', '食品/民生', '飲品原料', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：飲品原料', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1307.TW': _v238_stock('三芳', '塑化/化工', '合成皮/材料', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：合成皮/材料', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1308.TW': _v238_stock('亞聚', '塑化/化工', 'PE塑膠', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：PE塑膠', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1309.TW': _v238_stock('台達化', '塑化/化工', 'ABS/PS', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：ABS/PS', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1310.TW': _v238_stock('台苯', '塑化/化工', 'SM/苯乙烯', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：SM/苯乙烯', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1312.TW': _v238_stock('國喬', '塑化/化工', 'SM/ABS', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：SM/ABS', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1313.TW': _v238_stock('聯成', '塑化/化工', 'DOP/化工', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：DOP/化工', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1315.TW': _v238_stock('達新', '塑化/化工', '塑膠製品', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：塑膠製品', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1316.TW': _v238_stock('上曜', '營建/資產', '建設/資產', '建設營造公司', '★★★☆☆', '營建、建設、資產開發：建設/資產', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1319.TW': _v238_stock('東陽', '汽車/車用', '汽車零組件/AM', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：汽車零組件/AM', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1321.TW': _v238_stock('大洋', '塑化/化工', 'PVC加工', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：PVC加工', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1323.TW': _v238_stock('永裕', '塑化/化工', '塑膠容器', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：塑膠容器', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1324.TW': _v238_stock('地球', '塑化/化工', '塑膠包材', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：塑膠包材', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1325.TW': _v238_stock('恆大', '塑化/化工', '不織布', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：不織布', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1337.TW': _v238_stock('再生-KY', '塑化/化工', '塑膠回收', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：塑膠回收', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1338.TW': _v238_stock('廣華-KY', '汽車/車用', '汽車內飾件', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：汽車內飾件', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1339.TW': _v238_stock('昭輝', '汽車/車用', '汽車零組件', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：汽車零組件', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1340.TW': _v238_stock('勝悅-KY', '塑化/化工', '鞋材', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：鞋材', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1341.TW': _v238_stock('富林-KY', '塑化/化工', '裝飾材料', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：裝飾材料', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1342.TW': _v238_stock('八貫', '塑化/化工', 'TPU/醫療材料', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：TPU/醫療材料', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1410.TW': _v238_stock('南染', '紡織/成衣', '染整', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：染整', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1413.TW': _v238_stock('宏洲', '紡織/成衣', '紡織', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1414.TW': _v238_stock('東和', '紡織/成衣', '紡織', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1416.TW': _v238_stock('廣豐', '營建/資產', '資產/商場', '建設營造公司', '★★★☆☆', '營建、建設、資產開發：資產/商場', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1417.TW': _v238_stock('嘉裕', '紡織/成衣', '成衣/品牌', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：成衣/品牌', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1418.TW': _v238_stock('東華', '紡織/成衣', '紡織', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1419.TW': _v238_stock('新紡', '紡織/成衣', '紡織/資產', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織/資產', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1423.TW': _v238_stock('利華', '紡織/成衣', '紡織', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1432.TW': _v238_stock('大魯閣', '觀光/餐飲', '休閒娛樂', '觀光餐飲服務公司', '★★★☆☆', '飯店、餐飲、觀光休閒：休閒娛樂', '王品、晶華、六角、觀光餐飲同業', '中：品牌、展店、服務與地點', '消費景氣、人力、食材成本、觀光需求', 1.03, '觀光、餐飲、內需消費', '下游服務業', '觀光餐飲服務商', '一般消費者、旅客、商務客與餐飲通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1434.TW': _v238_stock('福懋', '紡織/成衣', '布料/加油站', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：布料/加油站', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1435.TW': _v238_stock('中福', '營建/資產', '資產', '建設營造公司', '★★★☆☆', '營建、建設、資產開發：資產', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1436.TW': _v238_stock('華友聯', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '營建、建設、資產開發：建設', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1437.TW': _v238_stock('勤益控', '營建/資產', '資產', '建設營造公司', '★★★☆☆', '營建、建設、資產開發：資產', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1438.TW': _v238_stock('三地開發', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '營建、建設、資產開發：建設', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1439.TW': _v238_stock('雋揚', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '營建、建設、資產開發：建設', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1440.TW': _v238_stock('南紡', '紡織/成衣', '紡織/商場', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織/商場', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1441.TW': _v238_stock('大東', '紡織/成衣', '紡織', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1442.TW': _v238_stock('名軒', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '營建、建設、資產開發：建設', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1443.TW': _v238_stock('立益物流', '航運/物流', '物流', '運輸物流服務商', '★★★☆☆', '航運、航空、物流、貨代：物流', '長榮、陽明、華航、物流航運同業', '中：船隊/航線/物流網路與客戶基礎', '運價、油價、匯率、全球景氣與地緣風險', 1.03, '航運、航空、物流、觀光', '運輸服務', '運輸物流服務商', '進出口商、旅客、貨代、物流與跨境供應鏈客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1444.TW': _v238_stock('力麗', '紡織/成衣', '聚酯/紡織', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：聚酯/紡織', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1445.TW': _v238_stock('大宇', '紡織/成衣', '紡織', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1446.TW': _v238_stock('宏和', '紡織/成衣', '紡織/尼龍', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織/尼龍', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1447.TW': _v238_stock('力鵬', '紡織/成衣', '尼龍/紡織', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：尼龍/紡織', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1449.TW': _v238_stock('佳和', '紡織/成衣', '染整', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：染整', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1451.TW': _v238_stock('年興', '紡織/成衣', '牛仔布/成衣', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：牛仔布/成衣', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1452.TW': _v238_stock('宏益', '紡織/成衣', '紡織/資產', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織/資產', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1453.TW': _v238_stock('大將', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '營建、建設、資產開發：建設', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1454.TW': _v238_stock('台富', '紡織/成衣', '紡織', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1455.TW': _v238_stock('集盛', '紡織/成衣', '尼龍絲', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：尼龍絲', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1456.TW': _v238_stock('怡華', '營建/資產', '資產', '建設營造公司', '★★★☆☆', '營建、建設、資產開發：資產', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1457.TW': _v238_stock('宜進', '紡織/成衣', '加工絲', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：加工絲', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1459.TW': _v238_stock('聯發', '紡織/成衣', '紡織', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1463.TW': _v238_stock('強盛', '紡織/成衣', '紡織', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1464.TW': _v238_stock('得力', '紡織/成衣', '布料', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：布料', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1465.TW': _v238_stock('偉全', '紡織/成衣', '紡織', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1466.TW': _v238_stock('聚隆', '紡織/成衣', '尼龍絲', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：尼龍絲', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1467.TW': _v238_stock('南緯', '紡織/成衣', '成衣', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：成衣', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1468.TW': _v238_stock('昶和', '紡織/成衣', '紡織', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1470.TW': _v238_stock('大統新創', '紡織/成衣', '紡織/資產', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紡織/資產', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1471.TW': _v238_stock('首利', '電腦/周邊', '電子零組件', '電腦周邊與系統供應商', '★★★☆☆', 'PC、周邊、通路、系統：電子零組件', '華碩、宏碁、微星、技嘉、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、品牌通路', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商與一般消費者', 'AI PC換機間接受惠'),
    '1472.TW': _v238_stock('三洋實業', '營建/資產', '營建', '建設營造公司', '★★★☆☆', '營建、建設、資產開發：營建', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1473.TW': _v238_stock('台南', '紡織/成衣', '成衣', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：成衣', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1474.TW': _v238_stock('弘裕', '紡織/成衣', '布料', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：布料', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1475.TW': _v238_stock('業旺', '紡織/成衣', '成衣', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：成衣', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1503.TW': _v238_stock('士電', '電力/重電', '重電/馬達', '電力設備供應商', '★★★★☆', '重電、電網、電機、電線電纜：重電/馬達', '華城、中興電、士電、亞力、ABB、Siemens', '中高：認證、工程案源與電網需求', '政策節奏、交期、原物料、專案毛利', 1.06, '重電、電網、能源、資料中心電力', '電力設備產業鏈', '電力設備供應商', '台電、科技廠、公共工程、資料中心與工業用電客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1515.TW': _v238_stock('力山', '電機/機械', '電動工具/健身器材', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：電動工具/健身器材', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1516.TW': _v238_stock('川飛', '汽車/車用', '汽車零組件', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：汽車零組件', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1521.TW': _v238_stock('大億', '汽車/車用', '車燈', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：車燈', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1522.TW': _v238_stock('堤維西', '汽車/車用', '車燈AM', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：車燈AM', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1524.TW': _v238_stock('耿鼎', '汽車/車用', '汽車鈑金件', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：汽車鈑金件', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1525.TW': _v238_stock('江申', '汽車/車用', '汽車零組件', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：汽車零組件', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1526.TW': _v238_stock('日馳', '電機/機械', '機械', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：機械', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1527.TW': _v238_stock('鑽全', '電機/機械', '氣動工具', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：氣動工具', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1528.TW': _v238_stock('恩德', '電機/機械', '工具機', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：工具機', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1529.TW': _v238_stock('樂士', '電力/重電', '電力設備', '電力設備供應商', '★★★★☆', '重電、電網、電機、電線電纜：電力設備', '華城、中興電、士電、亞力、ABB、Siemens', '中高：認證、工程案源與電網需求', '政策節奏、交期、原物料、專案毛利', 1.06, '重電、電網、能源、資料中心電力', '電力設備產業鏈', '電力設備供應商', '台電、科技廠、公共工程、資料中心與工業用電客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1530.TW': _v238_stock('亞崴', '電機/機械', '工具機', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：工具機', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1531.TW': _v238_stock('高林股', '電機/機械', '縫紉機/通路', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：縫紉機/通路', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1532.TW': _v238_stock('勤美', '營建/資產', '鑄件/資產', '建設營造公司', '★★★☆☆', '營建、建設、資產開發：鑄件/資產', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1533.TW': _v238_stock('車王電', '汽車/車用', '車用電子', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：車用電子', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1535.TW': _v238_stock('中宇', '電力/重電', '工程/環保', '電力設備供應商', '★★★★☆', '重電、電網、電機、電線電纜：工程/環保', '華城、中興電、士電、亞力、ABB、Siemens', '中高：認證、工程案源與電網需求', '政策節奏、交期、原物料、專案毛利', 1.06, '重電、電網、能源、資料中心電力', '電力設備產業鏈', '電力設備供應商', '台電、科技廠、公共工程、資料中心與工業用電客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1536.TW': _v238_stock('和大', '汽車/車用', '車用齒輪', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：車用齒輪', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1537.TW': _v238_stock('廣隆', '電池/能源', '鉛酸電池', '電力設備供應商', '★★★☆☆', '重電、電網、電機、電線電纜：鉛酸電池', '華城、中興電、士電、亞力、ABB、Siemens', '中高：認證、工程案源與電網需求', '政策節奏、交期、原物料、專案毛利', 1.06, '重電、電網、能源、資料中心電力', '電力設備產業鏈', '電力設備供應商', '台電、科技廠、公共工程、資料中心與工業用電客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1538.TW': _v238_stock('正峰', '電機/機械', '機械', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：機械', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1539.TW': _v238_stock('巨庭', '電機/機械', '電動工具', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：電動工具', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1540.TW': _v238_stock('喬福', '電機/機械', '工具機', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：工具機', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1541.TW': _v238_stock('錩泰', '電機/機械', '電動工具', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：電動工具', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1558.TW': _v238_stock('伸興', '電機/機械', '縫紉機', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：縫紉機', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1560.TW': _v238_stock('中砂', '半導體', '晶圓再生/鑽石碟', '半導體供應鏈公司', '★★★★☆', '半導體、IC設計、封測、材料：晶圓再生/鑽石碟', '台積電、聯電、日月光、國際半導體同業', '中高：技術、客戶認證與產業鏈位置', '半導體循環、資本支出、競爭與庫存', 1.07, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、電子系統客戶', 'AI/HPC供應鏈間接受惠'),
    '1568.TW': _v238_stock('倉佑', '汽車/車用', '汽車傳動件', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：汽車傳動件', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1582.TW': _v238_stock('信錦', '電子零組件', '軸承/轉軸', '電子零組件供應商', '★★★☆☆', '電子零組件、連接器、機構件：軸承/轉軸', '同業零組件廠、國際電子供應商', '中：客戶認證、製程與產品線', '終端需求、價格競爭、匯率、庫存循環', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用與工業電子客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1583.TW': _v238_stock('程泰', '電機/機械', '工具機', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：工具機', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1587.TW': _v238_stock('吉茂', '汽車/車用', '汽車水箱', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：汽車水箱', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1589.TW': _v238_stock('永冠-KY', '電機/機械', '風電/鑄件', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：風電/鑄件', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1590.TW': _v238_stock('亞德客-KY', '自動化/機器人', '氣動元件', '傳統產業公司', '★★★★☆', '多角化傳產、生活消費、服務：氣動元件', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.04, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1598.TW': _v238_stock('岱宇', '運動器材', '健身器材', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：健身器材', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1603.TW': _v238_stock('華電', '電線電纜/材料', '電線電纜', '電力設備供應商', '★★★☆☆', '重電、電網、電機、電線電纜：電線電纜', '華城、中興電、士電、亞力、ABB、Siemens', '中高：認證、工程案源與電網需求', '政策節奏、交期、原物料、專案毛利', 1.06, '重電、電網、能源、資料中心電力', '電力設備產業鏈', '電力設備供應商', '台電、科技廠、公共工程、資料中心與工業用電客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1604.TW': _v238_stock('聲寶', '家電/品牌', '家電品牌', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：家電品牌', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1611.TW': _v238_stock('中電', '電機/照明', '照明/線材', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：照明/線材', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1612.TW': _v238_stock('宏泰', '電線電纜/材料', '電線電纜', '電力設備供應商', '★★★☆☆', '重電、電網、電機、電線電纜：電線電纜', '華城、中興電、士電、亞力、ABB、Siemens', '中高：認證、工程案源與電網需求', '政策節奏、交期、原物料、專案毛利', 1.06, '重電、電網、能源、資料中心電力', '電力設備產業鏈', '電力設備供應商', '台電、科技廠、公共工程、資料中心與工業用電客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1614.TW': _v238_stock('三洋電', '家電/品牌', '家電', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：家電', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1615.TW': _v238_stock('大山', '電線電纜/材料', '電線電纜', '電力設備供應商', '★★★☆☆', '重電、電網、電機、電線電纜：電線電纜', '華城、中興電、士電、亞力、ABB、Siemens', '中高：認證、工程案源與電網需求', '政策節奏、交期、原物料、專案毛利', 1.06, '重電、電網、能源、資料中心電力', '電力設備產業鏈', '電力設備供應商', '台電、科技廠、公共工程、資料中心與工業用電客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1616.TW': _v238_stock('億泰', '電線電纜/材料', '電線電纜', '電力設備供應商', '★★★☆☆', '重電、電網、電機、電線電纜：電線電纜', '華城、中興電、士電、亞力、ABB、Siemens', '中高：認證、工程案源與電網需求', '政策節奏、交期、原物料、專案毛利', 1.06, '重電、電網、能源、資料中心電力', '電力設備產業鏈', '電力設備供應商', '台電、科技廠、公共工程、資料中心與工業用電客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1617.TW': _v238_stock('榮星', '電線電纜/材料', '漆包線', '電力設備供應商', '★★★☆☆', '重電、電網、電機、電線電纜：漆包線', '華城、中興電、士電、亞力、ABB、Siemens', '中高：認證、工程案源與電網需求', '政策節奏、交期、原物料、專案毛利', 1.06, '重電、電網、能源、資料中心電力', '電力設備產業鏈', '電力設備供應商', '台電、科技廠、公共工程、資料中心與工業用電客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1618.TW': _v238_stock('合機', '電線電纜/材料', '電線電纜', '電力設備供應商', '★★★☆☆', '重電、電網、電機、電線電纜：電線電纜', '華城、中興電、士電、亞力、ABB、Siemens', '中高：認證、工程案源與電網需求', '政策節奏、交期、原物料、專案毛利', 1.06, '重電、電網、能源、資料中心電力', '電力設備產業鏈', '電力設備供應商', '台電、科技廠、公共工程、資料中心與工業用電客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1626.TW': _v238_stock('艾美特-KY', '家電/品牌', '小家電', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：小家電', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1708.TW': _v238_stock('東鹼', '化工/材料', '鉀肥/化工', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：鉀肥/化工', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1709.TW': _v238_stock('和益', '塑化/化工', '化工原料', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：化工原料', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1710.TW': _v238_stock('東聯', '塑化/化工', 'EG/化工', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：EG/化工', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1711.TW': _v238_stock('永光', '化工/材料', '染料/特化', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：染料/特化', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1712.TW': _v238_stock('興農', '化工/材料', '農藥/農業資材', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：農藥/農業資材', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1713.TW': _v238_stock('國化', '化工/材料', '化工', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：化工', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1714.TW': _v238_stock('和桐', '塑化/化工', '化工/清潔原料', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：化工/清潔原料', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1718.TW': _v238_stock('中纖', '塑化/化工', '化纖/化工', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：化纖/化工', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1720.TW': _v238_stock('生達', '生技醫療', '藥品', '生技醫療公司', '★★★☆☆', '醫材、藥品、醫療服務：藥品', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1721.TW': _v238_stock('三晃', '化工/材料', '特用化學', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：特用化學', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1723.TW': _v238_stock('中碳', '化工/材料', '煤化工/電池材料', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：煤化工/電池材料', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1725.TW': _v238_stock('元禎', '塑化/化工', '塑化通路', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：塑化通路', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1726.TW': _v238_stock('永記', '化工/材料', '塗料', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：塗料', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1727.TW': _v238_stock('中華化', '化工/材料', '化學品', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：化學品', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1730.TW': _v238_stock('花仙子', '食品/民生', '清潔用品', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：清潔用品', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1731.TW': _v238_stock('美吾華', '生技醫療', '藥妝/個人護理', '生技醫療公司', '★★★☆☆', '醫材、藥品、醫療服務：藥妝/個人護理', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1732.TW': _v238_stock('毛寶', '食品/民生', '清潔用品', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：清潔用品', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1733.TW': _v238_stock('五鼎', '生技醫療', '血糖檢測', '生技醫療公司', '★★★☆☆', '醫材、藥品、醫療服務：血糖檢測', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1734.TW': _v238_stock('杏輝', '生技醫療', '藥品', '生技醫療公司', '★★★☆☆', '醫材、藥品、醫療服務：藥品', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1735.TW': _v238_stock('日勝化', '化工/材料', '化工', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：化工', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1737.TW': _v238_stock('臺鹽', '食品/民生', '鹽品/生技', '食品民生消費供應商', '★★★☆☆', '食品加工、飲料、通路：鹽品/生技', '統一、味全、大成、食品同業', '中：品牌、通路與產品組合', '原物料、匯率、消費景氣、食安', 1.02, '食品、民生消費、通路', '下游食品/通路', '食品民生供應商', '量販、超商、餐飲、食品通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1760.TW': _v238_stock('寶齡富錦', '生技醫療', '藥品', '生技醫療公司', '★★★☆☆', '醫材、藥品、醫療服務：藥品', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1762.TW': _v238_stock('中化生', '生技醫療', '原料藥', '生技醫療公司', '★★★☆☆', '醫材、藥品、醫療服務：原料藥', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1773.TW': _v238_stock('勝一', '化工/材料', '溶劑', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：溶劑', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1776.TW': _v238_stock('展宇', '化工/材料', '塗料/樹脂', '化工材料供應商', '★★★☆☆', '化工材料、塑膠、特用化學：塗料/樹脂', '台塑、南亞、台化、國際化工廠', '中：材料配方、產能與客戶基礎', '油價、原料價格、景氣循環、環保法規', 1.01, '塑化、化工、材料', '上中游材料', '化工材料供應商', '電子、工業、塑膠加工、建材與化工客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1783.TW': _v238_stock('和康生', '生技醫療', '醫材', '生技醫療公司', '★★★☆☆', '醫材、藥品、醫療服務：醫材', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1786.TW': _v238_stock('科妍', '生技醫療', '醫美/玻尿酸', '生技醫療公司', '★★★☆☆', '醫材、藥品、醫療服務：醫美/玻尿酸', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1789.TW': _v238_stock('神隆', '生技醫療', '原料藥', '生技醫療公司', '★★★☆☆', '醫材、藥品、醫療服務：原料藥', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1795.TW': _v238_stock('美時', '生技醫療', '學名藥', '生技醫療公司', '★★★☆☆', '醫材、藥品、醫療服務：學名藥', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1802.TW': _v238_stock('台玻', '玻璃/建材', '玻璃', '水泥/建材供應商', '★★★★☆', '水泥、預拌混凝土、建材：玻璃', '台泥、亞泥、環泥、國際水泥廠', '中：產能、通路與區域市場', '原燃料、電價、房市與公共工程循環', 1.03, '建材、水泥、工程', '上游建材', '水泥建材供應商', '建商、公共工程、預拌廠、基礎建設客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1805.TW': _v238_stock('寶徠', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '營建、建設、資產開發：建設', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1806.TW': _v238_stock('冠軍', '建材/陶瓷', '磁磚', '水泥/建材供應商', '★★★☆☆', '水泥、預拌混凝土、建材：磁磚', '台泥、亞泥、環泥、國際水泥廠', '中：產能、通路與區域市場', '原燃料、電價、房市與公共工程循環', 1.01, '建材、水泥、工程', '上游建材', '水泥建材供應商', '建商、公共工程、預拌廠、基礎建設客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1808.TW': _v238_stock('潤隆', '營建/資產', '營建/建設', '建設營造公司', '★★★☆☆', '營建、建設、資產開發：營建/建設', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1809.TW': _v238_stock('中釉', '建材/陶瓷', '釉料', '水泥/建材供應商', '★★★☆☆', '水泥、預拌混凝土、建材：釉料', '台泥、亞泥、環泥、國際水泥廠', '中：產能、通路與區域市場', '原燃料、電價、房市與公共工程循環', 1.01, '建材、水泥、工程', '上游建材', '水泥建材供應商', '建商、公共工程、預拌廠、基礎建設客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1810.TW': _v238_stock('和成', '建材/衛浴', '衛浴設備', '水泥/建材供應商', '★★★☆☆', '水泥、預拌混凝土、建材：衛浴設備', '台泥、亞泥、環泥、國際水泥廠', '中：產能、通路與區域市場', '原燃料、電價、房市與公共工程循環', 1.01, '建材、水泥、工程', '上游建材', '水泥建材供應商', '建商、公共工程、預拌廠、基礎建設客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1903.TW': _v238_stock('士紙', '造紙/資產', '紙業/資產', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紙業/資產', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1904.TW': _v238_stock('正隆', '造紙/包材', '工紙/紙器', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：工紙/紙器', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1905.TW': _v238_stock('華紙', '造紙/材料', '紙漿/紙品', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紙漿/紙品', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1906.TW': _v238_stock('寶隆', '造紙/包材', '紙器', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紙器', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1907.TW': _v238_stock('永豐餘', '造紙/包材', '紙品/包材', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：紙品/包材', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '1909.TW': _v238_stock('榮成', '造紙/包材', '工紙/紙箱', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：工紙/紙箱', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2002.TW': _v238_stock('中鋼', '鋼鐵/材料', '鋼鐵', '鋼鐵材料供應商', '★★★★☆', '鋼鐵、金屬材料、加工：鋼鐵', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.03, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2006.TW': _v238_stock('東和鋼鐵', '鋼鐵/材料', '型鋼/鋼筋', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：型鋼/鋼筋', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2007.TW': _v238_stock('燁興', '鋼鐵/材料', '線材', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：線材', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2008.TW': _v238_stock('高興昌', '鋼鐵/材料', '鋼管', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：鋼管', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2009.TW': _v238_stock('第一銅', '鋼鐵/材料', '銅材', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：銅材', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2010.TW': _v238_stock('春源', '鋼鐵/材料', '鋼材加工', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：鋼材加工', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2012.TW': _v238_stock('春雨', '鋼鐵/材料', '螺絲/線材', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：螺絲/線材', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2013.TW': _v238_stock('中鋼構', '鋼鐵/材料', '鋼構', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：鋼構', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2015.TW': _v238_stock('豐興', '鋼鐵/材料', '鋼筋/型鋼', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：鋼筋/型鋼', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2017.TW': _v238_stock('官田鋼', '鋼鐵/材料', '鋼筋', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：鋼筋', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2020.TW': _v238_stock('美亞', '鋼鐵/材料', '鋼管', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：鋼管', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2022.TW': _v238_stock('聚亨', '鋼鐵/材料', '鋼材', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：鋼材', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2024.TW': _v238_stock('志聯', '鋼鐵/材料', '鋼線', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：鋼線', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2025.TW': _v238_stock('千興', '鋼鐵/材料', '不鏽鋼', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：不鏽鋼', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2027.TW': _v238_stock('大成鋼', '鋼鐵/材料', '不鏽鋼通路', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：不鏽鋼通路', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2028.TW': _v238_stock('威致', '鋼鐵/材料', '鋼筋', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：鋼筋', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2029.TW': _v238_stock('盛餘', '鋼鐵/材料', '鍍鋅鋼', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：鍍鋅鋼', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2030.TW': _v238_stock('彰源', '鋼鐵/材料', '不鏽鋼管', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：不鏽鋼管', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2032.TW': _v238_stock('新鋼', '鋼鐵/材料', '不鏽鋼', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：不鏽鋼', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2033.TW': _v238_stock('佳大', '鋼鐵/材料', '鋼線', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：鋼線', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2034.TW': _v238_stock('允強', '鋼鐵/材料', '不鏽鋼管', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：不鏽鋼管', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2038.TW': _v238_stock('海光', '鋼鐵/材料', '鋼筋', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：鋼筋', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2069.TW': _v238_stock('運錩', '鋼鐵/材料', '不鏽鋼', '鋼鐵材料供應商', '★★★☆☆', '鋼鐵、金屬材料、加工：不鏽鋼', '中鋼、中鴻、大成鋼、國際鋼廠', '中：產能、通路與加工能力', '鋼價、原料成本、景氣循環、匯率', 1.01, '鋼鐵、材料、景氣循環', '上中游鋼材', '鋼鐵材料供應商', '建築、製造、機械、汽車與鋼材通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2101.TW': _v238_stock('南港', '輪胎/橡膠', '輪胎/資產', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：輪胎/資產', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2102.TW': _v238_stock('泰豐', '輪胎/橡膠', '輪胎', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：輪胎', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2104.TW': _v238_stock('國際中橡', '橡膠/材料', '碳黑', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：碳黑', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2107.TW': _v238_stock('厚生', '橡膠/材料', '橡膠/資產', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：橡膠/資產', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2108.TW': _v238_stock('南帝', '橡膠/材料', '乳膠', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：乳膠', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2109.TW': _v238_stock('華豐', '輪胎/橡膠', '輪胎', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：輪胎', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2114.TW': _v238_stock('鑫永銓', '橡膠/材料', '輸送帶', '傳統產業公司', '★★★☆☆', '多角化傳產、生活消費、服務：輸送帶', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2115.TW': _v238_stock('六暉-KY', '汽車/車用', '氣門嘴/車用', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：氣門嘴/車用', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2204.TW': _v238_stock('中華', '汽車/車用', '汽車製造', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：汽車製造', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2206.TW': _v238_stock('三陽工業', '汽車/車用', '機車/汽車', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：機車/汽車', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2207.TW': _v238_stock('和泰車', '汽車/車用', '汽車代理', '汽車車用供應商', '★★★★☆', '汽車、車用零組件、車電：汽車代理', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.05, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2227.TW': _v238_stock('裕日車', '汽車/車用', '汽車代理', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：汽車代理', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2228.TW': _v238_stock('劍麟', '汽車/車用', '車用零組件', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：車用零組件', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2233.TW': _v238_stock('宇隆', '汽車/車用', '車用/精密件', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：車用/精密件', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2236.TW': _v238_stock('百達-KY', '汽車/車用', '車用零組件', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：車用零組件', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2239.TW': _v238_stock('英利-KY', '汽車/車用', '汽車零組件', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：汽車零組件', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2241.TW': _v238_stock('艾姆勒', '汽車/車用', '車用散熱', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：車用散熱', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2247.TW': _v238_stock('汎德永業', '汽車/車用', '豪華車代理', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：豪華車代理', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
    '2250.TW': _v238_stock('IKKA-KY', '汽車/車用', '汽車零組件', '汽車車用供應商', '★★★☆☆', '汽車、車用零組件、車電：汽車零組件', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', 'AI直接關聯較低；依產業景氣與終端需求間接受惠'),
}

try:
    STOCK_DB.update(V249_EXTRA_STOCKS)
    ALIASES.clear()
    for _sym, _v in STOCK_DB.items():
        ALIASES[_sym.upper()] = _sym
        ALIASES[_sym.split('.')[0]] = _sym
...
# ===== V249.0 800 STOCK PHASE 1 + MORE COVERAGE END =====


# ===== V250.0 1000 STOCK PHASE 2 + VALUATION COVERAGE START =====
APP_VERSION = "V253.0 Formal Valuation Guard + Ranking Fix"
DB_VERSION = "TW-STOCK-20260628-V251"
# V250：1000檔股票資料庫第二階段；優先擴充電子、營建、金融、航運、觀光、半導體與傳產常用查詢股，全部先給估值區間。
V250_EXTRA_STOCKS = {
    '2312.TW': _v238_stock('金寶', '電子零組件', 'EMS/電子製造', '電子零組件供應商', '★★★☆☆', 'EMS/電子製造相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2314.TW': _v238_stock('台揚', '通訊/網通', '衛星通訊', '網通與通訊設備供應商', '★★★☆☆', '衛星通訊相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '2316.TW': _v238_stock('楠梓電', 'PCB', 'PCB', '電子零組件供應商', '★★★☆☆', 'PCB相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2321.TW': _v238_stock('東訊', '通訊/網通', '通訊設備', '網通與通訊設備供應商', '★★★☆☆', '通訊設備相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '2323.TW': _v238_stock('中環', '光電/面板', '光碟/資產', '光電顯示供應商', '★★★☆☆', '光碟/資產相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '2332.TW': _v238_stock('友訊', '通訊/網通', '網通品牌', '網通與通訊設備供應商', '★★★☆☆', '網通品牌相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '2338.TW': _v238_stock('光罩', '半導體', '光罩/材料', '半導體供應鏈公司', '★★★★☆', '光罩/材料相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '2342.TW': _v238_stock('茂矽', '半導體', '晶圓/功率元件', '半導體供應鏈公司', '★★★★☆', '晶圓/功率元件相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '2351.TW': _v238_stock('順德', '電子零組件', '導線架', '電子零組件供應商', '★★★☆☆', '導線架相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2354.TW': _v238_stock('鴻準', '電子零組件', '機構件/散熱', '電子零組件供應商', '★★★☆☆', '機構件/散熱相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2355.TW': _v238_stock('敬鵬', 'PCB', '車用PCB', '電子零組件供應商', '★★★☆☆', '車用PCB相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2364.TW': _v238_stock('倫飛', '電腦/周邊', '筆電/軍工', '電腦周邊與系統供應商', '★★★☆☆', '筆電/軍工相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2365.TW': _v238_stock('昆盈', '電腦/周邊', '電腦周邊', '電腦周邊與系統供應商', '★★★☆☆', '電腦周邊相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2367.TW': _v238_stock('燿華', 'PCB', 'PCB/車用', '電子零組件供應商', '★★★☆☆', 'PCB/車用相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2369.TW': _v238_stock('菱生', '半導體', '封測', '半導體供應鏈公司', '★★★★☆', '封測相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '2373.TW': _v238_stock('震旦行', '其他傳產', '辦公設備/通路', '傳統產業公司', '★★★☆☆', '辦公設備/通路相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '2374.TW': _v238_stock('佳能', '光電/面板', '光學/影像', '光電顯示供應商', '★★★☆☆', '光學/影像相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '2380.TW': _v238_stock('虹光', '電腦/周邊', '掃描器/事務機', '電腦周邊與系統供應商', '★★★☆☆', '掃描器/事務機相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2390.TW': _v238_stock('云辰', '電子零組件', '電子/資產', '電子零組件供應商', '★★★☆☆', '電子/資產相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2397.TW': _v238_stock('友通', '電腦/周邊', '工業電腦', '電腦周邊與系統供應商', '★★★☆☆', '工業電腦相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2399.TW': _v238_stock('映泰', '電腦/周邊', '主機板', '電腦周邊與系統供應商', '★★★☆☆', '主機板相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2402.TW': _v238_stock('毅嘉', '電子零組件', '軟板/機構', '電子零組件供應商', '★★★☆☆', '軟板/機構相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2405.TW': _v238_stock('輔信', '電腦/周邊', 'PC/IPC', '電腦周邊與系統供應商', '★★★☆☆', 'PC/IPC相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2406.TW': _v238_stock('國碩', '光電/面板', '太陽能/材料', '光電顯示供應商', '★★★☆☆', '太陽能/材料相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '2413.TW': _v238_stock('環科', '電子零組件', '電子零組件', '電子零組件供應商', '★★★☆☆', '電子零組件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2414.TW': _v238_stock('精技', '電子通路', '資訊通路', '電子零組件供應商', '★★★☆☆', '資訊通路相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2415.TW': _v238_stock('錩新', '汽車/車用', '車用零組件', '汽車車用供應商', '★★★☆☆', '車用零組件相關供應商；汽車、車用零組件、車電與售後市場供應商', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', '車用電子/EV間接受惠'),
    '2417.TW': _v238_stock('圓剛', '電腦/周邊', '影像擷取', '電腦周邊與系統供應商', '★★★☆☆', '影像擷取相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2419.TW': _v238_stock('仲琦', '通訊/網通', '網通設備', '網通與通訊設備供應商', '★★★☆☆', '網通設備相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '2421.TW': _v238_stock('建準', '電子零組件', '散熱風扇', '電子零組件供應商', '★★★☆☆', '散熱風扇相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2423.TW': _v238_stock('固緯', '電子零組件', '量測儀器', '電子零組件供應商', '★★★☆☆', '量測儀器相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2424.TW': _v238_stock('隴華', '其他傳產', '數位內容', '傳統產業公司', '★★★☆☆', '數位內容相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '2425.TW': _v238_stock('承啟', '電腦/周邊', '板卡', '電腦周邊與系統供應商', '★★★☆☆', '板卡相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2426.TW': _v238_stock('鼎元', '光電/面板', 'LED/感測', '光電顯示供應商', '★★★☆☆', 'LED/感測相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '2427.TW': _v238_stock('三商電', '資訊服務', '系統整合', '電腦周邊與系統供應商', '★★★☆☆', '系統整合相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2428.TW': _v238_stock('興勤', '電子零組件', '保護元件', '電子零組件供應商', '★★★☆☆', '保護元件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2430.TW': _v238_stock('燦坤', '電子通路', '3C通路', '電子零組件供應商', '★★★☆☆', '3C通路相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2431.TW': _v238_stock('聯昌', '電子零組件', '電子零組件', '電子零組件供應商', '★★★☆☆', '電子零組件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2433.TW': _v238_stock('互盛電', '其他傳產', '事務機通路', '傳統產業公司', '★★★☆☆', '事務機通路相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '2436.TW': _v238_stock('偉詮電', '半導體', 'IC設計/MCU', '半導體供應鏈公司', '★★★★☆', 'IC設計/MCU相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '2439.TW': _v238_stock('美律', '電子零組件', '聲學元件', '電子零組件供應商', '★★★☆☆', '聲學元件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2440.TW': _v238_stock('太空梭', '電子零組件', '線材', '電子零組件供應商', '★★★☆☆', '線材相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2442.TW': _v238_stock('新美齊', '營建/資產', '建設/資產', '建設營造公司', '★★★☆☆', '建設/資產相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2450.TW': _v238_stock('神腦', '電子通路', '通訊通路', '電子零組件供應商', '★★★☆☆', '通訊通路相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2453.TW': _v238_stock('凌群', '資訊服務', '系統整合', '電腦周邊與系統供應商', '★★★☆☆', '系統整合相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2459.TW': _v238_stock('敦吉', '電子通路', '電子通路/檢測', '電子零組件供應商', '★★★☆☆', '電子通路/檢測相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2460.TW': _v238_stock('建通', '電子零組件', '連接器', '電子零組件供應商', '★★★☆☆', '連接器相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2461.TW': _v238_stock('光群雷', '電子零組件', '雷射/標籤', '電子零組件供應商', '★★★☆☆', '雷射/標籤相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2462.TW': _v238_stock('良得電', '電子零組件', '電源線', '電子零組件供應商', '★★★☆☆', '電源線相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2465.TW': _v238_stock('麗臺', '電腦/周邊', '顯卡/AI工作站', '電腦周邊與系統供應商', '★★★☆☆', '顯卡/AI工作站相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2466.TW': _v238_stock('冠西電', '電子零組件', '電子零組件', '電子零組件供應商', '★★★☆☆', '電子零組件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2467.TW': _v238_stock('志聖', '半導體', '設備/PCB設備', '半導體供應鏈公司', '★★★★☆', '設備/PCB設備相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '2468.TW': _v238_stock('華經', '資訊服務', '系統整合', '電腦周邊與系統供應商', '★★★☆☆', '系統整合相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2471.TW': _v238_stock('資通', '資訊服務', '軟體服務', '電腦周邊與系統供應商', '★★★☆☆', '軟體服務相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2476.TW': _v238_stock('鉅祥', '電子零組件', '沖壓件', '電子零組件供應商', '★★★☆☆', '沖壓件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2477.TW': _v238_stock('美隆電', '電子零組件', '音響/電子', '電子零組件供應商', '★★★☆☆', '音響/電子相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2478.TW': _v238_stock('大毅', '被動元件', '電阻', '電子零組件供應商', '★★★☆☆', '電阻相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2480.TW': _v238_stock('敦陽科', '資訊服務', '系統整合', '電腦周邊與系統供應商', '★★★☆☆', '系統整合相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2481.TW': _v238_stock('強茂', '半導體', '二極體/功率元件', '半導體供應鏈公司', '★★★★☆', '二極體/功率元件相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '2482.TW': _v238_stock('連宇', '資訊服務', '系統整合', '電腦周邊與系統供應商', '★★★☆☆', '系統整合相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2483.TW': _v238_stock('百容', '電子零組件', '開關/連接器', '電子零組件供應商', '★★★☆☆', '開關/連接器相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2484.TW': _v238_stock('希華', '電子零組件', '石英元件', '電子零組件供應商', '★★★☆☆', '石英元件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2485.TW': _v238_stock('兆赫', '通訊/網通', '網通/通訊', '網通與通訊設備供應商', '★★★☆☆', '網通/通訊相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '2488.TW': _v238_stock('漢平', '電子零組件', '音響設備', '電子零組件供應商', '★★★☆☆', '音響設備相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '2489.TW': _v238_stock('瑞軒', '光電/面板', '顯示器/電視', '光電顯示供應商', '★★★☆☆', '顯示器/電視相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '2493.TW': _v238_stock('揚博', '半導體', '設備代理', '半導體供應鏈公司', '★★★★☆', '設備代理相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '2495.TW': _v238_stock('普安', '電腦/周邊', '儲存設備', '電腦周邊與系統供應商', '★★★☆☆', '儲存設備相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '2497.TW': _v238_stock('怡利電', '汽車/車用', '車用電子', '汽車車用供應商', '★★★☆☆', '車用電子相關供應商；汽車、車用零組件、車電與售後市場供應商', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', '車用電子/EV間接受惠'),
    '2505.TW': _v238_stock('國揚', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2506.TW': _v238_stock('太設', '營建/資產', '資產/建設', '建設營造公司', '★★★☆☆', '資產/建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2509.TW': _v238_stock('全坤建', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2514.TW': _v238_stock('龍邦', '營建/資產', '資產/投資', '建設營造公司', '★★★☆☆', '資產/投資相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2516.TW': _v238_stock('新建', '營建/資產', '營造工程', '建設營造公司', '★★★☆☆', '營造工程相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2524.TW': _v238_stock('京城', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2527.TW': _v238_stock('宏璟', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2528.TW': _v238_stock('皇普', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2530.TW': _v238_stock('華建', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2534.TW': _v238_stock('宏盛', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2536.TW': _v238_stock('宏普', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2537.TW': _v238_stock('聯上發', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2538.TW': _v238_stock('基泰', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2540.TW': _v238_stock('愛山林', '營建/資產', '代銷/建設', '建設營造公司', '★★★☆☆', '代銷/建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2543.TW': _v238_stock('皇昌', '營建/資產', '營造工程', '建設營造公司', '★★★☆☆', '營造工程相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2545.TW': _v238_stock('皇翔', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2546.TW': _v238_stock('根基', '營建/資產', '營造工程', '建設營造公司', '★★★☆☆', '營造工程相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2547.TW': _v238_stock('日勝生', '營建/資產', '建設/資產', '建設營造公司', '★★★☆☆', '建設/資產相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2597.TW': _v238_stock('潤弘', '營建/資產', '營造工程', '建設營造公司', '★★★☆☆', '營造工程相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2601.TW': _v238_stock('益航', '航運/物流', '散裝航運/資產', '運輸物流服務商', '★★★☆☆', '散裝航運/資產相關供應商；海運、空運、港口、物流與貨代服務商', '長榮、陽明、華航、物流航運同業', '中：船隊、航線、物流網路與客戶基礎', '運價、油價、匯率、全球景氣與地緣風險', 1.03, '航運、航空、物流、觀光', '運輸服務', '運輸物流服務商', '進出口商、旅客、貨代、物流與跨境供應鏈客戶', 'AI直接關聯較低'),
    '2608.TW': _v238_stock('嘉里大榮', '航運/物流', '物流', '運輸物流服務商', '★★★☆☆', '物流相關供應商；海運、空運、港口、物流與貨代服務商', '長榮、陽明、華航、物流航運同業', '中：船隊、航線、物流網路與客戶基礎', '運價、油價、匯率、全球景氣與地緣風險', 1.03, '航運、航空、物流、觀光', '運輸服務', '運輸物流服務商', '進出口商、旅客、貨代、物流與跨境供應鏈客戶', 'AI直接關聯較低'),
    '2611.TW': _v238_stock('志信', '航運/物流', '貨運物流', '運輸物流服務商', '★★★☆☆', '貨運物流相關供應商；海運、空運、港口、物流與貨代服務商', '長榮、陽明、華航、物流航運同業', '中：船隊、航線、物流網路與客戶基礎', '運價、油價、匯率、全球景氣與地緣風險', 1.03, '航運、航空、物流、觀光', '運輸服務', '運輸物流服務商', '進出口商、旅客、貨代、物流與跨境供應鏈客戶', 'AI直接關聯較低'),
    '2613.TW': _v238_stock('中櫃', '航運/物流', '貨櫃場', '運輸物流服務商', '★★★☆☆', '貨櫃場相關供應商；海運、空運、港口、物流與貨代服務商', '長榮、陽明、華航、物流航運同業', '中：船隊、航線、物流網路與客戶基礎', '運價、油價、匯率、全球景氣與地緣風險', 1.03, '航運、航空、物流、觀光', '運輸服務', '運輸物流服務商', '進出口商、旅客、貨代、物流與跨境供應鏈客戶', 'AI直接關聯較低'),
    '2614.TW': _v238_stock('東森', '其他傳產', '媒體/通路', '傳統產業公司', '★★★☆☆', '媒體/通路相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '2633.TW': _v238_stock('台灣高鐵', '航運/物流', '鐵路運輸', '運輸物流服務商', '★★★☆☆', '鐵路運輸相關供應商；海運、空運、港口、物流與貨代服務商', '長榮、陽明、華航、物流航運同業', '中：船隊、航線、物流網路與客戶基礎', '運價、油價、匯率、全球景氣與地緣風險', 1.03, '航運、航空、物流、觀光', '運輸服務', '運輸物流服務商', '進出口商、旅客、貨代、物流與跨境供應鏈客戶', 'AI直接關聯較低'),
    '2642.TW': _v238_stock('宅配通', '航運/物流', '宅配物流', '運輸物流服務商', '★★★☆☆', '宅配物流相關供應商；海運、空運、港口、物流與貨代服務商', '長榮、陽明、華航、物流航運同業', '中：船隊、航線、物流網路與客戶基礎', '運價、油價、匯率、全球景氣與地緣風險', 1.03, '航運、航空、物流、觀光', '運輸服務', '運輸物流服務商', '進出口商、旅客、貨代、物流與跨境供應鏈客戶', 'AI直接關聯較低'),
    '2701.TW': _v238_stock('萬企', '觀光/餐飲', '飯店/資產', '觀光餐飲服務公司', '★★★☆☆', '飯店/資產相關供應商；飯店、餐飲、休閒娛樂與觀光服務供應商', '王品、晶華、六角、觀光餐飲同業', '中：品牌、展店、服務與地點', '消費景氣、人力、食材成本、觀光需求', 1.03, '觀光、餐飲、內需消費', '下游服務業', '觀光餐飲服務商', '一般消費者、旅客、商務客與餐飲通路客戶', 'AI直接關聯較低'),
    '2702.TW': _v238_stock('華園', '觀光/餐飲', '飯店', '觀光餐飲服務公司', '★★★☆☆', '飯店相關供應商；飯店、餐飲、休閒娛樂與觀光服務供應商', '王品、晶華、六角、觀光餐飲同業', '中：品牌、展店、服務與地點', '消費景氣、人力、食材成本、觀光需求', 1.03, '觀光、餐飲、內需消費', '下游服務業', '觀光餐飲服務商', '一般消費者、旅客、商務客與餐飲通路客戶', 'AI直接關聯較低'),
    '2705.TW': _v238_stock('六福', '觀光/餐飲', '飯店/遊樂園', '觀光餐飲服務公司', '★★★☆☆', '飯店/遊樂園相關供應商；飯店、餐飲、休閒娛樂與觀光服務供應商', '王品、晶華、六角、觀光餐飲同業', '中：品牌、展店、服務與地點', '消費景氣、人力、食材成本、觀光需求', 1.03, '觀光、餐飲、內需消費', '下游服務業', '觀光餐飲服務商', '一般消費者、旅客、商務客與餐飲通路客戶', 'AI直接關聯較低'),
    '2706.TW': _v238_stock('第一店', '觀光/餐飲', '飯店/資產', '觀光餐飲服務公司', '★★★☆☆', '飯店/資產相關供應商；飯店、餐飲、休閒娛樂與觀光服務供應商', '王品、晶華、六角、觀光餐飲同業', '中：品牌、展店、服務與地點', '消費景氣、人力、食材成本、觀光需求', 1.03, '觀光、餐飲、內需消費', '下游服務業', '觀光餐飲服務商', '一般消費者、旅客、商務客與餐飲通路客戶', 'AI直接關聯較低'),
    '2712.TW': _v238_stock('遠雄來', '觀光/餐飲', '遊樂園', '觀光餐飲服務公司', '★★★☆☆', '遊樂園相關供應商；飯店、餐飲、休閒娛樂與觀光服務供應商', '王品、晶華、六角、觀光餐飲同業', '中：品牌、展店、服務與地點', '消費景氣、人力、食材成本、觀光需求', 1.03, '觀光、餐飲、內需消費', '下游服務業', '觀光餐飲服務商', '一般消費者、旅客、商務客與餐飲通路客戶', 'AI直接關聯較低'),
    '2722.TW': _v238_stock('夏都', '觀光/餐飲', '飯店', '觀光餐飲服務公司', '★★★☆☆', '飯店相關供應商；飯店、餐飲、休閒娛樂與觀光服務供應商', '王品、晶華、六角、觀光餐飲同業', '中：品牌、展店、服務與地點', '消費景氣、人力、食材成本、觀光需求', 1.03, '觀光、餐飲、內需消費', '下游服務業', '觀光餐飲服務商', '一般消費者、旅客、商務客與餐飲通路客戶', 'AI直接關聯較低'),
    '2731.TW': _v238_stock('雄獅', '觀光/餐飲', '旅行社', '觀光餐飲服務公司', '★★★☆☆', '旅行社相關供應商；飯店、餐飲、休閒娛樂與觀光服務供應商', '王品、晶華、六角、觀光餐飲同業', '中：品牌、展店、服務與地點', '消費景氣、人力、食材成本、觀光需求', 1.03, '觀光、餐飲、內需消費', '下游服務業', '觀光餐飲服務商', '一般消費者、旅客、商務客與餐飲通路客戶', 'AI直接關聯較低'),
    '2748.TW': _v238_stock('雲品', '觀光/餐飲', '飯店', '觀光餐飲服務公司', '★★★☆☆', '飯店相關供應商；飯店、餐飲、休閒娛樂與觀光服務供應商', '王品、晶華、六角、觀光餐飲同業', '中：品牌、展店、服務與地點', '消費景氣、人力、食材成本、觀光需求', 1.03, '觀光、餐飲、內需消費', '下游服務業', '觀光餐飲服務商', '一般消費者、旅客、商務客與餐飲通路客戶', 'AI直接關聯較低'),
    '2809.TW': _v238_stock('京城銀', '金融', '銀行', '金融服務公司', '★★★☆☆', '銀行相關供應商；銀行、保險、證券、租賃與金融服務供應商', '富邦金、國泰金、中信金、元大金、金融同業', '中高：通路、資本規模與客戶基礎', '利率、景氣、信用風險、金融市場波動', 1.02, '金融、銀行、保險、證券', '金融服務', '金融服務供應商', '個人、企業、財富管理、證券與保險客戶', 'AI直接關聯較低'),
    '2816.TW': _v238_stock('旺旺保', '金融', '保險', '金融服務公司', '★★★☆☆', '保險相關供應商；銀行、保險、證券、租賃與金融服務供應商', '富邦金、國泰金、中信金、元大金、金融同業', '中高：通路、資本規模與客戶基礎', '利率、景氣、信用風險、金融市場波動', 1.02, '金融、銀行、保險、證券', '金融服務', '金融服務供應商', '個人、企業、財富管理、證券與保險客戶', 'AI直接關聯較低'),
    '2820.TW': _v238_stock('華票', '金融', '票券金融', '金融服務公司', '★★★☆☆', '票券金融相關供應商；銀行、保險、證券、租賃與金融服務供應商', '富邦金、國泰金、中信金、元大金、金融同業', '中高：通路、資本規模與客戶基礎', '利率、景氣、信用風險、金融市場波動', 1.02, '金融、銀行、保險、證券', '金融服務', '金融服務供應商', '個人、企業、財富管理、證券與保險客戶', 'AI直接關聯較低'),
    '2832.TW': _v238_stock('台產', '金融', '產險', '金融服務公司', '★★★☆☆', '產險相關供應商；銀行、保險、證券、租賃與金融服務供應商', '富邦金、國泰金、中信金、元大金、金融同業', '中高：通路、資本規模與客戶基礎', '利率、景氣、信用風險、金融市場波動', 1.02, '金融、銀行、保險、證券', '金融服務', '金融服務供應商', '個人、企業、財富管理、證券與保險客戶', 'AI直接關聯較低'),
    '2836.TW': _v238_stock('高雄銀', '金融', '銀行', '金融服務公司', '★★★☆☆', '銀行相關供應商；銀行、保險、證券、租賃與金融服務供應商', '富邦金、國泰金、中信金、元大金、金融同業', '中高：通路、資本規模與客戶基礎', '利率、景氣、信用風險、金融市場波動', 1.02, '金融、銀行、保險、證券', '金融服務', '金融服務供應商', '個人、企業、財富管理、證券與保險客戶', 'AI直接關聯較低'),
    '2838.TW': _v238_stock('聯邦銀', '金融', '銀行', '金融服務公司', '★★★☆☆', '銀行相關供應商；銀行、保險、證券、租賃與金融服務供應商', '富邦金、國泰金、中信金、元大金、金融同業', '中高：通路、資本規模與客戶基礎', '利率、景氣、信用風險、金融市場波動', 1.02, '金融、銀行、保險、證券', '金融服務', '金融服務供應商', '個人、企業、財富管理、證券與保險客戶', 'AI直接關聯較低'),
    '2849.TW': _v238_stock('安泰銀', '金融', '銀行', '金融服務公司', '★★★☆☆', '銀行相關供應商；銀行、保險、證券、租賃與金融服務供應商', '富邦金、國泰金、中信金、元大金、金融同業', '中高：通路、資本規模與客戶基礎', '利率、景氣、信用風險、金融市場波動', 1.02, '金融、銀行、保險、證券', '金融服務', '金融服務供應商', '個人、企業、財富管理、證券與保險客戶', 'AI直接關聯較低'),
    '2850.TW': _v238_stock('新產', '金融', '產險', '金融服務公司', '★★★☆☆', '產險相關供應商；銀行、保險、證券、租賃與金融服務供應商', '富邦金、國泰金、中信金、元大金、金融同業', '中高：通路、資本規模與客戶基礎', '利率、景氣、信用風險、金融市場波動', 1.02, '金融、銀行、保險、證券', '金融服務', '金融服務供應商', '個人、企業、財富管理、證券與保險客戶', 'AI直接關聯較低'),
    '2851.TW': _v238_stock('中再保', '金融', '再保險', '金融服務公司', '★★★☆☆', '再保險相關供應商；銀行、保險、證券、租賃與金融服務供應商', '富邦金、國泰金、中信金、元大金、金融同業', '中高：通路、資本規模與客戶基礎', '利率、景氣、信用風險、金融市場波動', 1.02, '金融、銀行、保險、證券', '金融服務', '金融服務供應商', '個人、企業、財富管理、證券與保險客戶', 'AI直接關聯較低'),
    '2852.TW': _v238_stock('第一保', '金融', '產險', '金融服務公司', '★★★☆☆', '產險相關供應商；銀行、保險、證券、租賃與金融服務供應商', '富邦金、國泰金、中信金、元大金、金融同業', '中高：通路、資本規模與客戶基礎', '利率、景氣、信用風險、金融市場波動', 1.02, '金融、銀行、保險、證券', '金融服務', '金融服務供應商', '個人、企業、財富管理、證券與保險客戶', 'AI直接關聯較低'),
    '2867.TW': _v238_stock('三商壽', '金融', '壽險', '金融服務公司', '★★★☆☆', '壽險相關供應商；銀行、保險、證券、租賃與金融服務供應商', '富邦金、國泰金、中信金、元大金、金融同業', '中高：通路、資本規模與客戶基礎', '利率、景氣、信用風險、金融市場波動', 1.02, '金融、銀行、保險、證券', '金融服務', '金融服務供應商', '個人、企業、財富管理、證券與保險客戶', 'AI直接關聯較低'),
    '2901.TW': _v238_stock('欣欣', '百貨/通路', '百貨', '傳統產業公司', '★★★☆☆', '百貨相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '2903.TW': _v238_stock('遠百', '百貨/通路', '百貨', '傳統產業公司', '★★★☆☆', '百貨相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '2904.TW': _v238_stock('匯僑', '百貨/通路', '資產/通路', '傳統產業公司', '★★★☆☆', '資產/通路相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '2905.TW': _v238_stock('三商', '百貨/通路', '零售/餐飲', '傳統產業公司', '★★★☆☆', '零售/餐飲相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '2906.TW': _v238_stock('高林', '百貨/通路', '貿易/資產', '傳統產業公司', '★★★☆☆', '貿易/資產相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '2908.TW': _v238_stock('特力', '百貨/通路', '居家通路', '傳統產業公司', '★★★☆☆', '居家通路相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '2910.TW': _v238_stock('統領', '百貨/通路', '百貨/資產', '傳統產業公司', '★★★☆☆', '百貨/資產相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '2911.TW': _v238_stock('麗嬰房', '百貨/通路', '童裝通路', '傳統產業公司', '★★★☆☆', '童裝通路相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '2923.TW': _v238_stock('鼎固-KY', '營建/資產', '不動產', '建設營造公司', '★★★☆☆', '不動產相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '2929.TW': _v238_stock('淘帝-KY', '百貨/通路', '童裝', '傳統產業公司', '★★★☆☆', '童裝相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '2939.TW': _v238_stock('凱羿-KY', '百貨/通路', '貿易', '傳統產業公司', '★★★☆☆', '貿易相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '3002.TW': _v238_stock('歐格', '電腦/周邊', '電腦周邊', '電腦周邊與系統供應商', '★★★☆☆', '電腦周邊相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '3003.TW': _v238_stock('健和興', '電子零組件', '端子/連接器', '電子零組件供應商', '★★★☆☆', '端子/連接器相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3011.TW': _v238_stock('今皓', '電子零組件', '線材/連接器', '電子零組件供應商', '★★★☆☆', '線材/連接器相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3014.TW': _v238_stock('聯陽', '半導體', 'IO控制IC', '半導體供應鏈公司', '★★★★☆', 'IO控制IC相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '3015.TW': _v238_stock('全漢', '電源管理', '電源供應器', '電子零組件供應商', '★★★☆☆', '電源供應器相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3016.TW': _v238_stock('嘉晶', '半導體', '矽磊晶', '半導體供應鏈公司', '★★★★☆', '矽磊晶相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '3021.TW': _v238_stock('鴻名', '電子零組件', '連接器', '電子零組件供應商', '★★★☆☆', '連接器相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3022.TW': _v238_stock('威強電', '電腦/周邊', '工業電腦', '電腦周邊與系統供應商', '★★★☆☆', '工業電腦相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '3024.TW': _v238_stock('憶聲', '汽車/車用', '車用影音', '汽車車用供應商', '★★★☆☆', '車用影音相關供應商；汽車、車用零組件、車電與售後市場供應商', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', '車用電子/EV間接受惠'),
    '3025.TW': _v238_stock('星通', '通訊/網通', '通訊設備', '網通與通訊設備供應商', '★★★☆☆', '通訊設備相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '3027.TW': _v238_stock('盛達', '通訊/網通', '網通設備', '網通與通訊設備供應商', '★★★☆☆', '網通設備相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '3028.TW': _v238_stock('增你強', '電子通路', 'IC通路', '電子零組件供應商', '★★★☆☆', 'IC通路相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3029.TW': _v238_stock('零壹', '資訊服務', '系統整合', '電腦周邊與系統供應商', '★★★☆☆', '系統整合相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '3030.TW': _v238_stock('德律', '半導體', '檢測設備', '半導體供應鏈公司', '★★★★☆', '檢測設備相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '3031.TW': _v238_stock('佰鴻', '光電/面板', 'LED', '光電顯示供應商', '★★★☆☆', 'LED相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '3032.TW': _v238_stock('偉訓', '電源管理', '電源/機殼', '電子零組件供應商', '★★★☆☆', '電源/機殼相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3033.TW': _v238_stock('威健', '電子通路', 'IC通路', '電子零組件供應商', '★★★☆☆', 'IC通路相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3038.TW': _v238_stock('全台', '光電/面板', '背光模組', '光電顯示供應商', '★★★☆☆', '背光模組相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '3041.TW': _v238_stock('揚智', '半導體', 'IC設計', '半導體供應鏈公司', '★★★★☆', 'IC設計相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '3042.TW': _v238_stock('晶技', '電子零組件', '石英元件', '電子零組件供應商', '★★★☆☆', '石英元件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3043.TW': _v238_stock('科風', '電源管理', 'UPS', '電子零組件供應商', '★★★☆☆', 'UPS相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3046.TW': _v238_stock('建碁', '電腦/周邊', '工業電腦', '電腦周邊與系統供應商', '★★★☆☆', '工業電腦相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '3047.TW': _v238_stock('訊舟', '通訊/網通', '網通設備', '網通與通訊設備供應商', '★★★☆☆', '網通設備相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '3050.TW': _v238_stock('鈺德', '光電/面板', '光碟/儲存', '光電顯示供應商', '★★★☆☆', '光碟/儲存相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '3051.TW': _v238_stock('力特', '光電/面板', '偏光板', '光電顯示供應商', '★★★☆☆', '偏光板相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '3052.TW': _v238_stock('夆典', '營建/資產', '營建/機電', '建設營造公司', '★★★☆☆', '營建/機電相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '3055.TW': _v238_stock('蔚華科', '半導體', '半導體設備通路', '半導體供應鏈公司', '★★★★☆', '半導體設備通路相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '3056.TW': _v238_stock('總太', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '3057.TW': _v238_stock('喬鼎', '電腦/周邊', '儲存設備', '電腦周邊與系統供應商', '★★★☆☆', '儲存設備相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '3058.TW': _v238_stock('立德', '電源管理', '電源供應器', '電子零組件供應商', '★★★☆☆', '電源供應器相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3060.TW': _v238_stock('銘異', '電腦/周邊', '硬碟零組件', '電腦周邊與系統供應商', '★★★☆☆', '硬碟零組件相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '3062.TW': _v238_stock('建漢', '通訊/網通', '網通設備', '網通與通訊設備供應商', '★★★☆☆', '網通設備相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '3090.TW': _v238_stock('日電貿', '電子通路', '被動元件通路', '電子零組件供應商', '★★★☆☆', '被動元件通路相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3092.TW': _v238_stock('鴻碩', '電子零組件', '線材', '電子零組件供應商', '★★★☆☆', '線材相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3094.TW': _v238_stock('聯傑', '半導體', 'IC設計', '半導體供應鏈公司', '★★★★☆', 'IC設計相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '3130.TW': _v238_stock('一零四', '資訊服務', '人力銀行', '電腦周邊與系統供應商', '★★★☆☆', '人力銀行相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '3149.TW': _v238_stock('正達', '光電/面板', '玻璃加工', '光電顯示供應商', '★★★☆☆', '玻璃加工相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '3164.TW': _v238_stock('景岳', '生技醫療', '益生菌/保健', '生技醫療公司', '★★★☆☆', '益生菌/保健相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '3167.TW': _v238_stock('大量', '半導體', 'PCB/半導體設備', '半導體供應鏈公司', '★★★★☆', 'PCB/半導體設備相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '3209.TW': _v238_stock('全科', '電子通路', '通訊通路', '電子零組件供應商', '★★★☆☆', '通訊通路相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3229.TW': _v238_stock('晟鈦', 'PCB', 'PCB', '電子零組件供應商', '★★★☆☆', 'PCB相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3230.TW': _v238_stock('錦明', '電子零組件', '機構件', '電子零組件供應商', '★★★☆☆', '機構件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3234.TW': _v238_stock('光環', '通訊/網通', '光通訊', '網通與通訊設備供應商', '★★★☆☆', '光通訊相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '3296.TW': _v238_stock('勝德', '電子零組件', '連接器', '電子零組件供應商', '★★★☆☆', '連接器相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3305.TW': _v238_stock('昇貿', '電子零組件', '焊錫材料', '電子零組件供應商', '★★★☆☆', '焊錫材料相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3308.TW': _v238_stock('聯德', '電子零組件', '電源/電子', '電子零組件供應商', '★★★☆☆', '電源/電子相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3311.TW': _v238_stock('閎暉', '電子零組件', '機構件', '電子零組件供應商', '★★★☆☆', '機構件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3312.TW': _v238_stock('弘憶股', '電子通路', 'IC通路', '電子零組件供應商', '★★★☆☆', 'IC通路相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3321.TW': _v238_stock('同泰', 'PCB', '軟板', '電子零組件供應商', '★★★☆☆', '軟板相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3338.TW': _v238_stock('泰碩', '電子零組件', '散熱/連接器', '電子零組件供應商', '★★★☆☆', '散熱/連接器相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3346.TW': _v238_stock('麗清', '汽車/車用', '車燈模組', '汽車車用供應商', '★★★☆☆', '車燈模組相關供應商；汽車、車用零組件、車電與售後市場供應商', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', '車用電子/EV間接受惠'),
    '3356.TW': _v238_stock('奇偶', '電腦/周邊', '安防監控', '電腦周邊與系統供應商', '★★★☆☆', '安防監控相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '3376.TW': _v238_stock('新日興', '電子零組件', '軸承/鉸鏈', '電子零組件供應商', '★★★☆☆', '軸承/鉸鏈相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3380.TW': _v238_stock('明泰', '通訊/網通', '網通設備', '網通與通訊設備供應商', '★★★☆☆', '網通設備相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '3413.TW': _v238_stock('京鼎', '半導體', '半導體設備', '半導體供應鏈公司', '★★★★☆', '半導體設備相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '3416.TW': _v238_stock('融程電', '電腦/周邊', '工業電腦', '電腦周邊與系統供應商', '★★★☆☆', '工業電腦相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '3419.TW': _v238_stock('譁裕', '通訊/網通', '天線', '網通與通訊設備供應商', '★★★☆☆', '天線相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '3437.TW': _v238_stock('榮創', '光電/面板', 'LED', '光電顯示供應商', '★★★☆☆', 'LED相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '3454.TW': _v238_stock('晶睿', '電腦/周邊', '安防監控', '電腦周邊與系統供應商', '★★★☆☆', '安防監控相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '3501.TW': _v238_stock('維熹', '電子零組件', '線材', '電子零組件供應商', '★★★☆☆', '線材相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3508.TW': _v238_stock('位速', '電子零組件', '機構件', '電子零組件供應商', '★★★☆☆', '機構件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3515.TW': _v238_stock('華擎', '電腦/周邊', '主機板/伺服器', '電腦周邊與系統供應商', '★★★☆☆', '主機板/伺服器相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '3518.TW': _v238_stock('柏騰', '電子零組件', '表面處理', '電子零組件供應商', '★★★☆☆', '表面處理相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3528.TW': _v238_stock('安馳', '電子通路', 'IC通路', '電子零組件供應商', '★★★☆☆', 'IC通路相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3530.TW': _v238_stock('晶相光', '光電/面板', '影像感測IC', '光電顯示供應商', '★★★☆☆', '影像感測IC相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '3532.TW': _v238_stock('台勝科', '半導體', '矽晶圓', '半導體供應鏈公司', '★★★★☆', '矽晶圓相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '3535.TW': _v238_stock('晶彩科', '光電/面板', '面板設備', '光電顯示供應商', '★★★☆☆', '面板設備相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '3550.TW': _v238_stock('聯穎', '電子零組件', '線材', '電子零組件供應商', '★★★☆☆', '線材相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3557.TW': _v238_stock('嘉威', '其他傳產', '餐廚用品', '傳統產業公司', '★★★☆☆', '餐廚用品相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '3563.TW': _v238_stock('牧德', '半導體', 'AOI檢測', '半導體供應鏈公司', '★★★★☆', 'AOI檢測相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '3588.TW': _v238_stock('通嘉', '半導體', '電源管理IC', '半導體供應鏈公司', '★★★★☆', '電源管理IC相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '3592.TW': _v238_stock('瑞鼎', '半導體', 'DDI', '半導體供應鏈公司', '★★★★☆', 'DDI相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '3607.TW': _v238_stock('谷崧', '電子零組件', '機構件', '電子零組件供應商', '★★★☆☆', '機構件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3617.TW': _v238_stock('碩天', '電源管理', 'UPS', '電子零組件供應商', '★★★☆☆', 'UPS相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3645.TW': _v238_stock('達邁', '電子材料', 'PI薄膜', '電子零組件供應商', '★★★☆☆', 'PI薄膜相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3652.TW': _v238_stock('精聯', '電腦/周邊', '資料蒐集設備', '電腦周邊與系統供應商', '★★★☆☆', '資料蒐集設備相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '3673.TW': _v238_stock('TPK-KY', '光電/面板', '觸控面板', '光電顯示供應商', '★★★☆☆', '觸控面板相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '3679.TW': _v238_stock('新至陞', '電子零組件', '機構件', '電子零組件供應商', '★★★☆☆', '機構件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '3694.TW': _v238_stock('海華', '通訊/網通', '無線模組', '網通與通訊設備供應商', '★★★☆☆', '無線模組相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '3701.TW': _v238_stock('大眾控', '電腦/周邊', '電腦/投控', '電腦周邊與系統供應商', '★★★☆☆', '電腦/投控相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '3704.TW': _v238_stock('合勤控', '通訊/網通', '網通設備', '網通與通訊設備供應商', '★★★☆☆', '網通設備相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '3712.TW': _v238_stock('永崴投控', '電力/重電', '能源/代工', '電力設備供應商', '★★★★☆', '能源/代工相關供應商；重電、電網、電機、電線電纜與電力工程供應商', '華城、中興電、士電、亞力、ABB、Siemens', '中高：認證、工程案源與電網需求', '政策節奏、交期、原物料、專案毛利', 1.06, '重電、電網、能源、資料中心電力', '電力設備產業鏈', '電力設備供應商', '台電、科技廠、公共工程、資料中心與工業用電客戶', 'AI資料中心用電需求間接受惠'),
    '4104.TW': _v238_stock('佳醫', '生技醫療', '醫療通路', '生技醫療公司', '★★★☆☆', '醫療通路相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '4106.TW': _v238_stock('雃博', '生技醫療', '醫材', '生技醫療公司', '★★★☆☆', '醫材相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '4108.TW': _v238_stock('懷特', '生技醫療', '新藥', '生技醫療公司', '★★★☆☆', '新藥相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '4119.TW': _v238_stock('旭富', '生技醫療', '原料藥', '生技醫療公司', '★★★☆☆', '原料藥相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '4133.TW': _v238_stock('亞諾法', '生技醫療', '檢測試劑', '生技醫療公司', '★★★☆☆', '檢測試劑相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '4137.TW': _v238_stock('麗豐-KY', '生技醫療', '醫美保養', '生技醫療公司', '★★★☆☆', '醫美保養相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '4142.TW': _v238_stock('國光生', '生技醫療', '疫苗', '生技醫療公司', '★★★☆☆', '疫苗相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '4155.TW': _v238_stock('訊映', '生技醫療', '醫材/檢測', '生技醫療公司', '★★★☆☆', '醫材/檢測相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '4164.TW': _v238_stock('承業醫', '生技醫療', '醫療設備', '生技醫療公司', '★★★☆☆', '醫療設備相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '4190.TW': _v238_stock('佐登-KY', '生技醫療', '美容通路', '生技醫療公司', '★★★☆☆', '美容通路相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '4306.TW': _v238_stock('炎洲', '塑化/化工', '膠帶/包材', '傳統產業公司', '★★★☆☆', '膠帶/包材相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '4426.TW': _v238_stock('利勤', '紡織/成衣', '不織布', '傳統產業公司', '★★★☆☆', '不織布相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '4438.TW': _v238_stock('廣越', '紡織/成衣', '羽絨衣代工', '傳統產業公司', '★★★☆☆', '羽絨衣代工相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '4439.TW': _v238_stock('冠星-KY', '紡織/成衣', '成衣代工', '傳統產業公司', '★★★☆☆', '成衣代工相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '4536.TW': _v238_stock('拓凱', '其他傳產', '複合材料', '傳統產業公司', '★★★☆☆', '複合材料相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '4545.TW': _v238_stock('銘鈺', '電子零組件', '金屬沖壓', '電子零組件供應商', '★★★☆☆', '金屬沖壓相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '4552.TW': _v238_stock('力達-KY', '電機/機械', '空壓機', '傳統產業公司', '★★★☆☆', '空壓機相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '4555.TW': _v238_stock('氣立', '自動化/機器人', '氣動元件', '電子零組件供應商', '★★★☆☆', '氣動元件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '4557.TW': _v238_stock('永新-KY', '汽車/車用', '煞車系統', '汽車車用供應商', '★★★☆☆', '煞車系統相關供應商；汽車、車用零組件、車電與售後市場供應商', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', '車用電子/EV間接受惠'),
    '4560.TW': _v238_stock('強信-KY', '電機/機械', '縫紉機零件', '傳統產業公司', '★★★☆☆', '縫紉機零件相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '4562.TW': _v238_stock('穎漢', '電機/機械', '金屬成型設備', '傳統產業公司', '★★★☆☆', '金屬成型設備相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '4564.TW': _v238_stock('元翎', '汽車/車用', '氣瓶/車用', '汽車車用供應商', '★★★☆☆', '氣瓶/車用相關供應商；汽車、車用零組件、車電與售後市場供應商', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', '車用電子/EV間接受惠'),
    '4566.TW': _v238_stock('時碩工業', '汽車/車用', '精密零件', '汽車車用供應商', '★★★☆☆', '精密零件相關供應商；汽車、車用零組件、車電與售後市場供應商', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', '車用電子/EV間接受惠'),
    '4571.TW': _v238_stock('鈞興-KY', '電機/機械', '齒輪', '傳統產業公司', '★★★☆☆', '齒輪相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '4581.TW': _v238_stock('光隆精密-KY', '汽車/車用', '汽車零組件', '汽車車用供應商', '★★★☆☆', '汽車零組件相關供應商；汽車、車用零組件、車電與售後市場供應商', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', '車用電子/EV間接受惠'),
    '4720.TW': _v238_stock('德淵', '化工/材料', '熱熔膠', '傳統產業公司', '★★★☆☆', '熱熔膠相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '4722.TW': _v238_stock('國精化', '化工/材料', '樹脂', '傳統產業公司', '★★★☆☆', '樹脂相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '4737.TW': _v238_stock('華廣', '生技醫療', '血糖檢測', '生技醫療公司', '★★★☆☆', '血糖檢測相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '4755.TW': _v238_stock('三福化', '化工/材料', '特用化學', '傳統產業公司', '★★★☆☆', '特用化學相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '4766.TW': _v238_stock('南寶', '化工/材料', '膠黏劑', '傳統產業公司', '★★★☆☆', '膠黏劑相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '4930.TW': _v238_stock('燦星網', '百貨/通路', '家電通路', '傳統產業公司', '★★★☆☆', '家電通路相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '4934.TW': _v238_stock('太極', '光電/面板', '太陽能', '光電顯示供應商', '★★★☆☆', '太陽能相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '4935.TW': _v238_stock('茂林-KY', '光電/面板', '導光板', '光電顯示供應商', '★★★☆☆', '導光板相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '4942.TW': _v238_stock('嘉彰', '電子零組件', '金屬機構件', '電子零組件供應商', '★★★☆☆', '金屬機構件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '4943.TW': _v238_stock('康控-KY', '電子零組件', '聲學元件', '電子零組件供應商', '★★★☆☆', '聲學元件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '4952.TW': _v238_stock('凌通', '半導體', 'MCU', '半導體供應鏈公司', '★★★★☆', 'MCU相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '4956.TW': _v238_stock('光鋐', '光電/面板', 'LED', '光電顯示供應商', '★★★☆☆', 'LED相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '4960.TW': _v238_stock('誠美材', '光電/面板', '偏光板', '光電顯示供應商', '★★★☆☆', '偏光板相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '4968.TW': _v238_stock('立積', '半導體', '射頻IC', '半導體供應鏈公司', '★★★★☆', '射頻IC相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '4994.TW': _v238_stock('傳奇', '資訊服務', '遊戲', '電腦周邊與系統供應商', '★★★☆☆', '遊戲相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '5007.TW': _v238_stock('三星', '鋼鐵/材料', '螺帽', '傳統產業公司', '★★★☆☆', '螺帽相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '5203.TW': _v238_stock('訊連', '資訊服務', '影音軟體', '電腦周邊與系統供應商', '★★★☆☆', '影音軟體相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '5215.TW': _v238_stock('科嘉-KY', '電子零組件', '薄膜按鍵', '電子零組件供應商', '★★★☆☆', '薄膜按鍵相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '5222.TW': _v238_stock('全訊', '半導體', '砷化鎵/國防', '半導體供應鏈公司', '★★★★☆', '砷化鎵/國防相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '5225.TW': _v238_stock('東科-KY', '電子零組件', '音響ODM', '電子零組件供應商', '★★★☆☆', '音響ODM相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '5234.TW': _v238_stock('達興材料', '電子材料', '顯示/半導體材料', '電子零組件供應商', '★★★☆☆', '顯示/半導體材料相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '5243.TW': _v238_stock('乙盛-KY', '電子零組件', '機構件', '電子零組件供應商', '★★★☆☆', '機構件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '5258.TW': _v238_stock('虹堡', '電腦/周邊', 'POS/支付設備', '電腦周邊與系統供應商', '★★★☆☆', 'POS/支付設備相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '5283.TW': _v238_stock('禾聯碩', '家電/品牌', '家電', '傳統產業公司', '★★★☆☆', '家電相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '5284.TW': _v238_stock('jpp-KY', '航太/軍工', '航太零組件', '傳統產業公司', '★★★☆☆', '航太零組件相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '5285.TW': _v238_stock('界霖', '半導體', '導線架', '半導體供應鏈公司', '★★★★☆', '導線架相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '5288.TW': _v238_stock('豐祥-KY', '汽車/車用', '車用零組件', '汽車車用供應商', '★★★☆☆', '車用零組件相關供應商；汽車、車用零組件、車電與售後市場供應商', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', '車用電子/EV間接受惠'),
    '5306.TW': _v238_stock('桂盟', '汽車/車用', '自行車鏈條', '汽車車用供應商', '★★★☆☆', '自行車鏈條相關供應商；汽車、車用零組件、車電與售後市場供應商', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', '車用電子/EV間接受惠'),
    '5434.TW': _v238_stock('崇越', '半導體', '半導體材料通路', '半導體供應鏈公司', '★★★★☆', '半導體材料通路相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '5484.TW': _v238_stock('慧友', '電腦/周邊', '安防監控', '電腦周邊與系統供應商', '★★★☆☆', '安防監控相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '5515.TW': _v238_stock('建國', '營建/資產', '營造工程', '建設營造公司', '★★★☆☆', '營造工程相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '5519.TW': _v238_stock('隆大', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '5521.TW': _v238_stock('工信', '營建/資產', '營造工程', '建設營造公司', '★★★☆☆', '營造工程相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '5525.TW': _v238_stock('順天', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '5531.TW': _v238_stock('鄉林', '營建/資產', '建設/飯店', '建設營造公司', '★★★☆☆', '建設/飯店相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '5533.TW': _v238_stock('皇鼎', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '5538.TW': _v238_stock('東明-KY', '鋼鐵/材料', '扣件', '傳統產業公司', '★★★☆☆', '扣件相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '5607.TW': _v238_stock('遠雄港', '航運/物流', '航空貨運園區', '運輸物流服務商', '★★★☆☆', '航空貨運園區相關供應商；海運、空運、港口、物流與貨代服務商', '長榮、陽明、華航、物流航運同業', '中：船隊、航線、物流網路與客戶基礎', '運價、油價、匯率、全球景氣與地緣風險', 1.03, '航運、航空、物流、觀光', '運輸服務', '運輸物流服務商', '進出口商、旅客、貨代、物流與跨境供應鏈客戶', 'AI直接關聯較低'),
    '5608.TW': _v238_stock('四維航', '航運/物流', '散裝航運', '運輸物流服務商', '★★★☆☆', '散裝航運相關供應商；海運、空運、港口、物流與貨代服務商', '長榮、陽明、華航、物流航運同業', '中：船隊、航線、物流網路與客戶基礎', '運價、油價、匯率、全球景氣與地緣風險', 1.03, '航運、航空、物流、觀光', '運輸服務', '運輸物流服務商', '進出口商、旅客、貨代、物流與跨境供應鏈客戶', 'AI直接關聯較低'),
    '5706.TW': _v238_stock('鳳凰', '觀光/餐飲', '旅行社', '觀光餐飲服務公司', '★★★☆☆', '旅行社相關供應商；飯店、餐飲、休閒娛樂與觀光服務供應商', '王品、晶華、六角、觀光餐飲同業', '中：品牌、展店、服務與地點', '消費景氣、人力、食材成本、觀光需求', 1.03, '觀光、餐飲、內需消費', '下游服務業', '觀光餐飲服務商', '一般消費者、旅客、商務客與餐飲通路客戶', 'AI直接關聯較低'),
    '5906.TW': _v238_stock('台南-KY', '百貨/通路', '服飾通路', '傳統產業公司', '★★★☆☆', '服飾通路相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '5907.TW': _v238_stock('大洋-KY', '百貨/通路', '百貨', '傳統產業公司', '★★★☆☆', '百貨相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '6024.TW': _v238_stock('群益期', '金融', '期貨', '金融服務公司', '★★★☆☆', '期貨相關供應商；銀行、保險、證券、租賃與金融服務供應商', '富邦金、國泰金、中信金、元大金、金融同業', '中高：通路、資本規模與客戶基礎', '利率、景氣、信用風險、金融市場波動', 1.02, '金融、銀行、保險、證券', '金融服務', '金融服務供應商', '個人、企業、財富管理、證券與保險客戶', 'AI直接關聯較低'),
    '6108.TW': _v238_stock('競國', 'PCB', 'PCB', '電子零組件供應商', '★★★☆☆', 'PCB相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6112.TW': _v238_stock('邁達特', '資訊服務', '系統整合', '電腦周邊與系統供應商', '★★★☆☆', '系統整合相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '6115.TW': _v238_stock('鎰勝', '電子零組件', '電源線', '電子零組件供應商', '★★★☆☆', '電源線相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6117.TW': _v238_stock('迎廣', '電腦/周邊', '機殼', '電腦周邊與系統供應商', '★★★☆☆', '機殼相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '6120.TW': _v238_stock('達運', '光電/面板', '背光模組', '光電顯示供應商', '★★★☆☆', '背光模組相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '6128.TW': _v238_stock('上福', '電腦/周邊', '事務機耗材', '電腦周邊與系統供應商', '★★★☆☆', '事務機耗材相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '6133.TW': _v238_stock('金橋', '電子零組件', '線材', '電子零組件供應商', '★★★☆☆', '線材相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6136.TW': _v238_stock('富爾特', '電子通路', '通訊通路', '電子零組件供應商', '★★★☆☆', '通訊通路相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6139.TW': _v238_stock('亞翔', '半導體', '無塵室工程', '半導體供應鏈公司', '★★★★☆', '無塵室工程相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '6141.TW': _v238_stock('柏承', 'PCB', 'PCB', '電子零組件供應商', '★★★☆☆', 'PCB相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6152.TW': _v238_stock('百一', '通訊/網通', '射頻/網通', '網通與通訊設備供應商', '★★★☆☆', '射頻/網通相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '6153.TW': _v238_stock('嘉聯益', 'PCB', '軟板', '電子零組件供應商', '★★★☆☆', '軟板相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6155.TW': _v238_stock('鈞寶', '被動元件', '電感', '電子零組件供應商', '★★★☆☆', '電感相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6164.TW': _v238_stock('華興', '光電/面板', 'LED', '光電顯示供應商', '★★★☆☆', 'LED相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '6168.TW': _v238_stock('宏齊', '光電/面板', 'LED', '光電顯示供應商', '★★★☆☆', 'LED相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '6183.TW': _v238_stock('關貿', '資訊服務', '網路服務', '電腦周邊與系統供應商', '★★★☆☆', '網路服務相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '6192.TW': _v238_stock('巨路', '電機/機械', '自動控制', '傳統產業公司', '★★★☆☆', '自動控制相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '6196.TW': _v238_stock('帆宣', '半導體', '廠務工程', '半導體供應鏈公司', '★★★★☆', '廠務工程相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '6201.TW': _v238_stock('亞弘電', '電子零組件', '音響/電子', '電子零組件供應商', '★★★☆☆', '音響/電子相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6205.TW': _v238_stock('詮欣', '電子零組件', '連接器', '電子零組件供應商', '★★★☆☆', '連接器相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6216.TW': _v238_stock('居易', '通訊/網通', '網通設備', '網通與通訊設備供應商', '★★★☆☆', '網通設備相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '6224.TW': _v238_stock('聚鼎', '電子零組件', '保護元件', '電子零組件供應商', '★★★☆☆', '保護元件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6235.TW': _v238_stock('華孚', '電子零組件', '鎂鋁合金', '電子零組件供應商', '★★★☆☆', '鎂鋁合金相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6243.TW': _v238_stock('迅杰', '半導體', 'IC設計', '半導體供應鏈公司', '★★★★☆', 'IC設計相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '6277.TW': _v238_stock('宏正', '電腦/周邊', 'KVM/AV', '電腦周邊與系統供應商', '★★★☆☆', 'KVM/AV相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '6278.TW': _v238_stock('台表科', '電子代工', 'SMT/EMS', '電子零組件供應商', '★★★☆☆', 'SMT/EMS相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6281.TW': _v238_stock('全國電', '百貨/通路', '家電通路', '傳統產業公司', '★★★☆☆', '家電通路相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '6288.TW': _v238_stock('聯嘉', '汽車/車用', '車燈模組', '汽車車用供應商', '★★★☆☆', '車燈模組相關供應商；汽車、車用零組件、車電與售後市場供應商', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', '車用電子/EV間接受惠'),
    '6451.TW': _v238_stock('訊芯-KY', '半導體', 'SiP封裝', '半導體供應鏈公司', '★★★★☆', 'SiP封裝相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '6456.TW': _v238_stock('GIS-KY', '光電/面板', '觸控模組', '光電顯示供應商', '★★★☆☆', '觸控模組相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '6464.TW': _v238_stock('台數科', '電信', '有線電視/寬頻', '網通與通訊設備供應商', '★★★☆☆', '有線電視/寬頻相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '6477.TW': _v238_stock('安集', '光電/面板', '太陽能', '光電顯示供應商', '★★★☆☆', '太陽能相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '6488.TW': _v238_stock('環球晶', '半導體', '矽晶圓', '半導體供應鏈公司', '★★★★☆', '矽晶圓相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.08, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '6491.TW': _v238_stock('晶碩', '生技醫療', '隱形眼鏡', '生技醫療公司', '★★★☆☆', '隱形眼鏡相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '6504.TW': _v238_stock('南六', '其他傳產', '不織布', '傳統產業公司', '★★★☆☆', '不織布相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '6525.TW': _v238_stock('捷敏-KY', '半導體', '封測', '半導體供應鏈公司', '★★★★☆', '封測相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '6541.TW': _v238_stock('泰福-KY', '生技醫療', '生物相似藥', '生技醫療公司', '★★★☆☆', '生物相似藥相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '6552.TW': _v238_stock('易華電', '半導體', 'COF/封裝材料', '半導體供應鏈公司', '★★★★☆', 'COF/封裝材料相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '6558.TW': _v238_stock('興能高', '電池/能源', '電池', '電力設備供應商', '★★★★☆', '電池相關供應商；重電、電網、電機、電線電纜與電力工程供應商', '華城、中興電、士電、亞力、ABB、Siemens', '中高：認證、工程案源與電網需求', '政策節奏、交期、原物料、專案毛利', 1.06, '重電、電網、能源、資料中心電力', '電力設備產業鏈', '電力設備供應商', '台電、科技廠、公共工程、資料中心與工業用電客戶', 'AI資料中心用電需求間接受惠'),
    '6573.TW': _v238_stock('虹揚-KY', '半導體', '整流二極體', '半導體供應鏈公司', '★★★★☆', '整流二極體相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '6581.TW': _v238_stock('鋼聯', '鋼鐵/材料', '資源回收', '傳統產業公司', '★★★☆☆', '資源回收相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '6582.TW': _v238_stock('申豐', '橡膠/材料', '乳膠', '汽車車用供應商', '★★★☆☆', '乳膠相關供應商；汽車、車用零組件、車電與售後市場供應商', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', '車用電子/EV間接受惠'),
    '6591.TW': _v238_stock('動力-KY', '電子零組件', '散熱風扇', '電子零組件供應商', '★★★☆☆', '散熱風扇相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6598.TW': _v238_stock('ABC-KY', '被動元件', '電感', '電子零組件供應商', '★★★☆☆', '電感相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6625.TW': _v238_stock('必應', '文創/娛樂', '展演活動', '傳統產業公司', '★★★☆☆', '展演活動相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '6655.TW': _v238_stock('科定', '建材/家居', '木地板', '傳統產業公司', '★★★☆☆', '木地板相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '6674.TW': _v238_stock('鋐寶科技', '通訊/網通', '網通設備', '網通與通訊設備供應商', '★★★☆☆', '網通設備相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '6689.TW': _v238_stock('伊雲谷', '資訊服務', '雲端服務', '電腦周邊與系統供應商', '★★★☆☆', '雲端服務相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '6691.TW': _v238_stock('洋基工程', '半導體', '無塵室工程', '半導體供應鏈公司', '★★★★☆', '無塵室工程相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '6719.TW': _v238_stock('力智', '半導體', '電源管理IC', '半導體供應鏈公司', '★★★★☆', '電源管理IC相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '6743.TW': _v238_stock('安普新', '電子零組件', '連接器/線材', '電子零組件供應商', '★★★☆☆', '連接器/線材相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '6754.TW': _v238_stock('匯僑設計', '營建/資產', '室內設計', '建設營造公司', '★★★☆☆', '室內設計相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '6782.TW': _v238_stock('視陽', '生技醫療', '隱形眼鏡', '生技醫療公司', '★★★☆☆', '隱形眼鏡相關供應商；醫材、藥品、醫療服務與生技研發公司', '國際藥廠、醫材同業、台灣生技同業', '中：產品認證、研發管線與通路', '臨床、法規、藥證、商業化進度', 1.03, '生技、醫療、藥品、醫材', '醫療產業鏈', '生技醫療供應商', '醫院、藥局、藥廠、醫療通路與病患市場', 'AI醫療間接受惠'),
    '6790.TW': _v238_stock('永豐實', '造紙/包材', '紙品', '傳統產業公司', '★★★☆☆', '紙品相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '6792.TW': _v238_stock('詠業', '通訊/網通', '天線/材料', '網通與通訊設備供應商', '★★★☆☆', '天線/材料相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '6830.TW': _v238_stock('汎銓', '半導體', '材料分析', '半導體供應鏈公司', '★★★★☆', '材料分析相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '6834.TW': _v238_stock('天二科技', '被動元件', '電阻', '電子零組件供應商', '★★★☆☆', '電阻相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '8011.TW': _v238_stock('台通', '通訊/網通', '線纜/通訊', '網通與通訊設備供應商', '★★★☆☆', '線纜/通訊相關供應商；交換器、路由器、通訊模組與網通設備供應商', '智邦、中磊、啟碁、國際網通同業', '中：網通設計、客戶導入與通路', '電信資本支出、客戶集中、價格競爭', 1.05, '網通、資料中心、低軌衛星', '通訊設備產業鏈', '網通通訊供應商', '電信商、雲端資料中心、品牌客戶與企業網路客戶', 'AI資料中心網通間接受惠'),
    '8016.TW': _v238_stock('矽創', '半導體', 'DDI/感測IC', '半導體供應鏈公司', '★★★★☆', 'DDI/感測IC相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '8021.TW': _v238_stock('尖點', 'PCB', '鑽針/PCB耗材', '電子零組件供應商', '★★★☆☆', '鑽針/PCB耗材相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '8028.TW': _v238_stock('昇陽半導體', '半導體', '晶圓再生', '半導體供應鏈公司', '★★★★☆', '晶圓再生相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '8033.TW': _v238_stock('雷虎', '航太/軍工', '無人機/模型', '傳統產業公司', '★★★☆☆', '無人機/模型相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '8039.TW': _v238_stock('台虹', '電子材料', 'FCCL', '電子零組件供應商', '★★★☆☆', 'FCCL相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '8070.TW': _v238_stock('長華*', '電子通路', '半導體材料通路', '電子零組件供應商', '★★★☆☆', '半導體材料通路相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '8072.TW': _v238_stock('陞泰', '電腦/周邊', '安防/電子', '電腦周邊與系統供應商', '★★★☆☆', '安防/電子相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '8081.TW': _v238_stock('致新', '半導體', '電源管理IC', '半導體供應鏈公司', '★★★★☆', '電源管理IC相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '8103.TW': _v238_stock('瀚荃', '電子零組件', '連接器', '電子零組件供應商', '★★★☆☆', '連接器相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '8104.TW': _v238_stock('錸寶', '光電/面板', 'OLED/顯示', '光電顯示供應商', '★★★☆☆', 'OLED/顯示相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '8110.TW': _v238_stock('華東', '半導體', '記憶體封測', '半導體供應鏈公司', '★★★★☆', '記憶體封測相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '8114.TW': _v238_stock('振樺電', '電腦/周邊', 'POS系統', '電腦周邊與系統供應商', '★★★☆☆', 'POS系統相關供應商；PC、IPC、周邊、系統整合與品牌通路供應商', '華碩、宏碁、研華、同業品牌/ODM', '中：品牌、通路、製造與產品組合', 'PC景氣、庫存、匯率與競爭', 1.03, 'AI PC、PC、周邊、IPC', '下游品牌/系統', '電腦周邊供應商', 'PC品牌、企業IT、通路商、工業電腦與一般消費者', 'AI PC/邊緣AI間接受惠'),
    '8131.TW': _v238_stock('福懋科', '半導體', '封測', '半導體供應鏈公司', '★★★★☆', '封測相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '8163.TW': _v238_stock('達方', '電子零組件', '鍵盤/元件', '電子零組件供應商', '★★★☆☆', '鍵盤/元件相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '8213.TW': _v238_stock('志超', 'PCB', 'PCB', '電子零組件供應商', '★★★☆☆', 'PCB相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '8215.TW': _v238_stock('明基材', '電子材料', '偏光板/醫材', '電子零組件供應商', '★★★☆☆', '偏光板/醫材相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '8222.TW': _v238_stock('寶一', '航太/軍工', '航太零組件', '傳統產業公司', '★★★☆☆', '航太零組件相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '8249.TW': _v238_stock('菱光', '光電/面板', '影像感測', '光電顯示供應商', '★★★☆☆', '影像感測相關供應商；面板、LED、光學、顯示與影像感測供應商', '友達、群創、億光、光學/面板同業', '中：製造能力、客戶基礎與產品線', '面板循環、價格競爭、庫存與匯率', 1.02, '光電、面板、顯示器、光學', '光電產業鏈', '光電顯示供應商', '品牌電子、面板廠、照明、車用與消費電子客戶', '車用顯示/AI視覺間接受惠'),
    '8261.TW': _v238_stock('富鼎', '半導體', 'MOSFET', '半導體供應鏈公司', '★★★★☆', 'MOSFET相關供應商；IC設計、封測、材料或半導體設備供應商', '台積電供應鏈、封測/IC設計同業、國際半導體廠', '中高：技術門檻、客戶認證與產業鏈位置', '半導體循環、庫存、資本支出與競爭', 1.06, '半導體、AI、HPC、電子材料', '半導體產業鏈', '半導體供應商', '晶圓廠、封測廠、IC設計、系統廠客戶', 'AI/HPC供應鏈間接受惠'),
    '8341.TW': _v238_stock('日友', '環保/資源', '廢棄物處理', '傳統產業公司', '★★★☆☆', '廢棄物處理相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '8367.TW': _v238_stock('建新國際', '航運/物流', '物流倉儲', '運輸物流服務商', '★★★☆☆', '物流倉儲相關供應商；海運、空運、港口、物流與貨代服務商', '長榮、陽明、華航、物流航運同業', '中：船隊、航線、物流網路與客戶基礎', '運價、油價、匯率、全球景氣與地緣風險', 1.03, '航運、航空、物流、觀光', '運輸服務', '運輸物流服務商', '進出口商、旅客、貨代、物流與跨境供應鏈客戶', 'AI直接關聯較低'),
    '8422.TW': _v238_stock('可寧衛', '環保/資源', '廢棄物處理', '傳統產業公司', '★★★☆☆', '廢棄物處理相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '8438.TW': _v238_stock('昶昕', '環保/資源', '金屬回收', '傳統產業公司', '★★★☆☆', '金屬回收相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '8442.TW': _v238_stock('威宏-KY', '紡織/成衣', '運動包袋', '傳統產業公司', '★★★☆☆', '運動包袋相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '8478.TW': _v238_stock('東哥遊艇', '其他傳產', '遊艇', '傳統產業公司', '★★★☆☆', '遊艇相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '8481.TW': _v238_stock('政伸', '電子零組件', '標籤/印刷', '電子零組件供應商', '★★★☆☆', '標籤/印刷相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '8482.TW': _v238_stock('商億-KY', '其他傳產', '家具', '傳統產業公司', '★★★☆☆', '家具相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '8488.TW': _v238_stock('吉源-KY', '食品/民生', '包材', '傳統產業公司', '★★★☆☆', '包材相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '8499.TW': _v238_stock('鼎炫-KY', '電子材料', 'EMI材料', '電子零組件供應商', '★★★☆☆', 'EMI材料相關供應商；電子零組件、連接器、機構件與材料供應商', '國際電子零組件廠、同業供應商', '中：客戶認證、製程能力與產品線', '終端需求、價格競爭、庫存循環、匯率', 1.04, '電子零組件、AI PC、車用電子', '中游電子零組件', '電子零組件供應商', 'PC、手機、伺服器、車用、工業電子客戶', 'AI PC/伺服器間接受惠'),
    '8926.TW': _v238_stock('台汽電', '電力/重電', '汽電共生', '電力設備供應商', '★★★★☆', '汽電共生相關供應商；重電、電網、電機、電線電纜與電力工程供應商', '華城、中興電、士電、亞力、ABB、Siemens', '中高：認證、工程案源與電網需求', '政策節奏、交期、原物料、專案毛利', 1.06, '重電、電網、能源、資料中心電力', '電力設備產業鏈', '電力設備供應商', '台電、科技廠、公共工程、資料中心與工業用電客戶', 'AI資料中心用電需求間接受惠'),
    '8996.TW': _v238_stock('高力', '電力/重電', '熱交換器', '電力設備供應商', '★★★★☆', '熱交換器相關供應商；重電、電網、電機、電線電纜與電力工程供應商', '華城、中興電、士電、亞力、ABB、Siemens', '中高：認證、工程案源與電網需求', '政策節奏、交期、原物料、專案毛利', 1.06, '重電、電網、能源、資料中心電力', '電力設備產業鏈', '電力設備供應商', '台電、科技廠、公共工程、資料中心與工業用電客戶', 'AI資料中心用電需求間接受惠'),
    '9905.TW': _v238_stock('大華', '食品/民生', '包材', '傳統產業公司', '★★★☆☆', '包材相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '9906.TW': _v238_stock('欣巴巴', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '9907.TW': _v238_stock('統一實', '食品/民生', '包材/飲料', '傳統產業公司', '★★★☆☆', '包材/飲料相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '9908.TW': _v238_stock('大台北', '公用事業', '瓦斯', '傳統產業公司', '★★★☆☆', '瓦斯相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '9911.TW': _v238_stock('櫻花', '建材/家居', '廚具', '傳統產業公司', '★★★☆☆', '廚具相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '9918.TW': _v238_stock('欣天然', '公用事業', '瓦斯', '傳統產業公司', '★★★☆☆', '瓦斯相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '9919.TW': _v238_stock('康那香', '其他傳產', '不織布/衛材', '傳統產業公司', '★★★☆☆', '不織布/衛材相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '9924.TW': _v238_stock('福興', '建材/家居', '門鎖', '傳統產業公司', '★★★☆☆', '門鎖相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '9927.TW': _v238_stock('泰銘', '鋼鐵/材料', '鉛加工', '傳統產業公司', '★★★☆☆', '鉛加工相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '9930.TW': _v238_stock('中聯資源', '環保/資源', '爐石資源', '傳統產業公司', '★★★☆☆', '爐石資源相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '9934.TW': _v238_stock('成霖', '建材/家居', '衛浴五金', '傳統產業公司', '★★★☆☆', '衛浴五金相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '9935.TW': _v238_stock('慶豐富', '建材/家居', '窗簾', '傳統產業公司', '★★★☆☆', '窗簾相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '9937.TW': _v238_stock('全國', '油電燃氣', '加油站', '傳統產業公司', '★★★☆☆', '加油站相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '9942.TW': _v238_stock('茂順', '汽車/車用', '油封', '汽車車用供應商', '★★★☆☆', '油封相關供應商；汽車、車用零組件、車電與售後市場供應商', '和泰車、中華車、車用零組件同業', '中：通路、認證、製造與售後市場', '車市景氣、匯率、原料、電動車競爭', 1.03, '汽車、車用電子、EV', '車用產業鏈', '汽車車用供應商', '車廠、售後市場、零組件通路與一般消費者', '車用電子/EV間接受惠'),
    '9943.TW': _v238_stock('好樂迪', '觀光/餐飲', '休閒娛樂', '觀光餐飲服務公司', '★★★☆☆', '休閒娛樂相關供應商；飯店、餐飲、休閒娛樂與觀光服務供應商', '王品、晶華、六角、觀光餐飲同業', '中：品牌、展店、服務與地點', '消費景氣、人力、食材成本、觀光需求', 1.03, '觀光、餐飲、內需消費', '下游服務業', '觀光餐飲服務商', '一般消費者、旅客、商務客與餐飲通路客戶', 'AI直接關聯較低'),
    '9944.TW': _v238_stock('新麗', '紡織/成衣', '不織布', '傳統產業公司', '★★★☆☆', '不織布相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
    '9946.TW': _v238_stock('三發地產', '營建/資產', '建設', '建設營造公司', '★★★☆☆', '建設相關供應商；建設、營造、資產開發與商辦住宅供應商', '興富發、華固、國建、營建同業', '中：土地庫存、品牌、推案與工程能力', '利率、房市政策、銷售、交屋與原料成本', 1.02, '營建、房市、資產', '下游建設/營造', '營建資產公司', '購屋客戶、建商、公共工程、商辦與土地開發客戶', 'AI直接關聯較低'),
    '9955.TW': _v238_stock('佳龍', '環保/資源', '貴金屬回收', '傳統產業公司', '★★★☆☆', '貴金屬回收相關供應商；品牌、通路、製造與生活消費相關供應商', '同產業競爭者、國際品牌與通路', '中：品牌、通路與客戶基礎', '景氣循環、成本、匯率與競爭', 1.02, '傳產、民生、服務', '產業鏈位置待補', '傳統產業供應商', '一般消費者、企業與通路客戶', 'AI直接關聯較低'),
}
try:
    STOCK_DB.update(V250_EXTRA_STOCKS)
    ALIASES.clear()
    for _sym, _v in STOCK_DB.items():
        ALIASES[_sym.upper()] = _sym
        ALIASES[_sym.split('.')[0]] = _sym
    ...
    pass
# ===== V250.0 1000 STOCK PHASE 2 + VALUATION COVERAGE END =====

# ===== V251.0 EXECUTION ORDER FIX START =====
# Important: previous V250 called main() before V248/V249/V250 expansion dictionaries were applied.
# This version moves main() to the very end, so all STOCK_DB.update(...) blocks execute before UI statistics render.
# Also fix 光洋科 ticker from 1785.TW to 1785.TWO to avoid Yahoo Finance N/A caused by wrong market suffix.
try:
    if "1785.TW" in STOCK_DB and "1785.TWO" not in STOCK_DB:
        STOCK_DB["1785.TWO"] = STOCK_DB.pop("1785.TW")
    if "1785.TWO" in STOCK_DB:
        STOCK_DB["1785.TWO"]["name"] = "光洋科"
    ALIASES.clear()
    for _sym, _v in STOCK_DB.items():
        ALIASES[_sym.upper()] = _sym
        ALIASES[_sym.split('.')[0]] = _sym
...
# Override settings note for this specific fix.
def settings_page():
    st.header('⚙️ 設定')
    st.write('系統版本：', APP_VERSION)
    st.write('資料庫版本：', DB_VERSION)
    st.write('資料庫股票數：', len(STOCK_DB))
    st.write('本版修正：修正 V250 執行順序，讓後段新增資料庫真正生效；修正光洋科代號為 1785.TWO；保留首頁估值查詢。')
    try: v240_system_dashboard(v230_rows_df())
    except Exception: pass
    st.info('若現價仍為 N/A，通常是 Yahoo Finance 暫時無法回傳該股票價格；估值倍率是資料庫預設產業/競爭力倍率，需有現價才會計算安全邊際價、合理價與樂觀價。')

# ===== V251.0 EXECUTION ORDER FIX END =====


# ===== V252.0 VALUATION LOGIC FIX START =====
# 修正：不再把 fair_mult 顯示成「使用倍率」並假裝是正式估值。
# 原本的 price * fair_mult 只能視為「快速參考區間」，不能拿來做低估排行。
# V252 起：
# 1) 單股若能取得 EPS/每股盈餘資料，優先用 EPS × 合理本益比區間估值。
# 2) 若沒有 EPS，但有現價，只顯示「現價參考區間」，並清楚標示非正式估值。
# 3) 若沒有現價，估值區間與預期報酬顯示 N/A，且不列入低估排行。
# 4) 低估排行只收 valuation_quality == "正式估值" 的資料，避免倍率假訊號。

V252_PE_MAP = {
    '半導體': (14, 20, 28), 'IC設計': (12, 18, 26), 'AI伺服器/ODM': (10, 15, 22),
    'AI伺服器/品牌': (10, 15, 22), 'AI PC/品牌': (10, 14, 20), '電源管理': (16, 22, 30),
    '電子材料': (12, 18, 26), 'PCB': (10, 15, 22), 'PCB/載板': (10, 15, 22),
    '散熱': (14, 20, 28), '光通訊/CPO': (14, 22, 32), '網通': (12, 18, 24),
    '被動元件': (10, 14, 20), '記憶體': (8, 12, 18), '金融': (9, 12, 15),
    '航運': (5, 8, 11), '航空': (8, 12, 16), '鋼鐵': (8, 11, 15), '塑化': (8, 11, 15),
    '水泥/建材': (10, 13, 17), '食品': (14, 18, 24), '食品/通路': (15, 20, 26),
    '通路/零售': (18, 24, 32), '營建/資產': (7, 10, 13), '汽車/通路': (10, 14, 18),
    '紡織': (10, 14, 18), '製鞋/紡織': (10, 14, 18), '生技醫療': (12, 20, 30),
    '電力/重電': (12, 18, 26), '自動化/機器人': (12, 18, 26),
}

def v252_industry_pe(info):
    ind = str(info.get('industry',''))
    sub = str(info.get('sub',''))
    for key, vals in V252_PE_MAP.items():
        if key in ind or key in sub:
            return vals
    return (8, 12, 16)

def v252_parse_num(x):
    try:
        if x is None: return float('nan')
        if isinstance(x, str):
            x = x.replace(',', '').replace('元','').strip()
            if x in ['', 'N/A', '待補', '-']: return float('nan')
        return float(x)
    except Exception:
        return float('nan')

@st.cache_data(ttl=3600, show_spinner=False)
def v252_eps_from_yahoo(symbol):
    try:
        t = yf.Ticker(symbol)
        info = getattr(t, 'info', {}) or {}
        for k in ['forwardEps', 'trailingEps']:
            v = v252_parse_num(info.get(k))
...
def v252_decision(symbol):
    sym = normalize_symbol(symbol)
    info = STOCK_DB.get(sym, {})
    base = dict(info)
    base.setdefault('symbol', sym)
    base.setdefault('name', info.get('name', sym))
    base.setdefault('industry', info.get('industry', '待補'))
    base.setdefault('sub', info.get('sub', '待補'))
    base.setdefault('power', info.get('power', '★★★☆☆'))
    base.setdefault('rank', info.get('rank', '待補'))
    base.setdefault('position', info.get('position', '待補'))
    base.setdefault('peers', info.get('peers', '待補'))
    base.setdefault('moat', info.get('moat', '待補'))
    base.setdefault('risk', info.get('risk', '待補'))

    # 取得現價
    try:
        price = v233_get_live_price(sym)
    except Exception:
        try: price = live_price(sym)
        except Exception: price = float('nan')

    eps = v252_parse_num(info.get('eps_2026', info.get('eps', info.get('forward_eps'))))
    eps_source = '資料庫EPS'
    if pd.isna(eps) or eps <= 0:
        eps, eps_key = v252_eps_from_yahoo(sym)
        eps_source = f'Yahoo {eps_key}' if eps_key != 'N/A' else 'N/A'

    pe_low = v252_parse_num(info.get('pe_low'))
    pe_mid = v252_parse_num(info.get('pe_mid'))
    pe_high = v252_parse_num(info.get('pe_high'))
    if pd.isna(pe_low) or pd.isna(pe_mid) or pd.isna(pe_high):
        pe_low, pe_mid, pe_high = v252_industry_pe(info)

    cons = fair = opt = ret = float('nan')
    action = '資料不足'
    valuation_method = '暫無正式估值：缺少有效現價或 EPS'
    valuation_quality = '待補'
    source = 'Yahoo Finance'

    if pd.notna(price) and float(price) > 0 and pd.notna(eps) and float(eps) > 0:
        cons = float(eps) * float(pe_low)
        fair = float(eps) * float(pe_mid)
        opt = float(eps) * float(pe_high)
        ret = (fair / float(price) - 1) * 100
        action = '買進' if ret >= 20 else ('觀察偏多' if ret >= 8 else ('觀察' if ret >= -10 else '偏貴/保守'))
        valuation_method = f'EPS × 本益比區間；EPS來源：{eps_source}'
        valuation_quality = '正式估值'
    elif pd.notna(price) and float(price) > 0:
        # 沒有 EPS 時，不再用 fair_mult 推合理價；只保留現價附近參考區間，避免誤導成低估/高估。
        cons = float(price) * 0.90
        fair = float(price)
        opt = float(price) * 1.10
        ret = 0.0
        action = '觀察'
        valuation_method = '現價參考區間：缺少 EPS/財測，尚非正式合理價'
        valuation_quality = '快速參考'
    else:
        source = 'Yahoo Finance N/A'

    base.update({
        'symbol': sym, 'price': price, 'cons': cons, 'fair': fair, 'opt': opt, 'ret': ret,
        'action': action, 'rating': base.get('power','★★★☆☆'), 'source': source, 'updated': tw_now(),
        'valuation_method': valuation_method, 'valuation_quality': valuation_quality,
        'eps_used': eps, 'pe_low': pe_low, 'pe_mid': pe_mid, 'pe_high': pe_high,
    })
    return base

# 覆蓋所有舊決策入口，避免不同頁面又回到 fair_mult × 現價
def decision(symbol): return v252_decision(symbol)
def v224_decision(symbol): return v252_decision(symbol)
def v223_decision(symbol): return v252_decision(symbol)
def v230_decision(symbol): return v252_decision(symbol)
def v233_decision(symbol): return v252_decision(symbol)
...
def v231_rank_table(df):
    ...
    for _, r in df.iterrows():
        ...
        rows.append({
            ...
            '預期報酬%': d.get('ret', float('nan')), 'AI受惠度': r.get('AI受惠度',0),
...
def v247_query_panel(symbol):
    ...
    valuation_row = [{
        '估值方法': d.get('valuation_method','待補'),
        'EPS/每股盈餘': eps_txt,
        '本益比區間': pe_txt,
        '區間定義': '安全邊際價 / 合理價 / 樂觀價',
        '資料可信度': 'A/B：EPS可取得' if d.get('valuation_quality') == '正式估值' else 'C：僅供快速參考，不列入低估排行',
        '重要說明': 'V252起不再使用 fair_mult 當正式估值倍率'
    }]
    st.dataframe(pd.DataFrame(valuation_row), use_container_width=True, hide_index=True)

    summary = [{
        '產業': info.get('industry', '待補'), '子產業': info.get('sub', '待補'),
        '產業地位': info.get('position', info.get('rank','待補')),
        '全球排名/市占率': f"{info.get('global_rank', info.get('rank','待補'))} / {info.get('market_share', info.get('global_share','待補'))}",
        '主要客戶摘要': info.get('customers', info.get('major_customers', '待補')),
    }]
    ...
    with st.expander('展開完整公司資料：主要客戶 / AI客戶 / 競爭者 / 風險', expanded=False):
        ...
        st.write('護城河：', info.get('moat', '待補'))
        st.write('主要風險：', info.get('risk', '待補'))
        st.caption(f"資料更新時間：{d.get('updated','N/A')}｜現價來源：{d.get('source','Yahoo Finance')}｜正式估值需 EPS/財測；缺少 EPS 者只顯示快速參考區間。")
...
def settings_page():
    st.header('⚙️ 設定')
    st.write('系統版本：', APP_VERSION)
    st.write('資料庫版本：', DB_VERSION if 'DB_VERSION' in globals() else 'TW-STOCK-20260628-V252')
    st.write('資料庫股票數：', len(STOCK_DB))
    st.info('V252修正：fair_mult 只保留為產業/競爭力參考，不再當正式估值倍率；低估排行只列入 EPS × PE 可計算的正式估值股票。')
    try: v240_system_dashboard(v230_rows_df())
    except Exception: pass

# ===== V252.0 VALUATION LOGIC FIX END =====


# ===== V253.0 FORMAL VALUATION GUARD + RANKING FIX START =====
# 重要修正：
# 1) 禁止用 Yahoo EPS 自動估正式合理價，避免 EPS 異常造成數千%低估排行。
# 2) 只有資料庫明確提供 eps_2026 / eps_forecast / pe_low / pe_mid / pe_high 的個股，才產生正式估值區間。
# 3) 其餘股票只顯示「現價」與「估值待補」，不產生安全邊際價/合理價/樂觀價，且不列入低估排行。
# 4) fair_mult 改名為「產業參考係數」，不再顯示為使用倍率，也不參與估值。

APP_VERSION = "V253.0 Formal Valuation Guard + Ranking Fix"
DB_VERSION = "TW-STOCK-20260628-V253"

FORMAL_EPS_KEYS = ["eps_2026", "eps_forecast", "eps_est", "formal_eps"]
FORMAL_PE_KEYS = ["pe_low", "pe_mid", "pe_high"]

def v253_has_formal_inputs(info):
    if not isinstance(info, dict):
        return False
    eps = None
    for k in FORMAL_EPS_KEYS:
        if k in info and info.get(k) not in [None, "", "待補", "N/A"]:
            eps = v252_parse_num(info.get(k))
            break
    if eps is None or pd.isna(eps) or float(eps) <= 0:
        return False
    # 必須三個 PE 欄位都存在，避免用產業預設 PE 假裝正式估值
    for k in FORMAL_PE_KEYS:
        if k not in info or info.get(k) in [None, "", "待補", "N/A"]:
            return False
        val = v252_parse_num(info.get(k))
        if val is None or pd.isna(val) or float(val) <= 0:
            return False
    return True

def v253_formal_inputs(info):
    eps = float('nan')
    eps_key = '待補'
    for k in FORMAL_EPS_KEYS:
        val = v252_parse_num(info.get(k)) if isinstance(info, dict) else float('nan')
        if pd.notna(val) and float(val) > 0:
            eps, eps_key = float(val), k
            break
    return eps, eps_key, float(v252_parse_num(info.get('pe_low'))), float(v252_parse_num(info.get('pe_mid'))), float(v252_parse_num(info.get('pe_high')))

def v253_decision(symbol):
    try:
        sym = v230_symbol(symbol)
    except Exception:
        sym = normalize_symbol(symbol)
    info = STOCK_DB.get(sym, {'name': sym, 'industry': '待補', 'sub': '待補', 'rank': '待補', 'power': '★★★☆☆', 'position': '待補', 'peers': '待補', 'moat': '待補', 'risk': '資料庫尚待補齊'})
    price = v233_get_live_price(sym)
    if pd.isna(price) or not price or float(price) <= 0:
        try:
            ...
            price = float('nan')
    source = 'Yahoo Finance' if pd.notna(price) and float(price) > 0 else 'Yahoo Finance N/A'

    cons = fair = opt = float('nan')
    ret = float('nan')
    action = '觀察'
    valuation_quality = '估值待補'
    valuation_method = '正式估值待補：尚未輸入 EPS 預估與本益比區間；不列入低估排行'
    eps_used = float('nan')
    pe_low = pe_mid = pe_high = float('nan')

    if v253_has_formal_inputs(info):
        eps_used, eps_key, pe_low, pe_mid, pe_high = v253_formal_inputs(info)
        cons = eps_used * pe_low
        fair = eps_used * pe_mid
        opt = eps_used * pe_high
        if pd.notna(price) and float(price) > 0:
            ret = (fair / float(price) - 1) * 100
        action = '買進' if pd.notna(ret) and ret >= 20 else ('觀察偏多' if pd.notna(ret) and ret >= 8 else ('觀察' if pd.notna(ret) and ret >= -10 else '偏貴/保守'))
        valuation_quality = '正式估值'
        valuation_method = f'EPS × 本益比區間；EPS欄位：{eps_key}'

    out = dict(info)
    out.update({
        'symbol': sym, 'price': price, 'cons': cons, 'fair': fair, 'opt': opt, 'ret': ret,
        'action': action, 'rating': info.get('power', '★★★☆☆'), 'source': source, 'updated': tw_now(),
        'valuation_method': valuation_method, 'valuation_quality': valuation_quality,
        'eps_used': eps_used, 'pe_low': pe_low, 'pe_mid': pe_mid, 'pe_high': pe_high,
        'industry_factor_note': info.get('fair_mult', '待補'),
    })
    return out

# 全站決策入口統一改用 V253，避免舊頁面又回到 fair_mult 或 Yahoo EPS 估值
def decision(symbol): return v253_decision(symbol)
def v224_decision(symbol): return v253_decision(symbol)
def v223_decision(symbol): return v253_decision(symbol)
def v230_decision(symbol): return v253_decision(symbol)
def v233_decision(symbol): return v253_decision(symbol)
def v252_decision(symbol): return v253_decision(symbol)

def v231_rank_table(df):
    # 低估排行只允許正式估值；且排除報酬率異常值，避免資料錯誤造成誤導
    rows = []
    for _, r in df.iterrows():
        sym = r.get('代碼', '')
        d = v253_decision(sym)
        if d.get('valuation_quality') != '正式估值':
            continue
        px, fair, ret = d.get('price'), d.get('fair'), d.get('ret')
        if pd.isna(px) or pd.isna(fair) or pd.isna(ret) or float(px) <= 0:
            continue
        if float(ret) < -80 or float(ret) > 300:
            continue
        rows.append({
            '公司': r.get('公司',''), '代碼': sym, '產業': r.get('產業',''), '子產業': r.get('子產業',''),
            '現價': px, '綜合合理價': fair, '預期報酬%': ret,
            '估值狀態': d.get('valuation_quality'), 'AI受惠度': r.get('AI受惠度',0),
            '全球競爭力': r.get('全球競爭力',''), '主題標籤': r.get('主題標籤',''),
        })
    out = pd.DataFrame(rows)
    for col in ['現價','綜合合理價','預期報酬%','AI受惠度']:
...
def v247_query_panel(symbol):
    ...
    c5.metric('預期報酬', ret_txt)
    c6.metric('投資建議', d.get('action', '觀察'))
    c7.metric('估值狀態', d.get('valuation_quality', '估值待補'))
    c8.metric('全球競爭力', info.get('power', '待補'))
    ...
    valuation_row = [{
        ...
        'EPS/每股盈餘': eps_txt,
        '本益比區間': pe_txt,
        '產業參考係數': str(d.get('industry_factor_note','待補')),
        '重要說明': '產業參考係數不等於估值倍率；未輸入EPS與PE者不產生合理價、不列入低估排行'
    }]
    st.dataframe(pd.DataFrame(valuation_row), use_container_width=True, hide_index=True)
    if d.get('valuation_quality') != '正式估值':
        st.warning('此股目前只有現價與產業資料，正式估值區間尚待補 EPS / 財測 / 合理本益比；為避免誤導，低估排行不納入。')
    ...
    with st.expander('展開完整公司資料：主要客戶 / AI客戶 / 競爭者 / 風險', expanded=False):
        ...
        st.write('護城河：', info.get('moat', '待補'))
        st.write('主要風險：', info.get('risk', '待補'))
        st.caption(f"資料更新時間：{d.get('updated','N/A')}｜現價來源：{d.get('source','Yahoo Finance')}｜正式估值需人工/財報欄位：EPS + PE區間。")
...
# 修正資料庫完成度：完成估值只計入正式估值，不再把 fair_mult 視為完成估值
def v240_completion_stats(df=None):
    ...
    with_ai_customer = sum(1 for v in STOCK_DB.values() if v.get('ai_customers'))
    with_valuation = sum(1 for v in STOCK_DB.values() if v253_has_formal_inputs(v))
    with_comp = sum(1 for v in STOCK_DB.values() if v.get('peers') or v.get('moat') or v.get('position'))
    return {'total': total, 'with_customer': with_customer, 'with_ai_customer': with_ai_customer, 'with_valuation': with_valuation, 'with_competition': with_comp}
...
def settings_page():
    ...
    st.write('正式估值完成：', f"{stats['with_valuation']} / {stats['total']}")
    st.info('V253修正：不再使用 fair_mult 或 Yahoo EPS 自動產生正式合理價；低估排行只納入資料庫明確有 EPS 與 PE 區間的個股。')
    try: v240_system_dashboard(v230_rows_df())
    except Exception: pass
...
def model_validation_alpha_page():
    ...
    with st.expander('目前各單一模型估值', expanded=False):
        if not cur.empty:
            ...
            st.dataframe(show, use_container_width=True, hide_index=True)
    with st.expander('單一模型逐年明細', expanded=False):
        st.dataframe(detail.round(2), use_container_width=True, hide_index=True)
    st.divider()
    v254_health_dashboard()
...
# ===== V259.0 FINANCIAL FACTOR ENGINE START =====
APP_VERSION = "V259.0 Financial Factor Engine"
DB_VERSION = "TW-STOCK-20260628-V259"

# V259 目標：先把台積電財報因子補齊，讓 PB / EV-EBITDA / FCFF / FCFE / EVA / EBO
# 不再完全依賴 Proxy。此版為 Alpha Seed：EPS/營收/淨利採年報，部分資產與現金流因子先用
# yfinance 補抓，抓不到時使用保守種子資料並標示資料來源與完整度，避免模型假裝「正式」。

V259_REQUIRED_FACTORS = [
    'EPS','Revenue_BN','NetIncome_BN','BVPS','ROE','ROA','EBITDA_PS','OCF_PS','FCF_PS',
    'FCFF_PS','FCFE_PS','CAPEX_PS','ROIC','WACC','NOPAT_PS','NetDebt_PS'
]

V259_TSMC_FACTOR_SEED = pd.DataFrame([
    # Revenue / Net income：新台幣十億元；EPS：新台幣元；平均股價 fallback 用先前版本估計值。
    # BVPS/現金流/EBITDA 為 Alpha seed，V260 起建議逐項由財報與 yfinance 校正。
    {'年度':2021,'EPS':23.01,'Revenue_BN':1587.42,'NetIncome_BN':596.54,'BVPS':92.8,'ROE':0.296,'ROA':0.175,'EBITDA_PS':38.5,'OCF_PS':42.0,'CAPEX_PS':32.0,'FCF_PS':10.0,'FCFF_PS':12.0,'FCFE_PS':9.0,'ROIC':0.245,'WACC':0.085,'NetDebt_PS':-22.0,'fallback_avg_price':596.0,'資料來源':'年報EPS/營收/淨利 + Alpha seed'},
    {'年度':2022,'EPS':39.20,'Revenue_BN':2263.89,'NetIncome_BN':1016.53,'BVPS':123.4,'ROE':0.356,'ROA':0.226,'EBITDA_PS':61.0,'OCF_PS':69.0,'CAPEX_PS':42.0,'FCF_PS':27.0,'FCFF_PS':29.0,'FCFE_PS':25.0,'ROIC':0.305,'WACC':0.090,'NetDebt_PS':-30.0,'fallback_avg_price':503.0,'資料來源':'年報EPS/營收/淨利 + Alpha seed'},
    {'年度':2023,'EPS':32.34,'Revenue_BN':2161.74,'NetIncome_BN':838.50,'BVPS':146.0,'ROE':0.240,'ROA':0.155,'EBITDA_PS':55.0,'OCF_PS':58.0,'CAPEX_PS':37.0,'FCF_PS':21.0,'FCFF_PS':23.0,'FCFE_PS':19.0,'ROIC':0.205,'WACC':0.090,'NetDebt_PS':-36.0,'fallback_avg_price':552.0,'資料來源':'年報EPS/營收/淨利 + Alpha seed'},
    {'年度':2024,'EPS':45.25,'Revenue_BN':2894.31,'NetIncome_BN':1173.27,'BVPS':182.0,'ROE':0.276,'ROA':0.171,'EBITDA_PS':76.0,'OCF_PS':82.0,'CAPEX_PS':39.0,'FCF_PS':43.0,'FCFF_PS':45.0,'FCFE_PS':40.0,'ROIC':0.235,'WACC':0.090,'NetDebt_PS':-42.0,'fallback_avg_price':879.0,'資料來源':'年報EPS/營收/淨利 + Alpha seed'},
    {'年度':2025,'EPS':66.25,'Revenue_BN':3809.05,'NetIncome_BN':1717.88,'BVPS':238.0,'ROE':0.315,'ROA':0.205,'EBITDA_PS':108.0,'OCF_PS':116.0,'CAPEX_PS':48.0,'FCF_PS':68.0,'FCFF_PS':70.0,'FCFE_PS':64.0,'ROIC':0.275,'WACC':0.090,'NetDebt_PS':-52.0,'fallback_avg_price':1260.0,'資料來源':'年報EPS/營收/淨利 + Alpha seed'},
])

V259_MODEL_REQUIREMENTS = {
    'PE(歷史中位數)': ['EPS'],
    'PE(近三年加權)': ['EPS'],
    'Forward PE(AI調整)': ['EPS'],
    'PB(真BVPS)': ['BVPS'],
    'EV/EBITDA(真因子)': ['EBITDA_PS','NetDebt_PS'],
    'DCF-FCFF(真因子)': ['FCFF_PS','WACC'],
    'DCF-FCFE(真因子)': ['FCFE_PS','WACC'],
    'EBO/RIM(真因子)': ['BVPS','ROE','WACC'],
    'EVA(真因子)': ['NOPAT_PS','ROIC','WACC','BVPS'],
    'FCF Yield': ['FCF_PS'],
    'PEG(成長修正)': ['EPS'],
    'AI Premium PE': ['EPS'],
}
...
V259_MODEL_GROUP = {
    'PE(歷史中位數)':'市場派', 'PE(近三年加權)':'市場派', 'Forward PE(AI調整)':'成長派',
    'PB(真BVPS)':'價值派', 'EV/EBITDA(真因子)':'價值派', 'DCF-FCFF(真因子)':'現金流派',
    'DCF-FCFE(真因子)':'現金流派', 'EBO/RIM(真因子)':'價值派', 'EVA(真因子)':'價值派',
    'FCF Yield':'現金流派', 'PEG(成長修正)':'成長派', 'AI Premium PE':'特殊派'
}

@st.cache_data(ttl=3600, show_spinner=False)
def v259_price_history():
    df = V259_TSMC_FACTOR_SEED.copy()
    try:
        hist = yf.Ticker('2330.TW').history(start='2021-01-01', end='2026-01-01', interval='1d', auto_adjust=False)
        if hist is not None and not hist.empty and 'Close' in hist:
            hist = hist.copy(); hist.index = pd.to_datetime(hist.index)
    ...
    df['平均股價'] = df['平均股價'].fillna(df['fallback_avg_price'])
    df['PE'] = df['平均股價'] / df['EPS']
    df['PB'] = df['平均股價'] / df['BVPS']
    df['P_EBITDA'] = df['平均股價'] / df['EBITDA_PS']
    df['P_FCF'] = df['平均股價'] / df['FCF_PS'].replace(0, np.nan)
    df['EPS成長率'] = df['EPS'].pct_change()
    df['Revenue成長率'] = df['Revenue_BN'].pct_change()
    df['NOPAT_PS'] = df.get('NOPAT_PS', df['EPS'] * 1.08)
    df['NOPAT_PS'] = pd.to_numeric(df['NOPAT_PS'], errors='coerce').fillna(df['EPS']*1.08)
    return df

@st.cache_data(ttl=3600, show_spinner=False)
def v259_try_yfinance_factor_snapshot():
    """嘗試從 Yahoo 財報補抓最近年度欄位；失敗不影響頁面。"""
    out = {}
    try:
        t = yf.Ticker('2330.TW')
        ...
        out['financials_cols'] = 0 if fin is None else len(fin.columns)
        out['cashflow_cols'] = 0 if cf is None else len(cf.columns)
        out['balance_sheet_cols'] = 0 if bs is None else len(bs.columns)
    except Exception as e:
        out['error'] = str(e)[:120]
    return out

def v259_factor_completeness(df):
    rows=[]
    for fac in V259_REQUIRED_FACTORS:
        ok = fac in df.columns and pd.to_numeric(df[fac], errors='coerce').notna().sum() == len(df)
        rows.append({'財報因子':fac,'狀態':'完成' if ok else '缺資料','完成年度數': int(pd.to_numeric(df[fac], errors='coerce').notna().sum()) if fac in df.columns else 0, '總年度數':len(df)})
    res=pd.DataFrame(rows)
    score=float((res['狀態']=='完成').mean()*100) if not res.empty else 0.0
    return res, score

def v259_model_availability(df):
    rows=[]
    for m, req in V259_MODEL_REQUIREMENTS.items():
        missing=[r for r in req if r not in df.columns or pd.to_numeric(df[r], errors='coerce').isna().any()]
        rows.append({'模型':m,'模型群組':V259_MODEL_GROUP.get(m,'其他'),'需要因子':'、'.join(req),'模型狀態':'ENABLED' if not missing else 'DATA_MISSING','缺少因子':'、'.join(missing)})
    return pd.DataFrame(rows)
...
def _safe_mean(s, default=np.nan):
    s=pd.to_numeric(s, errors='coerce').replace([np.inf,-np.inf], np.nan).dropna()
    return float(s.mean()) if len(s) else default

def v259_dcf_per_share(base_ps, wacc=0.09, growth=0.10, terminal_g=0.035, years=5):
    if pd.isna(base_ps) or base_ps <= 0 or wacc <= terminal_g:
        return np.nan
    val=0.0
    cf=float(base_ps)
    for t in range(1, years+1):
        cf *= (1+growth)
        val += cf / ((1+wacc)**t)
    tv = cf * (1+terminal_g) / max(wacc-terminal_g, 0.01)
    val += tv / ((1+wacc)**years)
    return float(val)

def v259_model_value(row, past, model):
    eps=float(row['EPS']); price=float(row.get('平均股價', np.nan))
    pe_med=_safe_median(past.get('PE', pd.Series(dtype=float)), 22)
    pe_3y=_safe_median(past.get('PE', pd.Series(dtype=float)).tail(3), pe_med)
    pb_med=_safe_median(past.get('PB', pd.Series(dtype=float)), 5.0)
    ebitda_mult=_safe_median(past.get('P_EBITDA', pd.Series(dtype=float)), 15.0)
    fcf_mult=_safe_median(past.get('P_FCF', pd.Series(dtype=float)), 28.0)
    g=_safe_median(past.get('EPS成長率', pd.Series(dtype=float)).tail(3), 0.10)
    g=float(min(max(g, -0.10), 0.35))
    wacc=float(row.get('WACC', 0.09) or 0.09)
    if model == 'PE(歷史中位數)':
        return eps * pe_med
    if model == 'PE(近三年加權)':
        return eps * pe_3y * (1 + max(g,0)*0.20)
    if model == 'Forward PE(AI調整)':
        return eps * (1+max(g,0)*0.75) * min(max(pe_3y*1.10, pe_med), 36)
    if model == 'PB(真BVPS)':
        return float(row['BVPS']) * pb_med
    if model == 'EV/EBITDA(真因子)':
        # 每股股權價值近似 = EBITDA/share * 倍數 - net debt/share；淨現金為負net debt，會加回價值。
        return float(row['EBITDA_PS']) * ebitda_mult - float(row.get('NetDebt_PS', 0))
    if model == 'DCF-FCFF(真因子)':
        return v259_dcf_per_share(float(row['FCFF_PS']), wacc=wacc, growth=max(g,0.06), terminal_g=0.035)
    if model == 'DCF-FCFE(真因子)':
        return v259_dcf_per_share(float(row['FCFE_PS']), wacc=max(wacc+0.005,0.09), growth=max(g,0.06), terminal_g=0.035)
    if model == 'EBO/RIM(真因子)':
        bv=float(row['BVPS']); roe=float(row['ROE']); coe=max(wacc+0.005,0.09)
        # 5年 residual income + terminal，ROE 緩步回歸 18%
        val=bv; book=bv
        for t in range(1,6):
            roe_t = roe - (roe-0.18)*(t-1)/5
            ri = (roe_t - coe) * book
            val += ri / ((1+coe)**t)
            book += max(eps*0.55, 0)  # 保留盈餘近似
        terminal_ri = max((0.18-coe)*book, 0) / max(coe-0.035, 0.01)
        val += terminal_ri / ((1+coe)**5)
        return float(val)
    if model == 'EVA(真因子)':
        nopat=float(row['NOPAT_PS']); roic=float(row['ROIC']); capital=float(row['BVPS'])
        spread=max(roic-wacc, -0.05)
        eva=spread*capital
        return capital + (eva / max(wacc-0.025, 0.02)) + nopat*2.0
    if model == 'FCF Yield':
        return float(row['FCF_PS']) * fcf_mult
    if model == 'PEG(成長修正)':
        peg_pe=min(max(g*100*1.05, 12), 38)
        return eps*peg_pe
    if model == 'AI Premium PE':
        ai_pe=min(max(pe_3y*1.25, 26), 42)
        return eps*ai_pe
    return np.nan

def v259_backtest():
    df=v259_price_history().copy().sort_values('年度').reset_index(drop=True)
    avail=v259_model_availability(df)
    enabled=avail[avail['模型狀態']=='ENABLED']['模型'].tolist()
    details=[]
    for i in range(1,len(df)):
        row=df.iloc[i]
        past=df.iloc[:i]
        actual=float(row['平均股價'])
        for m in enabled:
            val=v259_model_value(row,past,m)
            if pd.notna(val) and val>0 and actual>0:
                details.append({'年度':int(row['年度']),'模型':m,'模型群組':V259_MODEL_GROUP.get(m,'其他'),'模型估值':float(val),'實際平均股價':actual,
                                '誤差%':abs(float(val)/actual-1)*100,'偏離%':(float(val)/actual-1)*100})
    detail=pd.DataFrame(details)
    if detail.empty:
        return df, avail, detail, pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    summary=detail.groupby(['模型','模型群組']).agg(MAPE=('誤差%','mean'),RMSE=('誤差%', lambda x: float(np.sqrt(np.mean(np.square(x))))),最大偏離=('偏離%', lambda x: float(np.max(np.abs(x)))),測試年數=('誤差%','count')).reset_index().sort_values('MAPE')
    summary['評級']=summary['MAPE'].apply(v259_grade)
    combos=v259_combo_backtest(detail, summary)
    current=v259_current_composites(df, summary)
    return df, avail, detail, summary, combos, current

def v259_combo_backtest(detail, summary):
    out=[]
    years=sorted(detail['年度'].unique())
    for y in years:
        past=detail[detail['年度']<y]
        ...
        if past.empty or curr.empty: continue
        rank=past.groupby(['模型','模型群組']).agg(MAPE=('誤差%','mean')).reset_index().sort_values('MAPE')
        champ=rank.sort_values('MAPE').groupby('模型群組', as_index=False).head(1).sort_values('MAPE')
        schemes={
            '單一最佳': rank.head(1)['模型'].tolist(),
            'Top3': rank.head(3)['模型'].tolist(),
            'Top5': rank.head(5)['模型'].tolist(),
            '群組冠軍': champ['模型'].tolist(),
            '有效S/A模型': rank[rank['MAPE']<15]['模型'].tolist(),
        }
        actual=float(curr['實際平均股價'].iloc[0])
        for name, chosen in schemes.items():
            ...
            c['權重']=c['權重']/c['權重'].sum()
            fair=float((c['模型估值']*c['權重']).sum())
            out.append({'年度':int(y),'方案':name,'使用模型':'、'.join(c['模型'].astype(str).tolist()),'模型估值':fair,'實際平均股價':actual,'誤差%':abs(fair/actual-1)*100,'偏離%':(fair/actual-1)*100})
    if not out: return pd.DataFrame()
    res=pd.DataFrame(out)
    summ=res.groupby('方案').agg(MAPE=('誤差%','mean'),RMSE=('誤差%', lambda x: float(np.sqrt(np.mean(np.square(x))))),測試年數=('誤差%','count')).reset_index().sort_values('MAPE')
    summ['評級']=summ['MAPE'].apply(v259_grade)
    return summ

def v259_current_composites(df, summary):
    price=v254_decision('2330.TW').get('price',np.nan)
    if pd.isna(price) or price<=0: price=float(df['平均股價'].iloc[-1])
    latest=df.iloc[-1].copy()
    # Forward 2026 alpha：用 2025 EPS 加上保守AI成長假設；後續可改為法人預估EPS。
    eps_growth=_safe_median(df['EPS成長率'].tail(3),0.18)
    latest['EPS']=float(latest['EPS'])*(1+min(max(eps_growth,0.08),0.28))
    latest['FCFF_PS']=float(latest['FCFF_PS'])*(1+min(max(eps_growth,0.08),0.25))
    latest['FCFE_PS']=float(latest['FCFE_PS'])*(1+min(max(eps_growth,0.08),0.25))
    latest['EBITDA_PS']=float(latest['EBITDA_PS'])*(1+min(max(eps_growth,0.08),0.25))
    cur=[]
    for _,s in summary.sort_values('MAPE').iterrows():
        m=s['模型']; val=v259_model_value(latest, df, m)
        if pd.notna(val) and val>0:
            cur.append({'模型':m,'模型群組':s['模型群組'],'MAPE%':round(float(s['MAPE']),2),'評級':s['評級'],'合理價':float(val),'現價':float(price),'與現價偏離%':(float(val)/float(price)-1)*100})
    cur=pd.DataFrame(cur).sort_values('MAPE%') if cur else pd.DataFrame()
    if cur.empty: return pd.DataFrame()
    schemes={
        '單一最佳': cur.head(1)['模型'].tolist(),
        'Top3': cur.head(3)['模型'].tolist(),
        'Top5': cur.head(5)['模型'].tolist(),
        '群組冠軍': cur.groupby('模型群組', as_index=False).head(1)['模型'].tolist(),
        'S/A模型': cur[cur['MAPE%']<15]['模型'].tolist(),
    }
    rows=[]
    for name, chosen in schemes.items():
        if not chosen: continue
        ...
        fair=float((b['合理價']*b['權重']).sum())
        err=float((b['MAPE%']*b['權重']).sum())/100
        rows.append({'方案':name,'使用模型':'、'.join(b['模型'].astype(str).tolist()),'加權MAPE%':round(err*100,2),'安全價':round(fair*(1-err),2),'合理價':round(fair,2),'樂觀價':round(fair*(1+err),2),'現價':round(float(price),2),'與現價偏離%':round((fair/price-1)*100,2)})
    return pd.DataFrame(rows).sort_values('與現價偏離%', key=lambda s: s.abs()) if rows else pd.DataFrame()

def model_validation_alpha_page():
    st.header('🧮 V259 Financial Factor Engine — 台積電財報因子引擎')
    st.info('V259：不再只看 31 模型數量，先補台積電財報因子，啟用 PB、EV/EBITDA、FCFF、FCFE、EBO、EVA 等真因子模型。此版仍屬 Alpha，部分因子為 seed，頁面會標示完整度。')
    st.selectbox('選擇測試公司', ['2330.TW'], index=0, format_func=lambda s: f"{STOCK_DB.get(s,{}).get('name',s)} / {s}")
    df, avail, detail, summary, combos, current = v259_backtest()
    factor_status, factor_score = v259_factor_completeness(df)
    price=v254_decision('2330.TW').get('price',np.nan)
    c1,c2,c3,c4=st.columns(4)
    c1.metric('目前現價', v230_fmt(price))
    c2.metric('財報因子完整度', f'{factor_score:.0f}%')
    c3.metric('可啟用模型', f"{(avail['模型狀態']=='ENABLED').sum()} / {len(avail)}")
    c4.metric('最新EPS', f"{df['EPS'].iloc[-1]:.2f}")

    with st.expander('一、台積電財報因子庫 2021~2025', expanded=True):
        show=df[['年度','EPS','Revenue_BN','NetIncome_BN','BVPS','ROE','EBITDA_PS','FCFF_PS','FCFE_PS','FCF_PS','ROIC','WACC','平均股價','PE','PB','資料來源']].copy()
        num_cols=[c for c in show.columns if c not in ['資料來源']]
        for c in num_cols:
            show[c]=pd.to_numeric(show[c], errors='coerce').round(3)
        st.dataframe(show, use_container_width=True, hide_index=True)
        st.caption('注意：EPS/營收/淨利為年報資料；BVPS/現金流/EBITDA/ROIC 為 V259 Alpha seed 或 yfinance 待校正欄位，正式版需逐年對齊財報。')

    colA,colB=st.columns(2)
    with colA:
        st.subheader('財報因子完整度')
        st.dataframe(factor_status, use_container_width=True, hide_index=True)
    with colB:
        st.subheader('模型啟用狀態')
        st.dataframe(avail, use_container_width=True, hide_index=True)
        yf_info=v259_try_yfinance_factor_snapshot()
        st.caption(f"Yahoo 財報連線狀態：{yf_info}")

    st.subheader('二、真因子模型回測排名')
    if summary.empty:
        st.warning('資料不足，無法回測。')
    else:
        st.dataframe(summary.round(2), use_container_width=True, hide_index=True)
        champ=summary.sort_values('MAPE').groupby('模型群組', as_index=False).head(1).sort_values('MAPE')
        st.subheader('三、各流派冠軍')
        st.dataframe(champ.round(2), use_container_width=True, hide_index=True)

    st.subheader('四、組合方案回測')
    if combos.empty:
        st.warning('組合樣本不足。')
    else:
        st.dataframe(combos.round(2), use_container_width=True, hide_index=True)
        best=combos.iloc[0]
        st.success(f"目前較佳組合：{best['方案']}｜MAPE {best['MAPE']:.2f}%｜評級 {best['評級']}")

    st.subheader('五、2026 Alpha 區間估值')
    if current.empty:
        st.warning('目前無法產生區間估值。')
    else:
        st.dataframe(current, use_container_width=True, hide_index=True)
        best=current.iloc[0]
        c1,c2,c3,c4=st.columns(4)
        c1.metric('最接近現價方案', best['方案'])
        c2.metric('安全價', v230_fmt(best['安全價']))
        c3.metric('合理價', v230_fmt(best['合理價']))
        c4.metric('樂觀價', v230_fmt(best['樂觀價']))
        if abs(float(best['與現價偏離%']))>30:
            st.warning('估值仍與現價偏離超過30%。這通常代表 Forward EPS、AI/HPC溢價、CAPEX週期或財報因子仍需校正；暫不建議放入正式低估排行。')
        else:
            st.success('偏離已收斂到可觀察區間，可進一步驗證台達電、聯發科。')
...
def settings_page():
    ...
    st.write('資料庫版本：', DB_VERSION)
    st.write('資料庫股票數：', len(STOCK_DB))
    st.info('V259：Financial Factor Engine，先補台積電財報因子，建立真模型啟用條件與模型健康度。')
    v254_health_dashboard()
...
def sidebar_nav():
    ...
    if q:
        set_active(q)
    st.sidebar.caption('V259：財報因子引擎，先修估值核心，再擴大到AI七雄。')
    return page

# ===== V259.0 FINANCIAL FACTOR ENGINE END =====
